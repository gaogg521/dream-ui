...
`;
  ...
  try {
    ...
    if (!blocking.some((process) => process.name === 'One Work installer' && Number(process.pid) > 0)) {
      throw new Error(`expected One Work installer blocker, got ${JSON.stringify(blocking)}`);
    }
