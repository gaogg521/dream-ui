 ...
 *
 * Env vars:
 *   DREAM_PORT           : static server port (default 33000)
 *   DREAM_HOST           : listen host; set to 0.0.0.0 to imply --remote
 *   DREAM_ALLOW_REMOTE   : "1"/"true" to expose to LAN
 ...
 *   $DREAM_DATA_DIR        env override (same effect)
 *   otherwise               ~/.dream-web         (production)
 *                           ~/.dream-web-dev     (dev, default)
 *                           ~/.dream-web-dev-2   (dev + DREAM_MULTI_INSTANCE=1)
 *
 * Why a dedicated `-web` name, not the same `~/.dream[-dev]` that Electron
 * uses: on macOS, Electron's getDataPath() (packages/desktop/src/process/utils/
 * utils.ts) creates `~/.dream-dev` as a **symlink** to
 * `~/Library/Application Support/Dream UI-Dev/dream` so CLI tools (claude,
 ...
 * `.dream-web` keeps standalone webui's data dir off of the path Electron's
 * symlink needs.
 *
 * If the user wants the two to share data they opt-in explicitly via
 *   --data-dir ~/.dream-dev                     (or equivalent on other OSes)
...
function resolveBackendDataDir(): string {
  const override = getFlag('--data-dir') ?? process.env.DREAM_DATA_DIR;
...
function resolveAllowRemote(): boolean {
  ...
  const host = process.env.DREAM_HOST?.trim();
  if (host && ['0.0.0.0', '::', '::0'].includes(host)) return true;
  return parseBoolean(process.env.DREAM_ALLOW_REMOTE ?? process.env.DREAM_REMOTE);
}

function resolveStaticDir(): string {
  if (process.env.DREAM_STATIC_DIR) return process.env.DREAM_STATIC_DIR;
  const candidate = path.join(repoRoot, 'out', 'renderer');
  if (fs.existsSync(path.join(candidate, 'index.html'))) return candidate;
  throw new Error(`Renderer assets not found at ${candidate}. Run "bun run package" first, or set DREAM_STATIC_DIR.`);
}
 ...
 *   $DREAM_NO_BUILD=1        : env-level opt-out
 *   $DREAM_STATIC_DIR is set : caller is pointing us at a prebuilt artifact dir
 */
function runPackageIfNeeded(): void {
  if (has('--no-build')) return;
  if (parseBoolean(process.env.DREAM_NO_BUILD)) return;
  if (process.env.DREAM_STATIC_DIR) return;
...
async function main(): Promise<void> {
  ...
  // This keeps `bun run webui` fully self-contained on hosts without Dream UI.app.
  const workDir = resolveBackendDataDir();
  const staticDir = resolveStaticDir();
  const backendBin = resolveBackendBinary();
  const logDir = process.env.DREAM_LOG_DIR ?? path.join(workDir, 'logs');
  ...
  console.log('');
  console.log('One Work WebUI is ready');
