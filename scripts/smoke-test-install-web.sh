...
export INSTALL_DIR="/tmp/dream-web-smoke-test"
...
if [[ ! -x "${INSTALL_DIR}/dream-web" ]]; then
    echo "❌ CLI executable not found or not executable: ${INSTALL_DIR}/dream-web"
    exit 1
fi
echo "✓ CLI executable exists"

if [[ ! -L "${BIN_DIR}/dream-web" ]]; then
    echo "❌ Symlink not found: ${BIN_DIR}/dream-web"
    exit 1
fi
...
VERSION_OUTPUT=$(dream-web version 2>&1 || echo "")
