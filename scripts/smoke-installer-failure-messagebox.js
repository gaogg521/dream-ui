...
const INSTALLER_ERROR_SCENARIOS = [
  ...
  },
  {
    ...
    action: 'Download a fresh installer and run it again.',
    diagnostics: 'scenario=bundled-dreamcore-incomplete phase=verify-bundled-dreamcore runtime=win32-x64 result=1',
  },
  {
...
function Find-FailureWindow([string]$Code, [int]$TimeoutSec = 90) {
  ...
  do {
    ...
    foreach ($window in $windows) {
      ...
      if ($text -like "*One Work installation failed ($Code)*" -or
...
try {
  ...
  foreach ($required in @(
    "One Work installation failed ($Code)",
    "scenario=$ScenarioId",
    'Suggested action:',
    'Diagnostics:',
    'Installer log:',
...
function createHarnessNsi({ exePath, logPath, projectRoot, scenario }) {
  ...
  return `
Unicode true
Name "One Work Failure MessageBox Smoke"
  ...
  try {
    ...
    if (autoDecline) {
      ...
      if (autoDecline === 'consent') {
        ...
        if (typeof status.copyText !== 'string' || !status.copyText.includes(`One Work installer failure ${code}`)) {
