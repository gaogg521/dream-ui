 ...
 *   DREAM_DATA_DIR=/x bun run resetpass
 ...
 * See the comment there for why the default is `~/.dream-web*` (not `~/.dream*`).
 */
 ...
 * desktop side). Callers can override with `--port` / `DREAM_PORT` to match
 ...
 * pass --port / DREAM_PORT if they launched webui on a non-default one.
 */
