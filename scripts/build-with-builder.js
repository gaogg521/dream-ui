...
function cleanupWindowsPackOutput() {
  ...
  for (const entry of fs.readdirSync(outDir, { withFileTypes: true })) {
    ...
    // Only clean the intermediate win-unpacked directories. NEVER delete
    // packaged installer artifacts (.exe/.msi/.zip) — 1ONE keeps every old
    // installer (version-bumped names don't collide). Deleting them lost a
    // shipped build once; don't repeat it.
...
try {
  ...
  if (builderArgs.includes('--win') || builderArgs.includes('--all')) {
    if (!multiArch) {
      ...
      if (targetArch === 'arm64') {
        ...
        // The zip payload and the differential opt-out used to be forced here
        // for arm64 alone, because the yml asked for LZMA + differential. Both
        // are now the yml's defaults for every arch (2026-09-11), so repeating
        // them per-arch would be the same setting written twice, free to drift.
  ...
  try {
    ...
    console.log('⚠️  Windows local build failed after the app executable was produced.');
    if (firstError) {
