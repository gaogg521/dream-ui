#!/usr/bin/env bash
# ============================================================================
# One Work WebUI — One-Click Installation Script
# ============================================================================
# Usage:
#   curl -fsSL https://raw.githubusercontent.com/gaogg521/dream-ui/main/scripts/install-web.sh | bash
#   # Or specify version:
#   VERSION=1.0.0 bash install-web.sh
#   # Or install to custom directory:
#   INSTALL_DIR=/opt/dream-web bash install-web.sh
# ============================================================================
...
INSTALL_DIR="${INSTALL_DIR:-${HOME}/.local/share/dream-web}"
BIN_DIR="${BIN_DIR:-${HOME}/.local/bin}"
MIRROR="${MIRROR:-https://1onework-1251001122.cos.ap-shanghai.myqcloud.com/releases}"
...
banner() {
    echo -e "${CYAN}${BOLD}"
    echo "  ╔══════════════════════════════════════════════╗"
    echo "  ║    One Work WebUI Installer (No Electron)    ║"
    echo "  ╚══════════════════════════════════════════════╝"
    echo -e "${NC}"
}
...
Options:
  ...
  --install-dir <path>      Specify installation directory (default: ~/.local/share/dream-web)
...
detect_platform_arch() {
    ...
    TARBALL_NAME="dream-web-${VERSION}-${PLATFORM}-${ARCH}.tar.gz"
...
resolve_version() {
    ...
    if [[ "$VERSION" == "latest" || "$VERSION" =~ [a-zA-Z_] ]]; then
        ...
        if command -v curl &>/dev/null; then
            VERSION=$(curl -fsSL "https://api.github.com/repos/gaogg521/dream-ui/releases/latest" \
                | grep '"tag_name"' | head -1 | sed 's/.*"v\([^"]*\)".*/\1/')
        elif command -v wget &>/dev/null; then
            VERSION=$(wget -qO- "https://api.github.com/repos/gaogg521/dream-ui/releases/latest" \
                | grep '"tag_name"' | head -1 | sed 's/.*"v\([^"]*\)".*/\1/')
        else
...
download_tarball() {
    ...
    # Build download URL
    # MIRROR formats:
    #   - HTTP:   https://1onework-1251001122.cos.ap-shanghai.myqcloud.com/releases
...
extract_tarball() {
    ...
    # Extract tarball
    # Tarball root directory is dream-web/, rename after extraction to INSTALL_DIR
    ...
    if [[ -d "${extract_temp}/dream-web" ]]; then
        mv "${extract_temp}/dream-web" "$INSTALL_DIR"
    else
        die "Tarball structure is invalid (missing dream-web/ directory)"
    fi

    success "Extracted to $INSTALL_DIR"

    # Set executable permission on the bun-compiled standalone binary
    chmod +x "${INSTALL_DIR}/dream-web" 2>/dev/null || true
    ...
    # Verify installation
    if [[ ! -x "${INSTALL_DIR}/dream-web" ]]; then
        die "Installation failed: ${INSTALL_DIR}/dream-web not found or not executable"
    fi
...
create_symlink() {
    local symlink_path="${BIN_DIR}/dream-web"
    local target_path="${INSTALL_DIR}/dream-web"
...
update_shell_profile() {
    ...
    # Add to profile
    echo "" >> "$profile_file"
    echo "# Added by dream-web installer" >> "$profile_file"
...
print_summary() {
    ...
    echo -e "${GREEN}${BOLD}  🎉 One Work WebUI v${VERSION} Installed!${NC}"
    echo -e "${GREEN}${BOLD}══════════════════════════════════════════════════${NC}"
    echo ""
    echo -e "  ${BOLD}📍 Installation directory:${NC}  ${INSTALL_DIR}"
    if [[ "$CREATE_SYMLINK" == "1" ]]; then
        echo -e "  ${BOLD}📍 Symlink:${NC}                ${BIN_DIR}/dream-web"
    fi
    echo ""
    echo -e "  ${BOLD}🚀 Usage:${NC}"
    echo ""
    if [[ "$CREATE_SYMLINK" == "1" && ":$PATH:" == *":${BIN_DIR}:"* ]]; then
        echo "    # Start One Work WebUI"
        echo "    dream-web start"
        echo ""
        echo "    # Check version"
        echo "    dream-web version"
    else
        echo "    # Start One Work WebUI (using full path)"
        echo "    ${INSTALL_DIR}/dream-web start"
        echo ""
        echo "    # Or add symlink to PATH:"
        if [[ "$CREATE_SYMLINK" == "1" ]]; then
            echo "    export PATH=\"${BIN_DIR}:\$PATH\""
        else
            echo "    ln -s ${INSTALL_DIR}/dream-web ~/.local/bin/dream-web"
            echo "    export PATH=\"~/.local/bin:\$PATH\""
        fi
    fi
    echo ""
    echo -e "  ${BOLD}📖 Documentation:${NC}  https://github.com/gaogg521/dream-ui"
    echo -e "  ${BOLD}🐛 Report issues:${NC}  https://github.com/gaogg521/dream-ui/issues"
    echo ""
