...
function Test-ManagedCliContract {
  ...
  foreach ($field in @('requiredFiles', 'requiredDirectories')) {
    if ($null -ne $Cli.$field -and -not (Test-StringArrayField $Cli $field)) {
  ...
  foreach ($pathValue in @($Cli.root, $Cli.executable) + @($Cli.requiredFiles) + @($Cli.requiredDirectories)) {
  ...
  foreach ($requiredFile in @($Cli.requiredFiles)) {
    if ($null -eq $requiredFile) { continue }
  ...
  foreach ($requiredDirectory in @($Cli.requiredDirectories)) {
    if ($null -eq $requiredDirectory) { continue }
...
function Test-ManagedResourcesContract {
  ...
  $schemaVersion = [double]$contract.schemaVersion
  # Both schema versions must verify: a machine may still hold an older install
  # produced before the switch.
  #
  # acpTools is the ACP wrapper layer sessions are actually spawned through, and
  # it is required in BOTH versions. v2 only adds prepared native CLIs (clis)
  # alongside it — it does not replace it. Reading v2 as "clis instead of
  # acpTools" is what let 2.1.51 ship with no acp/ subtree, leaving every fresh
  # install unable to start Claude or Codex.
