...
**装新版不再多出一份旧程序，早期版本留下的数据会被接管而不是被绕开。**
企业侧：3.0.3 那句「公司安全策略仍然不会生效」从本版起不再成立。

### 单机版

【安装与数据】

· 装过上一版的机器上，安装新版会把旧的那份删掉，而不是在旁边再装一份。上一版的程序目录叫 `One Work`（带空格），本版叫 `onework`，Windows 把它们当成两个不相干的程序：旧的既不在「添加或删除程序」里、也不会被新版覆盖，于是两份同时躺在磁盘上，用户看到两个一模一样的图标、点开的却是不同版本。真机验证：清掉旧目录后释放 2.24 GB，新版本身完好，同机的 `1onecode` 未被波及
· 早期版本留下的数据文件会被改名接管。数据库统一叫 `one-backend.db`，会话与进程记录同理。此前的做法是「新名字不存在就继续用旧名字」，能用，但每装一次新版就多一条要永远维护的兼容分支，且新旧两套名字可以同时存在。现在启动时直接改名，连 SQLite 的 `-wal`/`-shm` 一起搬；改到一半失败会回滚，两份都在时不猜、保持原样并记日志。迁移排在实例锁之前，锁本身也就落在新名字上

【真实缺陷修复】

· 本地部署的 Ollama 模型现在能保存了。保存时会要求填 API Key，而本机的 Ollama 根本没有 Key，于是这类模型一律保存失败
· Bedrock 拉取模型列表不再要求填 Base URL——它按区域寻址，本就没有这个字段
· Vertex AI 的渠道现在能保存、也能拉到模型列表。真实数据里这个平台名叫 `gemini-vertex-ai`，而判断逻辑只认 `vertex-ai`，两处对不上；同时它也不需要 Endpoint
· Gemini 渠道的「刷新模型」按钮点了没反应的问题已修
· 后端进程意外退出时会告诉用户，而不是只写进日志文件——此前界面只是一直转圈
· 对话过程中的上下文与 token 用量现在是实时走的。此前只在整轮结束时更新一次，长任务跑到一半看不出还剩多少余量
· 生成 PPT 时形状尺寸不再离谱地错位。几何数值不带单位时按 EMU 解析（1 厘米 = 360000），写 `height=3` 得到的是三个 EMU 而不是三厘米，整页排版随之崩掉；officecli 自己的帮助示例就是这么写的
· officecli 的自检循环存在没有出口的分支，会一直转下去
· 「最新优先」的列表在同一毫秒内创建的多条记录上顺序是随机的，现已按 id 补上稳定的次序

【界面】

· 「+」菜单里的技能改成可搜索的图标网格，选中的技能以标签形式回显；技能中心与之统一，展示技能自己的名称和图标
· 用户自行导入的技能会进入常驻集合，和内置技能同等对待
· 安装体积继续收窄：不再打包运行时从不会去启动的两个原生 CLI

### 企业版

· **被管理员封禁的节点现在真的停止同步团队资源，且客户端不发机器标识也一样被拒。**此前只要客户端不带 `x-dream-machine-id` 这个头，就会被当成无法判定而放行——封禁只在客户端愿意表明身份时才有效，等于没有。浏览器后台不受机器判定影响，管理员自己那台电脑被封禁时仍打得开解封它的页面
· 退出企业时，加入时下发的九类内容会被逐一收回，此前只收回了其中一部分
· 关闭企业模式后的资源清理改为按证据判断（后端里是否真有企业渠道），不再依赖本地记的一个标记——标记丢了就再也不清理了
· 渠道的模型白名单在服务端强制；不再拿客户端传来的 id 当服务端的键使用
· 模型代理的用量会记到发起它的那次会话上

### 后端

本版把 `dreamcore` 提到 `v0.1.71-one.4`（3.0.2 / 3.0.3 都停在 `v0.1.71-one.3`）。

✅ **3.0.3 里那条「公司安全策略不会生效」的警告，到本版为止不再成立。** 破坏性命令拦截、外网默认
拒绝、终端命令审批、发送限流、模型白名单，以及企业记忆的本机召回，执行点都在 dream-core 里，
这批改动已经包含在 `v0.1.71-one.4`。客户端 POST 到 `/api/tool-security/policy` 的策略不再落到
404 上被 `syncToolSecurityPolicy` 的 catch 吞掉，而是被真正执行。

C1-2（封禁节点 → 不再同步）本轮在真实企业二进制上跑通了完整闭环：封禁前放行 → 封禁后带机器
标识被拒（`MACHINE_BLOCKED`）→ 不带标识同样被拒 → 同一用户的浏览器会话仍然可用。
