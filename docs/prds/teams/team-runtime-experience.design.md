...
配套 PRD：`team-runtime-experience.md`。本文面向实现，给出现状调研结论、架构决策、模块设计、关键代码骨架与分阶段落地。全部为渲染层改动，不改 dreamcore 与团队数据结构；持久化走 `localStorage`。
...
```ts
// 低饱和 slate 邻近色，取自 dream-ui 品牌基调。每色给 accent(主) + soft(浅底) 两档，
