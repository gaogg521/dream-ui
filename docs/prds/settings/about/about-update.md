...
1. 用户打开「设置 → 关于」页面
2. 页面顶部居中显示应用名 "dream-ui"（h3 标题）
3. 下方显示应用描述（通过 i18n 系统，随语言设置变化）
4. 显示当前版本号 badge（格式 `v{x.y.z}`），版本号来自打包时的 `package.json`
5. 版本号旁有 GitHub 图标，点击在系统浏览器中打开项目仓库 `https://github.com/gaogg521/dream-ui`
...
| 序号 | 标题     | 行为               | 目标                                            |
| ---- | -------- | ------------------ | ----------------------------------------------- |
| 1    | 帮助文档 | 打开外部链接       | `https://github.com/gaogg521/dream-ui/wiki`     |
| 2    | 更新日志 | 打开外部链接       | `https://github.com/gaogg521/dream-ui/releases` |
| 3    | 意见反馈 | 打开外部链接       | `https://github.com/gaogg521/dream-ui/issues`   |
| 4    | 问题报告 | **打开应用内弹窗** | FeedbackReportModal（见 F-ABOUT-11）            |
| 5    | 联系我   | 打开外部链接       | `https://x.com/WailiVery`                       |
| 6    | 官网     | 打开外部链接       | `https://www.dream-ui.com`                      |
...
│                                                          │
│  AboutModalContent                                       │
│    ├─ CustomEvent('dream-ui-open-update-modal')            │
...
- Both x64 and arm64 installers run the shared `DREAM_VERIFY_CORE_APP_FILES` macro before bundled dream-core verification.
- Missing `dream-ui.exe`, core DLLs, or `resources\app.asar` fails through `DREAM_FAIL_UX` with code `E1031` and logs the missing label and path.
- Bundled dream-core verification remains `E1030`.
- If a silent `--updated` install cannot close dream-ui after retries, the installer writes `%APPDATA%\dream-ui\installer-last-failure.json`.
...
```json
{
  ...
  "updated": true,
  "retryCount": 3,
  "instDir": "C:\\Program Files\\dream-ui",
  "logPath": "C:\\Users\\user\\AppData\\Local\\Temp\\dream-ui-installer-process-check.log",
...
- Run normal x64 and arm64 installs. Expected: shared core verification runs before bundled dream-core verification and installation succeeds.
- Run silent `/S --updated` while dream-ui cannot be closed. Expected: marker file appears under `%APPDATA%\dream-ui`; next app launch shows the update failure notification; the following launch does not show it again.
