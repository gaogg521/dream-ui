...
dream-ui owns only feedback orchestration:
...
dream-ui main process must not read SQLite or expose `feedback:collect-db-diagnostics`.

## Profile Resolution

Route context is more trustworthy than the selected module because the user can choose the wrong module. The selected module is still useful user intent. dream-core must use the union of:
...
The dream-core response includes a `privacy` block:
