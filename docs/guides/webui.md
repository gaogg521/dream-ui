# dream-ui WebUI Mode - Startup Guide

dream-ui supports WebUI mode, allowing you to access the application through a web browser. This guide covers how to start WebUI mode on all supported platforms.
...
WebUI mode starts dream-ui with an embedded web server, allowing you to:

- Access the application through any modern web browser
- Use dream-ui from remote devices on the same network (with `--remote` flag)
...
```cmd
# Using full path
"C:\Program Files\dream-ui\dream-ui.exe" --webui

# Or if dream-ui is in your PATH
dream-ui.exe --webui
```
...
```bash
# Using full path
/Applications/dream-ui.app/Contents/MacOS/dream-ui --webui

# Or using open command
open -a dream-ui --args --webui
```

### Method 2: Create Shell Script

Create `start-dream-webui.sh`:
...
```bash
chmod +x start-dream-webui.sh
./start-dream-webui.sh
```
...
chmod +x dream-ui-*.AppImage

# Run with --webui flag
./dream-ui-*.AppImage --webui
```

### Method 2: Create Desktop Entry

Create `~/.local/share/applications/dream-webui.desktop`:

```ini
[Desktop Entry]
Name=dream-ui WebUI
Comment=Start dream-ui in WebUI mode
Exec=/opt/dream-ui/dream-ui --webui
Icon=dream-ui
Terminal=false
Type=Application
...
```bash
chmod +x ~/.local/share/applications/dream-webui.desktop
```
...
Create `~/bin/start-dream-webui.sh`:
...
```bash
chmod +x ~/bin/start-dream-webui.sh
```
...
Create `/etc/systemd/system/dream-webui.service`:

```ini
[Unit]
Description=dream-ui WebUI Service
After=network.target

[Service]
Type=simple
User=YOUR_USERNAME
ExecStart=/opt/dream-ui/dream-ui --webui --remote
Restart=on-failure
RestartSec=10
...
sudo systemctl enable dream-webui.service
sudo systemctl start dream-webui.service

# Check status
sudo systemctl status dream-webui.service
```
...
**Important Note**: Electron desktop mode is **not supported** on Android. However, you can run dream-ui in WebUI mode using Termux with a prooted Linux environment.

> **Community Contribution**: This guide is contributed by [@Manamama](https://github.com/Manamama). Special thanks for making dream-ui accessible on Android devices! 🙏
>
> **Original Tutorial**: [Running dream-ui WebUI on Android via Termux + Proot Ubuntu](https://gist.github.com/Manamama/b4f903c279b5e73bdad4c2c0a58d5ddd)
>
> **Related Issues**: [#217 - Android Support Discussion](https://github.com/gaogg521/dream-ui/issues/217)
...
#### 4. Download and Install dream-ui

```bash
# Download the ARM64 .deb package (replace VERSION with the actual version)
# Check latest version at: https://github.com/gaogg521/dream-ui/releases
wget https://github.com/gaogg521/dream-ui/releases/download/vVERSION/dream-ui_VERSION_arm64.deb

# Example (replace VERSION with the release tag, e.g. v1.5.2):
wget https://github.com/gaogg521/dream-ui/releases/download/vVERSION/dream-ui_VERSION_arm64.deb

# Install the package
apt install -y ./dream-ui_*.deb

# Verify installation
which dream-ui
```

#### 5. Launch dream-ui WebUI

```bash
# Start dream-ui in WebUI mode with no-sandbox flag
dream-ui --no-sandbox --webui
```
...
To access dream-ui from other devices on your local network:

```bash
# Start with --remote flag
dream-ui --no-sandbox --webui --remote
...
dream-ui --no-sandbox --webui --port 8080
```
...
chmod +x /opt/dream-ui/dream-ui
```

#### Out of Memory

dream-ui requires sufficient RAM. Close other apps if you encounter memory issues.

#### Cannot Access from Browser

1. Check if dream-ui is running: look for "Server started" message
...
4. **Keep device charged** - Running dream-ui consumes battery

### Tested Environment

- **Device**: Android 14
- **Termux Version**: 0.118.0
- **dream-ui Version**: Latest release (e.g. 1.5.2)
- **Proot-distro**: Ubuntu (latest)

### Creating a Startup Script

For convenience, create a script to launch dream-ui quickly:

```bash
# Create script in Ubuntu (proot)
cat > ~/start-dream-ui.sh << 'EOF'
#!/bin/bash
...
```bash
proot-distro login ubuntu -- bash -c "dream-ui --no-sandbox --webui --remote"
```
...
2. Report issues at [GitHub Issues #217](https://github.com/gaogg521/dream-ui/issues/217)
...
```cmd
dream-ui.exe --webui --remote
```

### macOS

```bash
/Applications/dream-ui.app/Contents/MacOS/dream-ui --webui --remote
```

### Linux

```bash
dream-ui --webui --remote
```
...
netsh advfirewall firewall add rule name="dream-ui WebUI" dir=in action=allow protocol=TCP localport=3000
```
...
**macOS:**
Go to **System Preferences** → **Security & Privacy** → **Firewall** → **Firewall Options** → Add dream-ui
...
which dream-ui
# or
find /opt -name "dream-ui" 2>/dev/null
```
...
```powershell
& "C:\Program Files\dream-ui\dream-ui.exe" --webui 2>&1 | Tee-Object -FilePath dream-ui.log
```

**macOS/Linux:**

```bash
/path/to/dream-ui --webui 2>&1 | tee dream-ui.log
```
...
export DREAM_PORT=8080

# Allow remote access without passing --remote
export DREAM_ALLOW_REMOTE=true

# Optional host hint (0.0.0.0 behaves the same as DREAM_ALLOW_REMOTE=true)
export DREAM_HOST=0.0.0.0

# Then start the application
dream-ui --webui

# You can also pass the port directly via CLI
dream-ui --webui --port 8080
```
...
| Platform | Location                                                   |
| -------- | ---------------------------------------------------------- |
| Windows  | `%APPDATA%/dream-ui/webui.config.json`                     |
| macOS    | `~/Library/Application Support/dream-ui/webui.config.json` |
| Linux    | `~/.config/dream-ui/webui.config.json`                     |
...
```cmd
# Using full path
"C:\Program Files\dream-ui\dream-ui.exe" --resetpass

# Or for a specific user
"C:\Program Files\dream-ui\dream-ui.exe" --resetpass username
```
...
```bash
# Using full path
/Applications/dream-ui.app/Contents/MacOS/dream-ui --resetpass

# Or for a specific user
/Applications/dream-ui.app/Contents/MacOS/dream-ui --resetpass username
```
...
# Or for a specific user
dream-ui --resetpass username

# Or using full path
/opt/dream-ui/dream-ui --resetpass
```
...
2. Search [existing issues](https://github.com/gaogg521/dream-ui/issues)
3. Create a [new issue](https://github.com/gaogg521/dream-ui/issues/new) with:
   - Your OS and version
   - dream-ui version
   - Steps to reproduce
...
**Happy using dream-ui in WebUI mode!** 🚀
