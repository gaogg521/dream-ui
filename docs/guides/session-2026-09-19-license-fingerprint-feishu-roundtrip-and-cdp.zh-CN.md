...
## 六b、企业能力 CDP 矩阵（2026-09-19 补充轮）

在 data-en-cdp2 真机上对管理台做了一轮能力矩阵扫描（CDP 驱动，全部 PASS）：

| 能力                       | 结果                                                                                                                 |
| -------------------------- | -------------------------------------------------------------------------------------------------------------------- |
| 成员列表                   | SSO JIT 用户（赵高/sso_01a0b7b7）与 admin 在册                                                                       |
| 组织架构（写）             | CDP 创建部门「CDP调度测试部」成功，toast + 审计留痕                                                                  |
| 资源授权矩阵               | 树含新部门/未分配桶/成员叶节点；自动选中详情；空注册表走 Empty 兜底（`ResourceGrantEditor` line 95，行为正确非 bug） |
| 邀请码（写）               | 生成 `0BE3-8E50-4401-210A`（7 天有效），复制/作废/邮件操作齐                                                         |
| 审计日志                   | `org.department.create` 等 2 条在案，领域/结果/导出可用                                                              |
| 用量总览 / 智能体会话      | 正常渲染；无模型会话数据（符合现状）                                                                                 |
| **目录同步**               | **真实拉通飞书：230 部门 / 752 人通讯录**（真实姓名/部门/职位入册）；同步人员不占席位（席位仍 2/50，一致性 ✓）       |
| **运行时节点（调度底座）** | 桌面端作为 agent 节点自动注册：机器 ID/主机名 LT00278-ZG/心跳在报；接入审批开关可配                                  |
| 项目组                     | 默认组织 2 成员，邀请码入口                                                                                          |

企业调度链路闭环：**节点接入 → 成员入企（SSO/邀请码）→ 组织归属（部门/目录）→
资源授权（矩阵）→ 审计留痕**，全部真机可复现。技能注册表为空属新部署常态
（导入走 `/api/skills/import`，本轮未做）。
