...
**日期**：2026-07-22-23
**状态**：三仓源码全部改完，编译/测试/lint 通过；**未重编 dreamcore、未打包、未提交**。
...
- **dream-core-app**：加 `CompanyAdminResolverAdapter`(→one-org)/`CompanyAdminCheckAdapter`(→one-sso)，`one_enterprise_service` 前移构造后挂到 `one_org_state`/`one_sso_state`。
...
- 后端：`one-enterprise`(12)/`one-org`(23)/`one-sso`(45) 单测全过；`cargo check -p dream-core-app` 通过；clippy 我的代码零告警（仅剩 service.rs:873 一个**既有** `too_many_arguments`，ratchet 不动）。`cargo test --workspace` 兜底跑过。
...
- **真机 CDP 全流程实测通过**（dev + backend-rebuild 后的新 dreamcore，`/api/one/enterprise/company` 404→200）：
...
`backend-rebuild.ps1` 第2步 `prepareDreamcore.js` 内嵌报 exit 1（疑版本校验/文件锁）；cargo build 本身成功（`target/release/dreamcore.exe` 已含新代码，grep 二进制有 `enterprise/company`/`COMPANY_ALREADY_EXISTS`），手动 `cp` 到 `1oneUI/resources/bundled-dreamcore/win32-x64/dreamcore.exe` 即可。另注意：起 dev 前要**杀干净旧 dev 的 electron/dreamcore 进程**，否则新 dev 撞单实例锁退出、留旧后端(旧 exe→新路由 404)。

## 顺带修：Windows 尾随空格 workspace 校验 bug（07-18 上游带入的既有 bug）

`cargo test --workspace` 唯一失败 `dream-core-conversation::create_rejects_unavailable_workspace_with_trailing_whitespace_in_request` = memory 记录的既有 Windows 平台 bug。根因：`dream-core-common::validate_workspace_path_availability`（`error.rs`）用 `fs::metadata` 判可用，但 Win32 在**创建和读取**两端都静默剥掉路径分量末尾的空格/制表/点号，`C:\…\workspace ` 会解析成 `C:\…\workspace`（存在）→ 误判「可用」，而 Unix 上该路径根本不存在→拒绝。
**修法**：加 `#[cfg(windows)]` 前置检查——最终路径分量以空格/制表/点结尾（且不是 `.`/`..`）就返回 `DoesNotExist`（与 Unix 一致，因为 Windows 无法忠实存储这种路径）。并把 `create_accepts_existing_workspace_with_trailing_whitespace_in_name`（在 Windows 上**不可能**创建这种目录）门控为 `#[cfg(not(windows))]`。验证：dream-core-conversation 314 全过、dream-core-common 81 全过、clippy 干净。
