...
1. 会话窗口 / 助手列表里 **「Dream CLI」** 显示为 **「1ONE CLI」**（仅品牌名，不改 `dream-engine` 类型与路由）。
2. **助手页与首页**只显示本机**已安装且可用**的 CLI 助手；未安装的不默认列出（如仅装了 Claude、Cursor、OpenClaw）。
3. 用户确认：**改名与过滤不影响已有会话**（底层仍是 `dream-engine` / 各 `acp_backend`）。
...
| 层                | 文件                                                                                             | 说明                                                 |
| ----------------- | ------------------------------------------------------------------------------------------------ | ---------------------------------------------------- |
| 前端统一          | `renderer/utils/model/assistantDisplay.ts` → `resolveAssistantName()`                            | `dream-engine` 或旧名 `Dream CLI` → `1ONE CLI`       |
...
| 编辑器 Agent 下拉 | `assistantUtils.ts` → `buildAssistantEditorBackends`                                             | `agent_type === 'dream-engine'` → `1ONE CLI`         |
| 后端显示名        | `1oneCore/.../enums.rs`                                                                          | `AgentType::DreamEngine.display_name()` → `1ONE CLI` |
...
| 文件                                           | 逻辑                                                                                                                               |
| ---------------------------------------------- | ---------------------------------------------------------------------------------------------------------------------------------- |
| `renderer/utils/model/assistantSelection.ts`   | 新增 `isInstalledGeneratedCliAssistant()`：`source === 'generated'` 时仅 `agent_status === 'online'` 或 **内置 dream-engine** 显示 |
...
2. `dream-core-runtime/resolver.rs`：`resolve_command_path("agent")` 在 PATH 未命中时，回退探测 **`%LOCALAPPDATA%\cursor-agent\agent.exe`**（及 `.cmd` 等 shim）。
...
- 资源：`crates/dream-core-assets/assets/logos/brand/1one.png`（与桌面 `resources/app.png` 同款吉祥物）
- 迁移 `021_rebrand_dream-engine_icon_to_1one.sql`：更新 `agent_metadata.icon` 与 `assistant_definitions.avatar_value`
...
| 改动类型                                                              | 命令                                                                                 |
| --------------------------------------------------------------------- | ------------------------------------------------------------------------------------ |
| **仅前端**（`assistantDisplay.ts`、`assistantSelection.ts`、助手 UI） | `D:\旧中转目录\scripts\frontend-dev.ps1`，刷新窗口                                   |
...
crates/dream-core-runtime/src/resolver.rs                           # cursor-agent install path for `agent`
crates/dream-core-db/migrations/020_fix_cursor_agent_cli_command.sql
crates/dream-core-db/migrations/021_rebrand_dream-engine_icon_to_1one.sql
crates/dream-core-ai-agent/src/registry.rs   # derive_management_status / refresh_availability
```

**发行包（Windows x64）**：`D:\旧中转目录\dream-ui\out\1ONE Code-2.1.32-win-x64.exe`
...
- `D:\旧中转目录\scripts\README.md`

---

## 7. 后续修订（2026-07-08 下午，装了就显示 + 登录渠道 + Cursor 澄清）

> 承接第 2 节。用户实测「明明装了 Cursor 却不显示」，据此把可见性判据从**「只显示 online」订正为「装了就显示」**，并附带修复企业登录渠道。**仅改源码，未重打包**（等实测后再出包）。提交：dream-ui `63957f3`、dream-core `8403b02`。
...
- **后端**（`crates/dream-core-assistant/src/service.rs` `reconcile_generated_assistants`）：生成型助手行的过滤从 `Online|Unchecked` 放宽到 **含 `Offline`**，否则「启动探测先把安装但离线的 agent 置 Offline → 不生成助手行」，前端无从显示。补单测 `bootstrap_materializes_generated_assistant_from_offline_agent`。
- **预期**：重编后点「一键扫描全部」→ Cursor 以「离线/需登录」出现；在 Cursor Agent CLI 登录后再扫描 → 变 `online` 才能真正对话。

### 7.2 1ONE CLI 猫图标：源码已就位，重编即生效

- 猫 logo = `app.png`（前端）/ `1one.png`（后端 `dream-core-assets` crate，同一张 236431B 粉猫图）。迁移 `021` 已把 dream-engine `agent_metadata.icon` → `/api/assets/logos/brand/1one.png`，`reconcile_generated_assistants` 每次启动从 icon 同步到生成助手 `avatar_value`，avatar 投影对 `/api/` 路径正常输出图片（`is_local_avatar_value` 对 `/api/` 返回 false）。
...
- **修**：改用 `getBaseUrl()`（`@/common/adapter/httpBridge`；WebUI→''同源、桌面→`127.0.0.1:port`、企业远端→远端 URL）拼绝对地址。渠道跳转机制（`enterpriseBrowserLogin.ts` 的 `ensureWebuiRunning` + `openExternalUrl`）为既有设计未动；若修后跳转本身仍失败，再考虑单机走本地后端 origin + `desktop=1` 深链（dream-core `/api/one/sso/{provider}/authorize?desktop=1` → `dream-ui://sso-callback?token=`，已确认支持）。

### 7.4 验证 / 待办

- 前端 `tsc --noEmit` 无错、`oxlint` 干净、`assistantSelection` 单测 8/8。
- 后端 crate 编译过，`reconcile` 8/8 + `bootstrap_materializes` 5/5（含新 offline 用例）。
- **待办**：重编（dream-core release + dream-ui）+ `dist:win` 出新包（bump 版本、不删旧 .exe）→ 用户实测 ①②③。
