...
`resolve_vision_delegate`（`1oneCore/crates/dream-core-ai-agent/src/factory/dream-engine.rs:341`）
从用户已配置的 provider 里挑视觉模型，**不查 allowlist**。

全仓 allowlist 只有两个强制点，都覆盖不到它：

- `crates/dream-core-conversation/src/routes.rs:323` 的 `check_send`（发消息时，查会话模型）
- `crates/dream-core-conversation/src/routes_aux.rs:44` 的 `check_model`（用户改模型设置时）
...
（`旧引擎本地检出/crates/dream-engine-types/src/llm.rs:52`），但
`旧引擎本地检出/crates/dream-engine-tools/src/read_image.rs` 里写的是
`LlmEvent::Done { .. } => break` —— 用量直接丢掉。

而 1oneCore 只记主模型那一次（`crates/dream-core-conversation/src/turn_orchestrator.rs:369`
...
修 A 需要让 `dream-core-ai-agent` 够到 `one-billing` 的判定。两者**同层**，按本仓既有
安排只能走 trait 注入（照 `EnterpriseSync` / `CredentialRevoker` 的做法）。
修 B 需要把用量从 `dream-engine-tools` 一路穿回 1oneCore 的 `record_turn`，跨三仓。
...
- 后端解析锚点：`crates/dream-core-conversation/src/service.rs:2686` 起，取「分叉点或之前
  最近一条非 NULL 的 `backend_turn_id`」
- 引擎截断：`旧引擎本地检出/crates/dream-engine-agent/src/session.rs` 的 `fork_from`，
...
避免了，at_turn 后端只在 turn 粒度上避免。而 dream-engine 是默认后端且迁移 049 给它
...
| 问题                                                                                                                                                   | 位置                                                     |
| ------------------------------------------------------------------------------------------------------------------------------------------------------ | -------------------------------------------------------- |
| 团队会话那条错误路径没有 `FORK_*` 前缀，且 `ApiError::Forbidden` 把 reason 抹成裸 "Forbidden."（目前 UI 不显示按钮所以只有直接打 API 才撞上）          | `service.rs:2605` + `dream-core-common/src/error.rs:185` |
...
1. 1oneCore 内嵌的是 `dream-engine-agent` 库,而 `OutputSink::emit_stream_end`——唯一携带
   model + 真实 token 数的那个回调——**只有 `dream-engine-cli` 调**
   (`dream-engine-cli/src/run.rs`、`dream-engine-cli/src/json_stream/message.rs`),
   `dream-engine-agent` 全仓零调用。所以 1oneCore 里那个写得很完整、还配了
...
2. 内嵌路径实际发的是 `runtime.emit_finish(None)`
   (`manager/dream-engine/agent.rs:464`,ACP 侧同理见 `manager/acp/agent.rs:1509/1519/1531`),
...
**范围是所有聊天**,不只 dream-engine。核过 ACP 路径唯一填 `input_tokens` 的生产代码是
...
- **dream-engine 侧**:让引擎在 run 结束时调 `emit_stream_end`(它手里有 usage),
  1oneCore 那侧现成的实现即刻生效。跨仓,要走「先推 dream-engine → bump lock → 重编内嵌」。
...
- 1oneCore `cargo test --workspace` 有 **8 条既有失败**：`dream-core-project` 的
  scm/discard 6 条 + `scm_request_path` 1 条 + `team_e2e` 的 Windows 尾随空格 1 条
  （最后那条是 07-18 上游同步带入的已知缺陷，一直没修）
- dream-engine 有 **6 条 timeout 测试在重负载下 flaky**（单独跑全绿）。根因是 Windows
...
```bash
cd D:\旧中转目录\1oneCore && cargo run -p license_tool -- keygen --out D:\keys\license-2026-08.json
```
...
1. **当前安装包不含最后四个修复**，要发就得重编内嵌 + 重新打包。
   ...
   现有安装包里，且它同时改了 1oneCore 与 dream-engine——发版前必须先重编内嵌后端。
2. **只出了 Windows 包。** 要出 Mac 包必须**先给 1oneCore 打新 tag 发 Release**
   ——Mac CI 按 `package.json` 的 `dreamcoreVersion` 从 Release 下载后端，不发新 tag
...
| 仓           | 提交范围               | 内容                                                                                                                            |
| ------------ | ---------------------- | ------------------------------------------------------------------------------------------------------------------------------- |
| dream-engine | `e4d1638..1d485e5`     | `ReadImage` 工具（纯文本模型读图）+ `image_source` 共用模块                                                                     |
...
> （`1oneUI` 前端 Electron / `1oneCore` Rust 后端 / `旧引擎本地检出` agent 引擎），
> 只单向同步上游、永不反向提 PR。
>
...
## 背景
2026-08-18 那轮给纯文本模型加了读图能力：模型调 dream-engine 的 ReadImage 工具，
...
A) 可以使用企业 allowlist 禁止的模型
   ...
   全仓 allowlist 只有两个强制点，都在「会话模型」那一侧、覆盖不到委托：
     - crates/dream-core-conversation/src/routes.rs:323 的 check_send
     - crates/dream-core-conversation/src/routes_aux.rs:44 的 check_model
...
B) 这次调用的 token 用量被丢弃
   ...
   `LlmEvent::Done { .. } => break` —— 用量直接丢掉。
   而 1oneCore 只记主模型那一次
   （crates/dream-core-conversation/src/turn_orchestrator.rs:369 的 record_turn）。

后果：企业成本上限与用量看板看不见这笔花费。公司设了上限，仍会被计费。

## 为什么不是随手能改
修 A 要让 dream-core-ai-agent 够到 one-billing 的判定，而两者**同层**——按本仓既有
...
修 B 要把用量从 dream-engine-tools 一路穿回 1oneCore 的 record_turn，跨三仓。
⚠️ 推送顺序不可颠倒：先推 dream-engine → 删掉 1oneCore/Cargo.toml 末尾可能存在的临时
...
## ⛔ 绝对不要做
不要放宽 1oneCore/crates/dream-core-ai-agent/src/capability/image_input.rs 的能力判定
...
```
工作目录 D:\旧中转目录，三仓 fork：1oneUI / 1oneCore / 旧引擎本地检出。
...
- 后端解析锚点：1oneCore/crates/dream-core-conversation/src/service.rs:2686 起，
...
turn 粒度上避免。而 dream-engine 是默认后端、迁移 049 给它硬编码了 at_turn: true，
所以这是主路径而非边角。
...
2. 让可见历史的复制也按 turn 对齐（改 copy_messages_up_to 的游标语义，
   风险是影响既有的复制行为）
3. 给引擎加消息级截断能力（改动最大，跨到 dream-engine）

## ⚠️ 必守
- 不要为了对齐语义去动 route_for_backend 的 claude/codex → AcpManager 这条
  fork 分歧点（1oneCore/crates/dream-core-ai-agent/src/factory/acp.rs 有测试锁死，
...
## 3. is_head 把 hidden 行算进「最新」，前端不算 → 点了报 422 的按钮
 - 后端 service.rs:2670 的 is_head 用 list_messages_page(InitialLatest, limit 1)，
   该查询（dream-core-db/src/repository/sqlite_conversation.rs:688）只排除
...
## 4. 四条小问题
 - 团队会话那条错误路径没有 FORK_* 前缀，且 ApiError::Forbidden 把 reason 抹成
   裸 "Forbidden."（service.rs:2605 + dream-core-common/src/error.rs:185）。
...
- 品牌复检：新增文案不得出现「dream-ui」，显示名是 One Work（BRAND_DISPLAY_NAME）。
...
## 必须知道的三件事
0) ⚠️ 2026-08-18 追加：视觉委托治理修复（1oneCore + dream-engine，交接文档 §1）也是打包
...
1) 现有的 out/One-Work-2.1.55-win-x64.exe **不含最后四个修复**——它们是打完包
   ...
   预览保存静默失败。
2) 安全修复在 1oneCore，所以**必须先跑 D:\旧中转目录\scripts\backend-rebuild.ps1
...
3) **要出 Mac 包必须先给 1oneCore 打新 tag 发 Release**——Mac CI 按 package.json
   的 dreamcoreVersion 从 Release 下载后端，不发新 tag 会拿到**不含这些改动的
...
- Windows 打包用 scripts/package-win.ps1（它会设 DREAM_BACKEND_LOCAL_PATH
  指向本地刚编的 dreamcore，不走 dreamcoreVersion 下载）
- ⚠️ 不许删任何旧 .exe 安装包
...
`process.env.DREAM_DEVTOOLS_CDP_PORT && app.isPackaged`，所以 dev 下设成
...
- 1oneCore cargo test --workspace 有 8 条既有失败：dream-core-project 的 scm/discard
...
2) 给托管 node 目录加杀软排除：本机启动一个 node 要 1~1.7 秒
   ...
   也是 dream-engine 那 6 条 flaky 测试的根因。需**管理员** PowerShell：
       Add-MpPreference -ExclusionPath "$env:APPDATA\1one-Dev\1one\runtime\node"
```
...
8. **每次跑 shell 命令前确认工作目录。** 真实发生过跨仓库误提交（cwd 停在
   旧引擎本地检出，却在提交 1oneUI 的文档改动），靠分支名不同才暴露。
...
10. **真机验证用 CDP**：`DREAM_DEVTOOLS_CDP_PORT=9230 bun run dev`，然后连
