...
> **✅ 2026-09-19 复核过一遍**（zcode 那轮之后，逐条对着代码量，不是照着文档抄）。
> 复核结论、量法和证据见 §4.0。**这张表以复核结果为准，不以任何一份 session 文档的自述为准。**

### 4.0 2026-09-19 复核：两条销账、六条仍开
...
§7 写「只剩两条」（P0 大项 + 宝云 Phase 4）。**那是对它自己 §7 范围说的**，
成立；但它收的是「自己的 §7 + 本文 SSO/企业侧的行」，本文 **B 表、E 条、
A 表第 2/3/4 行不在它范围内** —— 其中一条还在第三个仓库 dream-engine 里。

**销账 2 条（我登记错了，复核时发现早已修掉）：**

| 原条目                                                  | 实测                                                                                                                                                                 |
| ------------------------------------------------------- | -------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| ~~队长信箱重复「已暂停」通知~~ ✅ 已修（见 §4.0）未合并 | ✅ **已实现**。`dream-core-team/src/session.rs:2139` 的 `notify_leader_delivery_exhausted` 有 `already_pending` 检查 —— 同一 slot 的未读通知还在时直接跳过写入和唤醒 |
| `engine.rs` 截断工具调用仍发英文串                      | ✅ **已走 code**。`dream-engine-agent/src/engine.rs` 现在是 `emit_info_coded("TRUNCATED_TOOL_CALL_RETRY", …, &fallback)`                                             |

**复核确认 zcode 做到了的**（不是只看它自述）：
`dream-en/admin-web/tests/deep-link-scheme-cross-repo.test.ts` 文件真实存在 ——
这就是 §5.3 说的「没有任何东西锁住两边一致」的那把锁，已经补上了。

**仍然开着的 6 条**，下面三条是**刚量出来的**，不是照抄文档：

| 条目                            | 怎么量的                                                            | 结果                                                                                                  |
| ------------------------------- | ------------------------------------------------------------------- | ----------------------------------------------------------------------------------------------------- |
| Explorer 另一半（`fs/remount`） | `grep -rn 'fs/remount\|"remount"' dream-core/crates --include=*.rs` | **0 命中**，仍不存在                                                                                  |
| `sendbox.css` 其他重复规则块    | 抽出所有 `^\s*\.xxx {` 选择器做 `uniq -c`                           | 仍有 **2 处重复**：`.sendbox-highlight-textarea`、`.sendbox-actions .sendbox-model-btn:focus-visible` |
| `DEFAULT_CHAR_BUDGET` 过期注释  | `grep -rn '2% of 200k' dream-engine`                                | 仍在 `dream-engine-skills/src/prompt.rs:15`                                                           |

另外 3 条是**验证欠账**，性质上没法靠读代码销（见 A 表第 2/3/4 行）：
安装器/发布脚本、`install-web.sh` 的 COS 布局、mermaid/WaveDrom 缩放。

> 📌 **给下一个人的提醒**：这次「只剩两条」之所以偏乐观，不是谁写错了，是
> **一轮会话只会收自己视野里的那份清单**。本文 §4 是跨三个仓库的总表 ——
> 销账前请像上面这样**对着代码量一遍**，别对着 session 文档的自述勾。
...
| ~~`engine.rs:1102` 截断工具调用~~ ✅ 已修（见 §4.0） | 仍直接发英文串，没走 `emit_info_coded`。**正常使用可达**                                                                                                                                                                                                                                                        | [spend]             |
