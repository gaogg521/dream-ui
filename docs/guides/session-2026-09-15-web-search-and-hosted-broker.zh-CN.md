# 2026-09-15 / 09-17：联网搜索从零接入、双服务商兜底、上线

涉及三个仓库：**dream-ui**（内置 MCP + 设置入口）、**dream-core**（白名单）、
**dream-trial-broker**（mode C 托管搜索）。

- **09-15**：从零做出联网搜索（§1-§5）。结论：客户端藏不住密钥，key 放 broker。
- **09-17**：加智谱做兜底、按「Tavily 免费额度用完」切换、**部署上线**（§7 起）。
  部署把服务弄挂过一次，根因是我自己的打包 bug，记在 §8。
...
1. broker 生产环境还没开 mode C。**→ 09-17 已上线，见 §9。**
2. `dream-trial-broker` 是本地仓库，没有 remote，提交只在本机。**→ 仍然如此。**
3. 仍未用真实 key 验证过的服务商：智谱、腾讯、Serper、Brave。
   **→ 智谱 09-17 已实测，见 §7；腾讯 / Serper / Brave 仍只核到文档层面。**
4. 调试期间用过的 key **一个都没有进仓库**（每次提交前 `git grep` 核过），但都出现在
   对话记录里，**建议全部吊销重发**。09-17 又多了智谱一个，共五个。

---

## 7. 09-17：智谱做兜底，以及三个只有真打才知道的事实

用户要求：**Tavily 优先，它每月 1000 次免费额度用完之后再走智谱**。

### 7.1 智谱的 URL 字段叫 `link`，不是 `url`

实测 200 返回里，字段是 `title` / `content` / `link` / `media` / `icon` / `refer` /
`publish_date`。照搬 Tavily 的归一化（读 `url`）会**一条结果都取不到，而且返回的是
健康的 200** —— 又一次「同一个概念在两家那里名字不同」的静默失效。

### 7.2 `search_std` 对中文新闻根本不给链接

一开始以为是账户没余额。用户充值后**重测完全一样**，于是逐档对比（同一个中文 query）：

| 引擎             | 结果数 | 有链接   | 价格 |
| ---------------- | ------ | -------- | ---- |
| search_std       | 10     | **0/10** | 0.01 |
| search_pro       | 10     | **0/10** | 0.03 |
| search_pro_sogou | 50     | 50/50    | 0.05 |
| search_pro_quark | 10     | 10/10    | 0.05 |

**智谱自研的两档（std/pro）对中文新闻不返回 URL** —— 那是授权进他们索引的内容，不是
爬来的；接第三方引擎（搜狗/夸克）才有。同一个 `search_std`，英文 query 是 8/8 有链接。

于是契约得改：**`url` 变成可选**。原来「无 URL 即丢弃」的规则会把智谱在最该发挥作用的
中文场景清空 —— 它不是返回空，是返回一堆被我们丢光的好内容。现在照常送出，但
`url` 缺省（不是空字符串），渲染成

```
(no link available — cite this as an unlinked source, do not invent a URL)
```

**这句话是必须的**：真机跑出来时这里打印的是字面量 `undefined`，那在模型眼里就是一个
没解析出来的 URL，下一步就是瞎编一个。

### 7.3 文档里两个我漏发的必填项

- `search_intent` 是 schema 的 **required** 字段。开着意图识别时，被判为闲聊的 query
  会返回 `SEARCH_NONE`、**根本不执行搜索**。显式发 `false`。
- `search_pro_sogou` 的 `count` 只接受 10/20/30/40/50，发 8 是无效值。

### 7.4 免费额度切换：为什么自己数，而不是等厂商拒绝

"Tavily 用完 1000 次再走智谱"有两种实现：

1. 等 Tavily 报错 → 但**哪个状态码代表额度耗尽是猜的**（这个服务在"猜厂商语义"上已经
   栽过两次，见 §3.3），而且第一个信号是一次失败的搜索；如果它是超额继续计费而不是
   拒绝，第一个信号就是**账单**。
2. broker 自己按月计数，到 1000 就跳过 —— 确定性，不依赖任何猜测。

选 2。计数键是 UTC 的 `YYYY-MM`，所以**月初自动切回 Tavily**，不需要复位任务。
两条细节：额度只被"厂商真正服务了的调用"消耗（网络故障没打到对方不计数，否则 Tavily
一次抖动就白烧免费额度）；**返回空也计数**，因为厂商照样收费。

`GET /internal/stats` 现在报每家的用量/上限/是否耗尽 —— 免费转付费在外部本来完全不可见，
只能等账单。

### 7.5 被真实数据推翻过的一版设计

中间实现过「按查询语言路由」：中文先问智谱。理由是实测 **Tavily 对中文 query 返回
非空但无关的结果**（问新能源汽车出口，给西交利物浦的专业介绍页），所以"失败或空才回退"
在中文场景下永远触发不了，**回退会是死代码**。

用户否决了，定为 Tavily 始终优先。这条结论保留在 broker 的模块注释和 README 里 ——
它仍然成立：**谁排第一，谁就负责所有中文查询，好坏都归它**，兜底看不到这些查询。

---

## 8. 上线事故：服务被我弄挂了 40 分钟

部署 v1 之后服务起不来，`migration 1 was previously applied but has been modified`，
**mode A（体验 key）一起停摆**。

### 根因是我自己的打包 bug

sqlx 给每个已应用的 migration 存了 SHA-384，字节不一致就拒绝启动。线上原本应用的是
**LF**，我 tarball 里是 **CRLF** —— 这台开发机 `core.autocrlf=true`，`git archive`
导出时做了转换，**我没验证行尾就发了包**。

用 `git -c core.autocrlf=false archive` 重打，`0001/0002` 校验和直接 OK。

同一个坑咬了第二次：部署脚本本身也是 CRLF（Windows 上 Python 的 `write_text()`
默认把 `
` 转成 `
`），服务器报 `/usr/bin/env: 'bash
': No such file or directory`。

> **教训**：跨 Windows→Linux 送任何文本文件（迁移、脚本、配置），**送之前验行尾**。
> `git archive` 不是"原样导出"，它会应用 `core.autocrlf`。

### 更该记的是：v1 脚本本身不合格

它**没备份旧二进制**，也**没有预检**。于是一个能跑的服务变成了起不来的服务，
而且没有一键回退。v2 补了三样：

1. **预检**：动运行中的二进制之前，先把库里记的校验和和文件算出来比对，直接报告是
   CRLF/LF/内容不同，并给出对应的 `sed` 命令
2. **备份**：`bin/dream-trial-broker.prev`
3. **起不来自动回滚**到备份二进制

这三样的顺序不是随意的 —— 预检在最前面，意味着最常见的失败根本走不到"停服务"那一步。

顺带发现：**线上从来没应用过 `0003`**，mode B 的表压根不存在。这次一并建上了
（0003/0004/0005）。
...
```
https://work.1oneclaw.com/trial-broker     active + enabled（重启存活）
providers = "tavily,zhipu"                 SEARCH_TAVILY_MONTHLY_CAP=1000
.env 0600 dreambroker                      回滚二进制 bin/dream-trial-broker.prev
```

公网 `POST /v1/search` 中英文实测均通，mode A 的 `/internal/stats` 未受影响。

**客户端不需要发版** —— broker URL 在打包构建里已经是默认值（`resolveTrialBrokerUrl`），
存量安装下次启动时迁移会把 env 补上并把条目启用一次（一次性标记
`migration.webSearchHostedEnabled_v1` 挡住重复启用）。

---

## 10. CDP 真机验证（09-17）

用 `DREAM_MULTI_INSTANCE=1` 起**第二个 dev 实例**（独立数据目录 `dream-ui-Dev-2`），
不影响已在运行的那个。全新数据目录，等于顺带验证首装体验。

| 检查项                                         | 结果                                                   |
| ---------------------------------------------- | ------------------------------------------------------ |
| 首装迁移写入                                   | `enabled: true` + broker URL + 32 位哈希 install id    |
| 设置页「内置联网搜索」                         | **开箱可用** + 免费额度说明                            |
| 智谱引擎下拉框                                 | 四档全在，默认 `search_std`                            |
| 选 `search_pro_quark` → 保存 → **整页 reload** | 仍是 `search_pro_quark`                                |
| 后端实际存储（不是表单显示）                   | `enabled=True`、`engine=search_pro_quark`、broker 正确 |
| 打包用的 bundle 打**生产 broker**              | 零配置返回 3 条带链接的结果                            |

顺带验证了一条刻意写的逻辑：**没填任何 key 就点保存，条目仍保持启用**
（`enabled = 有key || 有broker`）—— 否则用户存一次空表单就会把本来能用的搜索关掉。

**没验到的**：会话里模型自己决定调用 `web_search`。工具层（MCP 协议 + 生产 broker +
真实结果）已全通，差的是新实例没配模型。
...
1. **`dream-trial-broker` 没有 remote**，四个提交（`32b84d1` / `f428766` / `4e13dcd` /
   `3775659`）只在本机。要么给它建远端，要么每次靠 `git archive` 送包。
2. **仍未用真实 key 验证**：腾讯、Serper、Brave（只核到文档层面）。
3. **中文搜索质量取决于 Tavily**（见 §7.5）。免费额度内的中文结果相关性明显不如智谱，
   这是已知取舍，不是 bug。
4. **UTC 月初 = 北京时间 1 号早上 8 点**。Tavily 自己的账单周期若不是 UTC 月初，中间有
   几小时错位，影响很小但存在。
5. 调试期间用过的**五个 key**（博查 / 豆包 / 阿里云 / Tavily / 智谱）都在对话记录里，
   仓库里一个都没有，**建议全部吊销重发**。线上 `.env` 里那两份不受影响。
