...
> ③创建弹窗的 Agent 类型下拉写着内部代号 `DreamEngine`，而且**没有地方选专家**。
...
| `Provider '' not found`          | **真 bug，dream-engine 员工 100% 必挂** | 见下方链路                                                                                                                                                                                                  |
| 下拉写 `DreamEngine`、不能选专家 | **真缺口**                              | `CreateAgentModal.tsx` 硬编码三个裸字符串，不走 i18n、不走 agent registry；后端 `provision_run` 硬写 `assistant: None`                                                                                      |
...
3. 会话表 `model` 列存 NULL
4. `dream-core-conversation/task_options.rs:42` 把 NULL 解析成哨兵值 `provider_id: ""`
5. `dream-core-ai-agent/factory/dream-engine.rs:81-87` 拿空串查 provider → 查不到 → 报错
...
**误导的真正来源**：三个选项塞在同一个下拉里，但语义不一致——Claude/Gemini 自带模型（走 CLI 账号或桥接），dream-engine 必须显式给 provider + model，UI 却毫无提示。
...
| 关注点                                | 定时任务既有实现                                                                                          | 本轮                                                          |
| ------------------------------------- | --------------------------------------------------------------------------------------------------------- | ------------------------------------------------------------- |
| 只有 dream-engine 带顶层 model        | `dream-core-cron/executor.rs:965-970` `resolve_model()`                                                   | 逐字照搬进 one-employee                                       |
...
**硬约束（已核实）**：`dream-core-conversation/service.rs:795-802` —— 顶层 `model` **只允许 dream-engine**，其他类型传了直接 400。

**后端手动覆盖的唯一通道**：有 `assistant` 时 `r#type` 被忽略，effective type 由人设快照推导。覆盖必须走 07-28 加的
`AssistantConversationOverridesRequest.agent_id`（`dream-core-api-types/conversation.rs:44-49`，doc 注释原文就点名 "1ONE CLI"），
优先级见 `dream-core-conversation/service.rs:1473-1477`。
...
- **迁移 `004_persona_and_model.sql`** + 账本条目 `employee_004_persona_model`，四列：
  `assistant_id` / `agent_id_override` / `model_id`(ACP) / `model`(dream-engine 的 ProviderWithModel JSON)。
...
- `validate_model_binding()` 三条保存期规则：dream-engine **必须**有模型（否则又会退回 `Provider '' not found`）、
  非 dream-engine **不许**带顶层 model（否则撞 795 那条 400）、模型必须来自**已启用**的 provider
  （参照 `dream-core-team/provisioning.rs:702-714`）。第一条只在请求真的动了绑定时才强制，
...
- 新增 `utils/employeeDisplay.ts`——品牌名解析（catalog 里 dream-engine 那行**就是** `1ONE CLI`，迁移 019 设的，**永远不要硬编码 label**）
  - 运行错误友好化
...
- **真机端到端（29 条断言全绿）**，脚本直连后端 HTTP API + 用 `node:sqlite` 读真实库核对落库：
  ...
  **核心判据**——同一条路径上报错从 `Provider '' not found` 变成
  `DreamEngine agent error: Provider error: HTTP error: error sending request for url (http://127.0.0.1:59999/v1/chat/completions)`，
  ...
  后端覆盖真的把会话 type 翻成 `dream-engine`（证明 `conversation_overrides.agent_id` 生效）/ 编辑改模型落库 / 存量 backend-only 员工照常
...
——那是 `dream-core-db` 的 sqlx 账本（我的迁移在 one-employee 的独立 `_one_migrations` 账本里，与之无关），
对应工作区里并行会话正在改的 `.gitattributes` / `database.rs` 行尾校验和问题。
绕开方式：项目自带的 E2E 隔离（`DREAM_E2E_TEST=1` + `DREAM_E2E_USER_DATA_DIR=<临时目录>`，
...
- **团队路径的友好错误已失效**：`team_conversation_adapters.rs:570-572` 的 `is_stale_provider_binding` 只匹配
  ...
  （`dream-core-conversation/error.rs:80-81`），落进 `other =>` 分支直接透出原文。单测是手搓 `BadRequest` 所以测不出来
