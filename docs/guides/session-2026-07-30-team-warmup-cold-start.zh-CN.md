...
**提交**：dream-engine `02f3c50`（master）/ 1oneCore `ad407cdb` + `a229a3eb`（CLAUDE.md 索引）+ `1133f33c`（Cargo.lock 对齐 dream-engine），**均已推 `origin/one-main`**。
...
| 提交 / 推送                 | ✅ 三仓均已推（hash 见上）                            |
| 主干 `Cargo.lock` 对齐      | ✅ `1133f33c`，否则主干拿不到 dream-engine 那半边修复 |
| 打包                        | ❌ 未做                                               |
...
3. **打包前确认 bundled 后端** —— 这一天有三条会话各自重编过 `dreamcore.exe`，两个检出的 bundled 各缺一半修复，**直接打包会漏东西**。见 §11.4。

> 推送顺序（fork 铁律）本轮已按规矩走完：先推 dream-engine（`02f3c50`）→ 主干 `cargo update -p dream-engine-mcp` → `cargo build --locked` 验证 → 提交 `1133f33c`。**注意主检出 `1oneCore/Cargo.toml` 里那个指向 `../旧引擎本地检出` 的 `[patch]` 是未提交的本地临时措施，不在主干上**，所以主干必须靠 `Cargo.lock` 锁定的 commit 才能拿到 dream-engine 侧修复——这一步漏了的话，代码推了但修复不生效。
...
- 这跟团队没有本质关系。普通 dream-engine 会话走同一条路径，只是没有全屏 overlay 挡着，感受被放大了。

---

## 三 三个连不上的 MCP，三个完全不同的根因

一次冷启动里 dream-engine 日志报了三个失败，很容易被当成同一类问题，其实互不相干：
...
它**不在后端注入的列表里**（`mcp_names=["stock-sdk","team-search","ftshare","dream-core-team"]`），来源是 dream-engine 自己的全局配置：

```toml
# %APPDATA%\dream-engine\config.toml（2026-07-12 写入）
...
⚠️ **这条的教训比它本身重要**：数据库里那条 `one-web-tools` 是 `enabled=0` 的，但禁用完全没用——dream-engine 会读自己的 `config.toml`，那是应用 MCP 管理页看不见也管不着的一层。排查"某个 MCP 从哪来的"时，注入列表之外还要看这个文件。
...
### 5.1 `1oneCore/crates/dream-core-runtime/src/node_runtime/{mod,managed}.rs`
...
> ⚠️ **统计口径的坑**：我一开始跑的是 `cargo test -p dream-core-runtime node_runtime`（带模块过滤），得到"基线 5 红 → 现 3 红"，据此在早期结论里报过"3 条既有红"。跑完整 crate 才看到真实数字是 8 → 6（`node_runtime` 之外还有 `shell_env` 1 条、`spawn` 2 条）。**报既有失败数必须跑完整 crate，不能带过滤**。另一条并行会话报的"`dream-core-runtime` 8 条红"是对的，我的 3 条是口径问题。
...
### 5.2 `dream-engine/crates/dream-engine-mcp/src/manager.rs`
...
**测试**：新增 `default_startup_timeout_bounds_a_server_that_never_answers`，用 `#[tokio::test(start_paused = true)]` 的虚拟时钟精确断言在默认 deadline 放弃，外加一条 `<= 20_000` 的上界断言防止将来无意改回。dream-engine-mcp 58 passed。

⚠️ 已知代价：注入路径（`factory/dream-engine.rs`、`factory/acp.rs`）构造 `McpServerConfig` 时 `startup_timeout_ms` 一律写 `None`，**应用内配置的 MCP 用户无法单独调这个值**（改动前也一样）。走 dream-engine `config.toml` 的用户可以自己设。
...
| 对象                                 | 处理                                                            | 备份                                      |
| ------------------------------------ | --------------------------------------------------------------- | ----------------------------------------- |
| `%APPDATA%\dream-engine\config.toml` | 整个文件只有 `one-web-tools` 一段，重命名为 `.disabled`         | scratchpad `dream-engine-config.toml.bak` |
...
| 场景                           | 结果                                                                                                  |
| ------------------------------ | ----------------------------------------------------------------------------------------------------- |
| dream-engine 团队冷启动        | overlay 15.5s 清除；`ftshare`(172 工具)、`dream-core-team`(10 工具) 正常连上；`stock-sdk` 被 15s 截断 |
...
2. **`dream-core-runtime` 在 Windows 上还有 6 条红** —— 本轮只修了 `node_runtime::tests` 里的 2 条（顺带，因为它们挡住了我自己的验证），剩下 6 条同类根因（夹具/断言按 Unix 假设写）没动：
   ...
   已在干净的 `origin/one-main` worktree 上跑基线确认这 6 条改动前就红（**85 passed / 8 failed → 87 passed / 6 failed**）。**`cargo test -p dream-core-runtime` 在 Windows 上不是全绿基线**，接手人别把它当成自己引入的。另一条并行会话另报 `dream-core-team` 3 条红（同类定性），我未独立复核。

3. **dream-engine 的"影子 MCP 配置"没有任何 UI 可见性** —— `%APPDATA%\dream-engine\config.toml` 里的 server 会被注入，但应用的 MCP 管理页既看不到也管不了（§3.2）。这次是死路径所以只是噪音，但同样的机制可以让一个用户完全不知情的 MCP 进入每个会话。
...
- **同一天有两个 1oneUI 检出在用**：`1oneUI`（主检出，另一会话在改，当时 60 个未提交文件）和 `1oneUI-sync`（干净）。判断"用户当时跑的是哪个"的依据是团队会话 `extra.team_mcp_stdio_config.binary_path` 里记的 dreamcore 绝对路径。
- **`backend-rebuild.ps1` 写死内嵌到 `1oneUI`**。要给别的检出内嵌，在该检出下 `$env:DREAM_BACKEND_LOCAL_PATH=<exe>; node scripts/prepareDreamcore.js`。
...
| B    | 三仓上游同步收尾 + `dream-core-extension` 12 条 + WebUI 反代 | 见 1oneCore CLAUDE.md 中 `4821b65d` / `f70abde2` 条目                                                                            |
| C    | **本文** —— 团队唤醒冷启动                                   | 本文件                                                                                                                           |
...
| 说法                                                                              | 核实结果                                                                                                                             |
| --------------------------------------------------------------------------------- | ------------------------------------------------------------------------------------------------------------------------------------ |
| 会话 B：「dream-engine 那 2 个未提交文件不是我的，是别的会话在改 MCP 启动超时」   | **是本轮（C）的**。已提交推送 `02f3c50`，dream-engine 工作区现已干净。                                                               |
| 会话 B：「`dream-core-runtime` 红 8 条」 vs 本文早期的「3 条」                    | **B 是对的**。我最初带了 `node_runtime` 模块过滤所以口径偏小。完整 crate 基线确实 8 条，本轮修好 2 条剩 6 条。详见 §5.1 的口径提醒。 |
...
| 路径                                  | 位置       | 分支         | 未提交 | 说明                              |
| ------------------------------------- | ---------- | ------------ | ------ | --------------------------------- |
| `D:\旧中转目录\1oneUI-sync`           | 仓库**外** | `sync-v2142` | 0      | 本轮（C）用它跑 dev、做基线与验证 |
| `D:\旧中转目录\1oneUI\.sync-worktree` | 仓库**内** | `sync-v2142` | **87** | 会话 B 查到的雷，B 停手没动       |
...
| ----------------------------------------------- | ------------ | -------------------- | ------------------- |
| `1oneUI/resources/bundled-dreamcore/`（主检出） | Jul 30 22:26 | ✅                   | ❌                  |
| `1oneUI-sync/resources/bundled-dreamcore/`      | Jul 30 22:26 | ✅                   | ❌                  |
| B 自己检出里的那份                              | 更晚         | ❌                   | ✅                  |
...
⚠️ 但主检出 `1oneCore` 当前在 `sync-v0153` 且 ahead 8 / behind 2、暂存区里躺着另一批改动（会话 A 在 CLAUDE.md 里警告过：直接 `git commit` 会把 `dream-core-assistant/src/routes.rs` 回退），**切分支前先处理那个暂存区**。本轮为了绕开它，全程用 `git worktree` 在 `origin/one-main` 上提交，没碰主检出任何一个文件。
...
| #   | 事项                                                                                                                                                                                                                                                                                                        | 我的核实状态                                                                                                                                                                        |
| --- | ----------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | ----------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| 1   | **11 条后端测试仍红（等决定）** —— `dream-core-runtime` 8 条 + `dream-core-team` 3 条，全部是上游按 Unix 写的既有测试（断言 `Command` 的 Unix 专有 Debug 渲染、硬编码 `~/.nvm` 布局、`/conversations/acp-temp-` 正斜杠）。B 逐条定性：**无一是产品缺陷，无一由 B 引入**。                                   | `dream-core-runtime` 那 8 条**我独立核实过**（干净 `origin/one-main` worktree 上 85 passed / 8 failed），本轮修好 2 条剩 6 条，清单见 §9.2。`dream-core-team` 那 3 条**我未复核**。 |
| 2   | **主检出 `D:/旧桌面端仓库` 是隐患** —— 落后远端 73+ 提交、60 个未提交文件，其中 **21 个**与远端新提交重叠会撞：`enterprise/index.tsx`、`ipcBridge.ts`、`i18n-keys.d.ts`、13 个 locale 的 `common.json`。                                                                                                    | 未独立核实条数。**本轮全程走仓库外 worktree，没有加剧这个状态**，也没有动主检出任何文件。                                                                                           |
...
| 5   | **11 条后端既有红测试要不要收拾** —— `dream-core-runtime` 8 条（C 已修好 2 条，剩 6 条）+ `dream-core-team` 3 条（C 未复核）。均为上游按 Unix 写的测试，B 与 C 一致定性为**非产品缺陷**。 | B+C  |
...
| 9   | **主检出 `D:/旧桌面端仓库` 对齐** —— 落后 73+ 提交、60 个未提交文件、21 个与远端重叠会撞。**这是当前最大的一处隐患**，越晚处理撞得越狠。                                                                        | B     |
...
- ~~dream-engine 两个"无主"未提交文件~~ —— 是 C 的，已推 `02f3c50`，工作区干净。
- ~~主干 `Cargo.lock` 拿不到 dream-engine 修复~~ —— 已 `cargo update -p dream-engine-mcp` + `--locked` 构建验证，提交 `1133f33c`。
- ~~`dream-core-runtime` 红几条的口径分歧~~ —— 完整 crate 8 条为准，C 已核实并修好 2 条。
