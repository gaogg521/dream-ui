...
**日期**：2026-08-24 · **范围**：dream-ui（本次代码修复）+ 影响到 `D:\旧中转目录\1oneUI` 的本机数据

## 一、发生了什么

dream-ui 是 2026-08-23 从 `D:\旧中转目录\1oneUI` 原样复制出来的独立仓库（见 `CLAUDE.md`
...
1oneUI 那边的同名函数逻辑完全一样，只是判断的环境变量叫 `DREAM_MULTI_INSTANCE`——**两边解析
...
`dreamcore.exe` 认不出来的地方。之后用户重新打开 1oneUI 的 dev 环境时，其后端在启动阶段检测到
...
自己"多开模式"（`DREAM_MULTI_INSTANCE=1`）用的**第二个**共享槽位，跟 `1one-Dev` 是同一类
...
profile**，只有显式设置 `DREAM_MULTI_INSTANCE=1` 才会碰到它，属于潜伏风险而非当前阻断。
...
保留的历史值，为了让真实用户从 dream-ui/1oneUI 升级到 One Work 时 `%APPDATA%` 数据不丢失，这
