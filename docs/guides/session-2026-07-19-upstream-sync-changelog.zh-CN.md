# 上游同步变更清单（v2.1.32→v2.1.37 / Core 0.1.45→0.1.48 / dream-engine 0.2.2→0.2.5）
...
| ---------------------------------------- | --------------------------- | ------------------------------- | -------------------------------------------------- |
| `旧引擎本地检出` ← gaogg521/dream-engine | v0.2.2 + fork 补丁          | **v0.2.5**（另吃 main 上 #230） | `master` @ `78672b3`（v0.2.5 + #230 + 6 专属补丁） |
| `1oneCore` ← gaogg521/dream-core         | v0.1.45-one.1               | **v0.1.48**                     | **0.1.48-one.1**（`dreamcoreVersion` 同）          |
| `1oneUI` ← gaogg521/dream-ui             | 内容≈v2.1.32；产品号 2.1.46 | **v2.1.37**                     | 内容对齐 v2.1.37；**产品号仍 2.1.46**              |

级联：`dream-engine → 1oneCore → 1oneUI`。企业模块（`one-*`、设置 IA）全程保 fork。
...
| 能力                                                         | 来源                             | 我们是否吃进    | 备注                                                         |
| ------------------------------------------------------------ | -------------------------------- | --------------- | ------------------------------------------------------------ |
| **Dream CLI 能看图**（按模型能力路由图片附件）               | dream-engine 0.2.5 + Core 0.1.48 | ✅              | 依赖能力白名单；自定义网关需见 §5                            |
...
| Provider **流式诊断**（#230，v0.2.5 之后）                  | dream-engine               | ✅ 已合 fork `master`          |
| 团队 lead **工具策略**继承 / 作用域                         | dream-engine #226；UI 相关 | ✅                             |
...
| dream-engine pin  | 保持 `gaogg521/dream-engine` **`master`**，不用官方裸 tag                                       |
...
### 2.1 dream-ui（`v2.1.32` → `v2.1.37`，节选）
...
### 2.2 dream-core（`v0.1.45` → `v0.1.48`，节选）

- feat: 按模型能力路由图片附件；消费 dream-engine 结构化图片块
...
- chore: dream-engine → v0.2.5

### 2.3 dream-engine（`v0.2.2` → `v0.2.5` + #230）
...
- [x] 助手身份文案：**注入技能 / ACP 不再自称 dream-ui**（dev 隔离验证 22:22；见品牌专项）
...
- [x] bundled `dreamcore.exe` 含上述修复（看图约 21:45；品牌约 **22:20**）
...
| 角色                                         | 路径                                                                                    |
| -------------------------------------------- | --------------------------------------------------------------------------------------- |
| **白名单数据（模型 ID 列表）**               | `1oneCore/crates/dream-core-ai-agent/assets/model-capabilities/image_input_models.json` |
| **白名单原则 / 维护说明**                    | `1oneCore/crates/dream-core-ai-agent/assets/model-capabilities/README.md`               |
| **解析逻辑（匹配 API / 自定义网关 / 别名）** | `1oneCore/crates/dream-core-ai-agent/src/capability/image_input.rs`                     |
| 运行时剥图（dream-engine）                   | `旧引擎本地检出/crates/dream-engine-agent/src/engine.rs`（`project_image_input`）       |
...
| **本仓库开发者 / 运维**        | ✅ 可以     | 改 `image_input_models.json`（和必要时 `image_input.rs`）→ `cargo build -p dream-core-app --release` → `scripts/backend-rebuild.ps1`（或 copy 进 `1oneUI/resources/bundled-dreamcore/...`）→ 重启桌面 / 重新出包 |
| **只改 JSON 不重编**           | ❌ 无效     | JSON 是 `include_str!` 打进 `dreamcore.exe` 的                                                                                                                                                                   |
...
3. 出包前 bundled `dreamcore.exe` 须含 **看图 + 品牌** tip（≥ 22:20 那版或更新）。
