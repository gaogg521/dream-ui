# 上游同步（v2.1.37 / Core 0.1.48 / dream-engine 0.2.5）合并后 BUG/冲突审查报告
...
**没有发现会阻断发布的真实 BUG。** 三仓合并质量总体干净：6 类 dream-engine fork 专属补丁全部完整保留、1oneUI 企业设置 IA 和技能详情页手工移植都没有冲突残留、1oneCore migration 026-029 连续无冲突、dream-engine pin 版本一致无漂移。

发现 **2 项需要动手处理的真实遗留**、**1 项建议补测试的脆弱点**、**1 项需要接手者知晓但不用恐慌的历史包袱**：

| #   | 严重度         | 内容                                                                                                                                           | 仓       |
| --- | -------------- | ---------------------------------------------------------------------------------------------------------------------------------------------- | -------- |
| 1   | 待修复         | 品牌注入（`9504fa47`）遗漏了 4 个文件共 ~12 处 agent 会读回的 "dream-ui" 错误字符串                                                            | 1oneCore |
...
| 本次新功能相关代码里是否有品牌残留（"dream-ui"/"Dream CLI" 用户可见文案）       | **NO ISSUE**                       | 仅 license header 注释和 1 处内部代码注释命中，均非用户可见                                                                                              |
...
| 核查点                                                                                                     | 结论                               | 证据                                                                                                                                                                                                                       |
| ---------------------------------------------------------------------------------------------------------- | ---------------------------------- | -------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| migration 026-029 编号是否真的连续无冲突                                                                   | **NO ISSUE**                       | `crates/dream-core-db/migrations/` 001-029 连续无跳号无重复，内容与文件名描述匹配，`sqlx::migrate!()` 扫描目录取版本号不存在人工注册表脱节风险                                                                             |
...
| dream-engine pin 是否指向 `gaogg521/dream-engine` master 且版本一致                                        | **NO ISSUE**                       | `Cargo.toml`/`Cargo.lock` 全部 12 处一致指向 `78672b36...`，短哈希与 dream-engine 仓库当前一致，无漂移                                                                                                                     |

### 2.1 品牌注入遗漏详情（待办）

`9504fa47`（"注入技能与 ACP 身份改为 1One Work"）本意是让助手/ACP 不再自称 "dream-ui"，但只覆盖了 `cmd_capabilities.rs` 里的两处 description。以下路径**仍然硬编码 "dream-ui"**，且都是 agent 会主动调用（`dreamcore config` / `diagnose` / `doctor`）并把 stdout/stderr 读回自己上下文的命令，出错时模型仍会看到 "dream-ui" 字样，与本次修复的初衷矛盾：

- `crates/dream-core-app/src/commands/cmd_config.rs:1385,1395,1411,1432,1468`（"failed to call dream-ui backend" 等 5 处错误字符串）
- `crates/dream-core-app/src/commands/cmd_diagnose.rs:307,317,326,340,359`（同类 5 处）
- `crates/dream-core-app/src/commands/cmd_doctor.rs:71`（"dream-ui backend doctor —…"）
- `crates/dream-core-app/src/cli.rs:13`（`about = "dream-ui Backend Server"`）
...
⚠️ 另外它的 `Cargo.lock` 基线早于本次同步的 dream-engine pin 提交，直接 `pop` 会产生 Cargo.lock 冲突。**接手者需要做的判断只有一件事**：这个新架构方向要不要推进——要推进就手工重新整理，不要直接 `git stash pop`；不推进也可以直接 `git stash drop`，不影响任何生产代码路径。

## 3. dream-engine（`master` / `sync-v025` @ `78672b3`，四项核查）
...
| 看图能力判断（`project_image_input`）是否会跟 1oneCore 白名单产生"各自判断"分歧                                                                         | **架构上不可能分歧，但有跨仓时序风险** | dream-engine 自身无任何白名单/判断逻辑，能力值完全由 Core 通过 `SetConfig` 注入；唯一风险是 Core 切模型时若忘记带 `image_input` 字段会 fail-closed 剥图——这是已知的跨仓时序问题，非本次同步新引入 |
...
- [x] ~~补齐 1oneCore 品牌注入遗漏的 4 个文件 "dream-ui" agent-facing 字符串~~（`1oneCore@sync-v0148` `1305500d`；实际 ~15 处：cmd_config/diagnose 各 5、doctor 1、cli.rs 4——审查漏列了 cli.rs 91/93/317 三处 `--help` 文案，一并补齐）
...
- [x] ~~`sync-2137` / `sync-v0148` / dream-engine 各自合进 `one-main`（周末同步收尾）~~（2026-07-20；dream-engine master/sync-v025 本就同一 commit `78672b3` 无需动；1oneCore one-main FF 合并 sync-v0148 → `282f5c02` 已推送；1oneUI one-main FF 合并 sync-2137 → `5e593f818` 已推送，随手带的 5 个游离素材图片改动另提交 `0f33be347`）
- [x] ~~出正式包前确认 bundled `dreamcore.exe` ≥ 含 `9504fa47`（品牌）+ `1305500d`(品牌补齐) 的 build~~（2026-07-20；核实原 bundled exe 停在 07-19 22:20（只含 `9504fa47`），缺 07-20 10:50 的 `1305500d`/`282f5c02`；已 `cargo build -p dream-core-app --release`（HEAD=`282f5c02`）+ `prepareDreamcore.js` 重新内嵌，`resources/bundled-dreamcore/win32-x64/dreamcore.exe` 现已是最新 build）。
...
- **恢复点一句话**:stash 里 Part A(one-enterprise crate)/Part B(one-sso)/Part C 主体已做;**没做完** = ① one-org B2 测试清理(service.rs 还有 8 处 `auto_provision_enterprise` 引用→编译不过,中断点)② Part D dream-core-app 装配(stash 没动)③ Part E 前端(没动)④ 编译测试。
