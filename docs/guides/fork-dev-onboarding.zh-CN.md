...
> **仓库更名（2026-07-08）**：GitHub 与本地目录已由 `dream-ui` / `dream-core` 改为 **`1oneUI` / `1oneCore`**。旧链接见 [repo-rename-2026-07-08.zh-CN.md](repo-rename-2026-07-08.zh-CN.md)。
...
```text
D:\旧中转目录\                 ← 工作区根（不是 git 仓库，放 scripts）
...
| 你要做的事                  | 进入目录                                      |
| --------------------------- | --------------------------------------------- |
| 改界面 / 设置页 / 打包前端  | `D:\旧中转目录\1oneUI`                        |
| 改 API / 数据库迁移 / Agent | `D:\旧中转目录\1oneCore`                      |
| 一键起 dev / 重编后端       | 在任意处执行 `D:\旧中转目录\scripts\*.ps1`    |
| Cursor 打开整个工程         | **`D:\旧中转目录`**（推荐，同时看到前后端）   |
...
等价于在 `1oneUI` 下 `bun run dev`；数据目录 `%APPDATA%\dream-ui-Dev`（与正式安装隔离）。
...
**常见误区**：`bun run dev` **不会**自动编译 Rust；dev 启动的是 `1oneUI/resources/bundled-dreamcore/.../dreamcore.exe`。
...
```powershell
D:\旧中转目录\scripts\package-win.ps1            # 用现有本地 dreamcore.exe 打包
D:\旧中转目录\scripts\package-win.ps1 -Rebuild   # 改过后端时:先 cargo build 再打包
```

或手动等价：

```powershell
D:\旧中转目录\scripts\backend-rebuild.ps1   # 若后端有改动

cd D:\旧中转目录\1oneUI
$env:DREAM_BACKEND_LOCAL_PATH = 'D:\旧中转目录\1oneCore\target\release\dreamcore.exe'
bun run dist:win
# 产物：out\1ONE Code-<version>-win-x64.exe
```

> ⚠️ **`DREAM_BACKEND_LOCAL_PATH` 不是可选的。** 打包链(`build-with-builder.js` →
> `prepare-dreamcore.js`)默认按 `package.json` 的 `dreamcoreVersion` **去 GitHub Release 下载**
> dreamcore 二进制;私有 fork 那个 tag(如 `v0.1.48-one.1`)通常没有对应平台产物,
> 不设这个变量就会在 electron-builder 之前挂掉:
> `❌ Build failed: dreamcore binary not found for win32-x64 (tag: ...)`。
> 设了它就走 `prepare-dreamcore.js` 的优先级 0 本地分支、跳过下载;日志出现
> `Bundled dreamcore prepared: ... [source=local]` 即对。`package-win.ps1` 已替你设好。
>
...
`scripts/` 放在 `旧中转目录` 根下，**不是** git 仓库的一部分（避免污染 fork 提交）。若你只有两个 clone、没有 scripts 文件夹，可从同事处复制 `D:\旧中转目录\scripts\` 三个 `.ps1`，或按 [`scripts/README.md`](../../../scripts/README.md)（若已复制到本地）手写等价命令。

| 脚本                  | 作用                                                    |
| --------------------- | ------------------------------------------------------- |
| `frontend-dev.ps1`    | 起 Electron dev                                         |
| `backend-rebuild.ps1` | `cargo build` + 内嵌到 `resources/bundled-dreamcore`    |
| `backend-run.ps1`     | 仅跑后端 HTTP API                                       |
| `package-win.ps1`     | 打 Windows 安装包(dist:win),自动用本地 dreamcore 不下载 |
...
| 项            | fork (`gaogg521`)              | 上游                               |
| ------------- | ------------------------------ | ---------------------------------- |
| 前端          | `gaogg521/1oneUI` `one-main`   | `gaogg521/dream-ui` `main`         |
| 后端          | `gaogg521/1oneCore` `one-main` | `gaogg521/dream-core`              |
| 品牌          | **1ONE Code** / 1ONE CLI       | dream-ui / Dream CLI（上游产品名） |
| 关于页 GitHub | `github.com/gaogg521/1oneUI`   | `github.com/gaogg521/dream-ui`     |
| 官网          | `https://1one.1oneclaw.com`    | `https://www.dream-ui.com`         |
...
| `D:\旧中转目录\scripts\README.md`                                                                | 脚本与加载速查（本地）                      |
