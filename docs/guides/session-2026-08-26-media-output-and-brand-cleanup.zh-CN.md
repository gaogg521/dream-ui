...
| 端        | 变量名                                                      | 位置                                    |
| --------- | ----------------------------------------------------------- | --------------------------------------- |
| Rust 发出 | `DREAM_MEDIA_WORKSPACE_DIR` / `DREAM_MEDIA_CONVERSATION_ID` | `dream-core-mcp/src/media_workspace.rs` |
...
（`readEnv('DREAM_MEDIA_WORKSPACE_DIR', 'DREAM_MEDIA_WORKSPACE_DIR')`）——
桌面端配的是固定版本的 `dreamcore`（`package.json` 的 `dreamcoreVersion`），
...
| 新名                                                                    | 老名（只读兜底）               |
| ----------------------------------------------------------------------- | ------------------------------ |
| `one-config.txt` / `one-chat-message.txt` / `one-chat.txt` / `.one-env` | `dream-ui-*` / `.dream-ui-env` |
| `one-chat-history/`                                                     | `dream-ui-chat-history/`       |
...
**`dream-ui.db` 刻意不改名**：它是只读的历史遗留库，没有任何代码创建它，改名反而弄坏读它的迁移。

## 5. 顺带修掉的真 bug

- `sentry.ts` 的 `getInstallPathKind()` 只认 `\programs\dream-ui\resources`，
  但实际安装目录是 `executableName` 派生的 `1onecode` → **所有 Windows 装机都误报 `custom`**，
  正好把这个 tag 存在的意义抹掉了。现在两个名字都认。
- `x-dream-ui-internal` 是跨仓协议头（dream-ui 发、dream-core-cron 收）。
...
| 项                                                     | 理由                                                 |
| ------------------------------------------------------ | ---------------------------------------------------- |
| `dream-ui://` 深链、`appId`、`1ONE Code` userData 目录 | CLAUDE.md 列为冻结历史值                             |
| Sentry tag 命名空间 `dream-ui.*`                       | 改了会断历史报表/告警                                |
| localStorage 键、DOM 事件名                            | 前者改了会重置用户偏好，后者纯内部约定，本次不在范围 |
| `[[AION_FILES]]` marker 值                             | 活的跨进程协议，值本身不含品牌                       |
| `_dream-ui_` 时间戳分隔符                              | 两仓都只声明未使用，死常量                           |
...
**改了后端就必须重编**：Rust 侧的 `DREAM_MEDIA_*` 改名要等新 `dreamcore` 发版 +
抬 `dreamcoreVersion` 才生效；在那之前靠 TS 的旧名兜底工作。
...
| 类别                      | 旧 → 新                                                                                                                                             |
| ------------------------- | --------------------------------------------------------------------------------------------------------------------------------------------------- |
| window CustomEvent 名 ×10 | `dream-ui-workspace-*` / `dream-ui-chat-*` / `dream-ui-update-*` / `dream-ui-open-*` / `dream-ui:speech-to-text-config-changed` → `one-*` / `one:*` |
| AudioWorklet processor    | `dream-ui-pcm-capture` → `one-pcm-capture`                                                                                                          |
| CDP 内部协议              | WS 路径 `/dream-ui-cdp` → `/one-cdp`；`dream-ui-browser-{target,session,context}` → `one-browser-*`                                                 |
| preload E2E 全局          | `__dream-uiE2ETest` → `__oneE2ETest`                                                                                                                |
...
> `__dream-ui_theme` 是例外：`index.html` 的内联脚本在任何模块加载前就要读它（防闪白），
> JS 迁移来不及，所以那两处直接写成 `getItem('__one_theme') || getItem('__dream-ui_theme')`。

**`dream-ui.dir` env 键**：读时 `getSync('one.dir') ?? getSync('dream-ui.dir')`，
写时**两个键都写**，降级不丢用户自定义的 cache/work/log 目录。

**内置浏览器 MCP 名** `dream-ui-browser` → `one-browser`：新增
...
**headless 数据目录** `~/.dream-ui-server` → `~/.one-server`：同样是「新名不存在且老名存在就用老名」。

### 这一轮仍然不动（有硬理由）

| 项                                            | 理由                                                                                                |
| --------------------------------------------- | --------------------------------------------------------------------------------------------------- |
| `dream-core-assistant`                        | 是**已发布 SQL 迁移**里的 `source_ref`（`018_reset_builtin_assistant_enabled.sql`），迁移文件不可改 |
| `persist:dream-ui-browser` session partition  | 改了内嵌浏览器的 cookie / 登录态全丢                                                                |
| `dream-ui.*` Sentry tag 命名空间              | 改了断历史报表与告警                                                                                |
| `migrations.ts` 里的 `'dream-ui'` source 值   | 是 schema 历史，已发布的迁移不可改                                                                  |
| `dream-ui://` 深链、`appId`、`1ONE Code` 目录 | CLAUDE.md 冻结值                                                                                    |

### 验证

`tsc` 0 错误；`npx vitest run` **544 文件 / 5059 测试全绿，0 失败**。
dream-engine 全仓扫描无任何 `dream-ui` 残留。

---

## 10. 第三轮：`dream-ui://` → `dream://`，浏览器 partition

### 深链 scheme：**后端必须先放行**

`dream-ui://` 能改，但有个致命前置：dream-core 的 `sanitize_deep_link_scheme`
（`dream-domain-sso/src/routes.rs`）对**任何不认识的值都 fallback 回 `"dream-ui"`**。
桌面端配的是固定版本 dreamcore，所以如果 UI 单方面改成只发/只认 `dream://`：

> 用户在浏览器里登录成功 → 后端回调页拿 `dream-ui://sso-callback` → UI 不认 → 静默丢弃 →
> **SSO 登录直接坏掉，且界面上没有任何报错**。

正确顺序（本次已按此执行）：

1. **dream-core 先放行**：`sanitize_deep_link_scheme` 增加 `dream` / `dream-dev`，
   保留 `dream-ui` / `dream-ui-dev`；**默认 fallback 仍是 `dream-ui`** —— 不发 `scheme` 参数的
   是老客户端，它只向 OS 注册过 `dream-ui://`。
2. **UI 两个都注册、两个都认**：
...
⚠️ 这意味着**后端不发版的话，新 UI 走的仍是 `dream-ui://` 回调路径**（后端 fallback），
但因为两个都注册、两个都认，功能是好的。发版后自动切到 `dream://`。

### 浏览器 session partition

`persist:dream-ui-browser` → `persist:one-browser`，但**不是无条件改名**。
...
- **`migrations.ts` 里的 `'dream-ui'`**：`runLegacyDatabaseMigrations` 在
  `dream-ui.db` 不存在时直接 early return。新装机**从来不会执行**这些迁移，那些字符串是
  纯历史 schema。而且已发布的迁移内容改了会破坏校验和。
- **`dream-core-assistant`**：是 dream-core 已发布 SQL 迁移
...
全新用户依然会拿到 `dream-core-assistant`，工作区文件树里依然是 `.dream/skills/dream-ui-config`。

### 涉及 5 个标识符

`dream-core-assistant` → `one-assistant`，以及四个技能
`dream-ui-config` / `dream-ui-troubleshooting` / `dream-webui-public` / `dream-webui-setup`
→ `one-*`。
...
`DREAM_{USER_ID,CONVERSATION_ID,HELPER_BIN,BASE_URL,RUNTIME_TOKEN}` → `ONE_*`
（定义在 `dream-core-ai-agent/src/types.rs`）。

**新旧两套都注入**：内置技能和 helper CLI 跟后端同一个二进制发布、不存在版本偏移，
但**技能是用户可自己写的** —— 谁的自定义技能里写了 `$DREAM_BASE_URL`，只发新名会让
那条命令收到空字符串，坏了还没地方看。
...
JSON 列是 TEXT，替换针对**带引号的 token**（`"dream-ui-config"` 而不是裸名），
否则 `dream-ui-config-extra` 这种更长的 id 会被误伤。测试里专门钉了这一条。
...
但**新旧 id 都匹配** —— 桌面端配的是固定版本 dreamcore，比后端新是常态，
只认新 id 会让「找管家」在没升级后端的装机上静默返回空。

### 仍未改

`DREAM_LOG_DIR` / `DREAM_FONTS_DIR` / `DREAM_CHANNEL_SEND`：全仓扫过，
**没有任何代码设置或解析它们**（只出现在技能文本和一个 bundled JS 里）。
改了只有「仓外某处在用」的风险、没有收益，留着。

---

## 12. 第五轮：`DREAM_*` 环境变量家族彻底改名

### 先确认了一件事：这批**不是**跨仓契约

一开始以为 dream-ui 拉起后端时会设 `DREAM_{CACHE,WORK,LOG}_DIR`（`index.ts` 里有句注释这么提）。
实际查下来 **dream-ui 一个都没设** —— 它用的是 `DREAM_BACKEND_*`；`DREAM_WORK_DIR` 是后端
`bootstrap/environment.rs` **自己设给自己子进程**的。所以这批是后端内部 + 运维可设的 CLI 变量，
可以彻底改。

### 做法：一次性采纳，而不是 50 个回退

`DREAM_*` → `ONE_*` 共 38 个变量。没有在每个读取点写回退（其中一半是 clap 的
`#[arg(env = "...")]`，只接受一个名字），而是在 `main()` / `admin.rs` 最开头调用
`dream_core_common::adopt_legacy_env()`：

> 遍历 `ADOPTED_ENV_SUFFIXES`，凡 `ONE_X` 未设而 `DREAM_X` 有值就拷过去。新名永远优先。
...
（`DREAM_FILES_MARKER` → `ONE_FILES_MARKER` 等，**只改标识符，wire 值 `[[AION_FILES]]` 不动**）。
`cmd_capabilities.rs` 对外声明的也换成新名。

### 差点引入的安全回归

`registry.rs::is_blocked_override_env_key` 是道护栏：禁止用户自定义的 env override 覆盖内部变量，
判断依据是 `DREAM_` 前缀。改名后如果不同步，**用户就能覆盖 `ONE_RUNTIME_TOKEN` / `ONE_BASE_URL`**。
...
1. `legacy_env.rs` 的测试 —— 本来是「设 `DREAM_DATA_DIR`、断言 `ONE_DATA_DIR` 拿到值」，
   被扫描改成设置和断言同一个变量，测试全绿但什么都没测。
2. `types.rs` 的 `LEGACY_*` 常量 —— 值被改成新名，于是「双注入」变成同一个名字写两遍，
   **旧名兜底整个消失**。这个是靠一个 `count() == 1` 断言碰巧抓到的。

两处都改成 `concat!("DREAM", "_X")` 构造，未来的字面量扫描碰不到；`types.rs` 另加了个测试
断言两种拼写必须不同。**教训：写完批量改名，要专门回头检查"负责兼容旧值"的那些文件。**

### 仍未改

`DREAM_LOG_DIR`（skill 文本里）、`DREAM_FONTS_DIR`、`DREAM_CHANNEL_SEND`：
...
（本机快约 10 倍），并修掉 10 处残留的 `dream-ui-<crate>` 占位符。
...
| MCP 活跃行        | 只有 `one-*` 五个；两个 `dream-ui-*` 已是墓碑 ✅                            |
