...
> 作者：接手 AI（2026-07-08 第二十三轮续）。仓库 `D:\旧中转目录\dream-ui`(one-main)。
...
- ❔ **运行时消费**：agent 实际执行时是否加载团队 `oneDevops` 技能/MCP（还是只加载本机 FS 技能）？需查后端 dreamcore 的 skill/mcp 装配链路（本文档暂标未确认，进阶段 C 时先勘察后端）。
...
> ⚠️ 记忆（/memory）目前**无团队后端**（`oneDevops` 只有 skills/mcp/rag，没有 memory）。team-memory 下发需先在 dreamcore 加租户级 memory 注册表/接口，属阶段 B 的后端前置。

### 阶段 C（后续，需先勘察后端）——运行时真正消费

确认并打通 agent 执行时装配团队技能/MCP：读 dreamcore skill/mcp 注入链路，若只认本机 FS 则需让团队注册表参与 materialize。**这是「策略分发」真正生效的一环**，工程最深，单独立项。
