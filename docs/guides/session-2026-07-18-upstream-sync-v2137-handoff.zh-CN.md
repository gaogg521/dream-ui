...
> **目标**：吃进上游 dream-ui **v2.1.33→v2.1.37** + dream-core **v0.1.46→v0.1.48** + dream-engine **v0.2.3→v0.2.5**。  
...
| 后端版本钉        | `dreamcoreVersion = vX.Y.Z-one.N`                                               | 绝不取上游裸 `v0.1.48`                                    |
| dream-engine 依赖 | `Cargo.toml` → `gaogg521/dream-engine` **`master`**                             | 绝不跟上游改成 `gaogg521/dream-engine` tag                |
...
4. dream-engine 6 专属补丁丢失（文本化兜底 / 空参 / thinking 阶梯 / deferred / GLM）
...
| 仓           | 开工前                      | 目标上游                               | 目标 fork                 |
| ------------ | --------------------------- | -------------------------------------- | ------------------------- |
| dream-engine | v0.2.2 + 6 补丁（`master`） | **v0.2.5**                             | v0.2.5 + 6 补丁重贴       |
...
**级联顺序（强制）**：① dream-engine → ② 1oneCore → ③ 1oneUI。  
...
| 文件                                                  | 决策                                                                                                |
| ----------------------------------------------------- | --------------------------------------------------------------------------------------------------- |
| `Cargo.toml` / `Cargo.lock`                           | 保 `one-*` + `dream-engine-* = { git=gaogg521/dream-engine, branch=master }`；版本 → `0.1.48-one.1` |
...
**禁止**：把 dream-engine 改成官方 tag；删除 Bun 时确认 fork 无依赖再跟。
...
| `package.json` version / dreamcoreVersion                        | 保 fork 号；dreamcoreVersion → 将来的 `v0.1.48-one.1`                           |
| locales 品牌                                                     | 合并后批量 `dream-ui →` 显示品牌                                                |
| NSIS / `dream-ui.exe`                                            | 扫 `1onecode.exe` / `DREAM_APP_EXECUTABLE_FILENAME`                             |
...
| 能力                                        | 仓                               | 备注                     |
| ------------------------------------------- | -------------------------------- | ------------------------ |
| CLI 看图                                    | dream-engine 0.2.5 + Core 0.1.48 | 三仓级联                 |
...
1. dream-engine `sync-v025` → `master` + push
2. 1oneCore 重编 `dreamcore.exe` → prepareDreamcore → tag `v0.1.48-one.1` → `one-main`
3. 1oneUI 回填 `dreamcoreVersion`、bump、合 `one-main`、`dist:win`（不删旧 exe）
...
| 2026-07-18       | **dream-engine `sync-v025`**：merge v0.2.5 + 保留 deferred/GLM/textualize；`cargo test --workspace` 全绿                                                                                               | ✅ 分支 tip `4ead7e1`（**未合 master，push 因 GitHub 网络失败**）                                                                                                         |
| 2026-07-18       | **1oneCore `sync-v0148`**：merge v0.1.48 → `0.1.48-one.1`；**`crates/one-*` 相对 one-main 零 diff**；`cargo check --workspace` 绿                                                                      | ✅ tip `3634e5b8`。⚠️ **临时** `dream-engine-*` 为 path=`../旧引擎本地检出/...`（网络不通无法拉 git）；合主前须改回 `gaogg521/dream-engine` `master` 并 `cargo update`    |
| 2026-07-18       | **1oneUI `sync-2137`**：merge upstream/main(=v2.1.37)；Router/SettingsSider/Capabilities/Enterprise **保 fork**；`dreamcoreVersion=v0.1.48-one.1`；`tsc --noEmit` 0                                    | ✅ tip `60cd183fa`。⚠️ 技能详情页/批量删除/UsedBy **未整吃上游 SkillsSettings**（防冲企业 IA），待手工移植进 fork `SkillsHubSettings`；拖拽 polish 保 fork GroupedHistory |
...
| 2026-07-19 晚    | **品牌补齐**：注入技能 / ACP / capabilities 用户可见名 → **1One Work**（保留 `dream-ui-*` / `DREAM_*`）                                                                                                | ✅ commit `9504fa47`；详见 [`session-2026-07-19-brand-skills-acp.zh-CN.md`](session-2026-07-19-brand-skills-acp.zh-CN.md)                                                 |
| 2026-07-19 22:22 | **隔离 dev 验证**：`1one-Dev`；窗口标题 1One Work；`/api/skills` 无 dream-ui 品牌串                                                                                                                    | ✅                                                                                                                                                                        |

### 合主前必做

1. `dream-engine`: push `sync-v025` → 合 `master`（或用户授权 force-with-lease）
