...
1. **被动**：客户端识别推理返回的 403 + `Key limit exceeded` 特征 → 弹充值提示。
   反应最快，但耦合了厂商的错误语义，多厂商时每家都要适配。
   **注意 [§1.2](#12-额度耗尽时厂商返回什么) 的泄露问题，不能透传原文。**
2. **主动**：broker 新增 `POST /v1/quota/status`（`install_id` 放 body，不放 query
   string），内部用 `read_usage` 查。返回 `{vendor, limit_usd, used_usd, remaining_usd,
   reset_period, resets_at, exhausted}`。厂商无关，还能做"额度快用完了"的提前提醒。
...
3. **额度耗尽时的产品形态**：硬阻断（不能再发消息）还是降级（回落到 `openrouter/free`
   免费池继续用，只是慢/弱）？后者体验好很多，且技术上几乎免费——
   免费池不消耗 key 的 USD 额度。**但注意免费池的请求配额是账号级全局共享的**，
   所有体验用户共用一个天花板。

4. **要不要现在就接第二家厂商**，还是先只做抽象、等有实际合作再接。
