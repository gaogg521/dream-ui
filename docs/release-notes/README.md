...
每一版的 `zh`/`en` 恰好两个条目，各自内部恰好三个小节，顺序固定：

| 条目 `t`                        | 小节（`【】` 包起来单独成行）        |
| ------------------------------- | ------------------------------------ |
| `单机版` / `Personal edition`   | `【新增】` `【优化改进】` `【修复】` |
| `企业版` / `Enterprise edition` | `【新增】` `【优化改进】` `【修复】` |

- 小节名**不能自创**。官网 `changelog-render.js` 的 `updatesCategoryClass()` 按关键字
  上色：含「新增」→ `new`（亮薄荷绿）、含「改进」→ `improved`（亮青）、含「修复」→
  `fixed`（亮红）。写成「真实缺陷修复」「安装与更新」这类自创名字，要么掉进兜底的
  `topic` 色、要么和别的小节撞色，用户就分不清哪条是修复、哪条是新增。
- 某一版某个小节确实没有内容，就**整段省略**，不要留空标题。
- 小节里每条以 `· ` 开头（渲染成有序列表并自动计数）；正文第一段不带 `【】`，
  渲染成该条目的导语。
- 这三个名字同时是 `CHANGELOG.md` 里的 `####` 小标题，由
  `docs/release-notes/<version>.json` 生成，两边不要各写一份。

> 2026-09-11 统一了全部历史条目。在此之前每版自创分类（3.0.0 一版就有 8 个），
> 用户无法横向比较「这版修了什么」，而「修复」当时还是 `var(--muted)` 灰色，
> 是三个小节里唯一被视觉降级的那个。

- 中文用全角标点（`，；：（）`），与其余条目一致；紧贴 ASCII 技术串的除外
  （`one-backend.db`、`height=3`、`(1 厘米 = 360000)` 保持半角）。
