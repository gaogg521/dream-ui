...
## 3.0.0 = Dream 品牌 + 企业版架构重构起点

**`3.0.0` 是一次有意的品牌里程碑跳版**，不是按下方「号段含义」表逐条套出来的结果。这一版
标记的是产品叙事层面的换代：

- 建立自有 **Dream** 品牌与技术身份（`2.2.0` 起的三仓独立化在 `3.0.0` 对外成型）。
- **企业版架构重构**——治理面（org / sso / enterprise / billing / platform）feature 门控、
  admin 独立进程、企业版主存储切换等一系列结构调整的起点。

严格按表，架构/品牌换代属 MINOR（`2.2.0` 已用过一次）。`3.0.0` 是产品/市场决策**主动**把
这条线提级到 MAJOR，用来对外传达「新阶段」。代价是永久跳过 `2.3`~`2.9`（版本号单调递增、
不可重用）——这是知情选择。后续版本若无同级别的换代，回落到 `3.x` 的 MINOR/PATCH 常规节奏。
...
| 位    | 什么时候进位                                                                                | 例子                                |
| ----- | ------------------------------------------------------------------------------------------- | ----------------------------------- |
| MAJOR | 产品形态或数据格式发生不可回退的变化，老版本装不回去；或经产品决策主动提级的品牌/架构里程碑 | `3.0.0` Dream 品牌 + 企业版架构重构 |
