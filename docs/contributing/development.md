...
- **Rust stable + Cargo** — Required to build the local dream-core backend ([install](https://rustup.rs))
...
dream-ui development uses two repositories:

- **dream-core** (`https://github.com/gaogg521/dream-core.git`) builds the local backend binary: `dreamcore` on macOS/Linux and `dreamcore.exe` on Windows.
- **dream-ui** (`https://github.com/gaogg521/dream-ui.git`) starts the Electron desktop app and launches the backend binary automatically.
...
The desktop development server resolves the backend from the `PATH` inherited by `bun run start`. Install dream-core first, verify the binary is discoverable in the same terminal, then start dream-ui.
...
### 2. Build and Install dream-core

Run these commands from the `dream-core` repository.
...
cd dream-core
cargo clean
cargo install --path crates/dream-core-app --locked

# Make Cargo-installed binaries visible to this shell if needed.
export PATH="$HOME/.cargo/bin:$PATH"

# Verify that dream-ui will be able to find the backend.
which dreamcore
dreamcore --help
```

If `which dreamcore` prints nothing, add `export PATH="$HOME/.cargo/bin:$PATH"` to your shell profile (`~/.zshrc`, `~/.bashrc`, or your shell's equivalent), open a new terminal, and verify again.
...
If `where.exe dreamcore` prints nothing, make sure `%USERPROFILE%\.cargo\bin` is in your user `Path`, open a new PowerShell window, and verify again.

### 3. Start dream-ui

Run these commands from the `dream-ui` repository in a terminal where `dreamcore` is discoverable.
...
During startup, dream-ui launches `dreamcore` automatically and passes the backend port to the renderer. You do not need to start dream-core in a separate terminal.

## Updating the Local Backend

When you pull or change dream-core, reinstall the backend binary and restart dream-ui:

```bash
cd ../dream-core
cargo install --path crates/dream-core-app --locked --force

cd ../dream-ui
bun run start
```

Use `--force` when rebuilding local changes with the same dream-core package version; otherwise Cargo may keep the already installed binary.

## Backend Startup Troubleshooting

### `Cannot find "dreamcore" binary`

dream-ui cannot find the backend from the `PATH` inherited by `bun run start`.

Check from the same terminal where you start dream-ui:
...
If the command fails, add Cargo's binary directory to `PATH` and start dream-ui from a new terminal.

### `dreamcore` Works in a Terminal but dream-ui Still Cannot Find It

Make sure you start `bun run start` from the same terminal environment that can run `dreamcore --help`. IDE terminals and GUI-launched shells can inherit a different `PATH`; restart the IDE or launch it from a terminal after updating `PATH`.

### Backend Changes Do Not Show Up

Quit dream-ui, reinstall dream-core with `cargo install --path crates/dream-core-app --locked --force`, then start dream-ui again. The Electron app owns the backend subprocess during development, so a running dream-ui instance will not pick up a newly installed binary until it restarts.

### Windows Rust Build Errors

Use the Rust MSVC toolchain and install Microsoft C++ Build Tools. After installing or changing toolchains, open a new PowerShell window and rerun the dream-core install command.
...
When you have two clones of the repository (e.g. `dream-ui` and `dream-ui-refactor`) and need to run both simultaneously, the second instance can be started with:

```bash
bun run start:multi
```

This sets `DREAM_MULTI_INSTANCE=1`, which:

- Skips the Electron single-instance lock
- Uses a separate userData directory (`dream-ui-Dev-2`) to avoid database and config conflicts
- Isolates data/config symlink paths (`~/.dream-ui-dev-2`, `~/.dream-ui-config-dev-2`)
...
dream-ui uses **electron-vite** for fast bundling:
