...
/**
 * Download a release asset by resolving it through the authenticated Releases
 * Assets API (`Accept: application/octet-stream`). Required for private source
 * repos — the plain `releases/download/...` URL only works via a signed-in
 * browser session and returns 404 for anonymous/token-in-URL requests even
 * when a valid GH_TOKEN is supplied as a Bearer header on that URL.
 * Falls back to the plain anonymous download when no token is available
 * (public repos, or local dev without `gh` auth).
 */
function downloadReleaseAsset(assetName, tag, outputPath) {
  ...
  try {
    const release = githubApiGetJson(`repos/${GITHUB_OWNER}/${GITHUB_REPO}/releases/tags/${tag}`);
    ...
    console.warn(`  Authenticated asset download failed (${error.message}), falling back to anonymous download`);
    downloadFile(url, outputPath);
  }
}
...
function prepareAioncore(options) {
  ...
  if (localBinaryPath) {
    ...
    console.log(`  Using locally built binary: ${localBinaryPath}`);
  }
  ...
  if (sourcePath) {
    ...
    const manifest = {
      platform,
      arch,
      version: tag || (actionsRunId ? `actions-run-${actionsRunId}` : 'local-build'),
      generatedAt: new Date().toISOString(),
      sourceType,
      source: sourceDetail,
      files: [binaryName, 'managed-resources/'],
