import { startWebHost, startStaticServer } from '@dream/web-host';
import type { WebHostHandle, StaticServerHandle } from '@dream/web-host';
...
async function runStart(flags: Map<string, string | true>): Promise<void> {
  ...
  if (!backendAvailable) {
    ...
    console.log('');
    console.log('One Work WebUI (frontend only) is ready');
...
Environment variables:
  ...
  DREAM_BACKEND_BIN, DREAM_OPEN_BROWSER
`);
    return;
  }
