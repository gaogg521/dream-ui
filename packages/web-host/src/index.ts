...
export { startStaticServer, stopStaticServer, getLanIP } from './static-server.js';
...
export async function startWebHost(opts: WebHostOptions): Promise<WebHostHandle> {
  ...
  try {
    ...
    staticHandle = await startStaticServer({
      ...
      rendererDevServerUrl: opts.rendererDevServerUrl,
      extraHandlers: opts.extraHandlers,
      sessionGuardedPrefixes: opts.sessionGuardedPrefixes,
