...
// Read the real Dream UI version from the repo-root package.json.
  ...
  return {
    main: {
      plugins: [
        ...
        // '@dream/web-host' excluded so its TS sources (which use ESM ".js" import specifiers)
        // are bundled by esbuild rather than left as `require('@dream/web-host')`, which Node
        // cannot resolve because the package ships no compiled .js files (workspace-only).
        externalizeDepsPlugin({ exclude: ['fix-path', '@dream/web-host'] }),
        ...(isDevelopment
          ? [
              {
      ...
      define: {
        ...
        // Discontinued-build fork flag (see discontinuedBuild.ts). Only Dream UI's
    ...
    renderer: {
      ...
      appType: 'mpa',
      server: {
        // Default to 5173; when occupied (e.g. another Dream UI clone is running),
      ...
      define: {
        'process.env.NODE_ENV': JSON.stringify(mode),
        'process.env.env': JSON.stringify(process.env.env),
        'process.env.DREAM_MULTI_INSTANCE': JSON.stringify(process.env.DREAM_MULTI_INSTANCE ?? ''),
        'process.env.SENTRY_DSN': JSON.stringify(process.env.SENTRY_DSN ?? ''),
        // Inject the real Dream UI version (root package.json) so renderer code
