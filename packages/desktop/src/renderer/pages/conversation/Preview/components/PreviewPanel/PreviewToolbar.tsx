...
import { Close, Save } from '@icon-park/react';
...
interface PreviewToolbarProps {
  ...
  /**
   * 是否显示保存按钮（仅可编辑类型、且内容完整可写回时）。
   * Whether to show the save button (only for editable types whose content is
   * complete enough to be written back).
   */
  showSave?: boolean;

  /**
   * 保存按钮是否可点（当前 tab 有未保存修改时）
   * Whether the save control accepts a click (the active tab has unsaved edits)
   */
    ...
    <div className='flex items-center justify-between h-32px px-10px bg-bg-2 flex-shrink-0 border-b border-border-1 overflow-x-auto'>
      <div className='flex items-center justify-between gap-8px w-full' style={{ minWidth: 'max-content' }}>
        ...
        <div className='flex items-center gap-4px flex-shrink-0'>
          ...
          {/* 保存：可编辑类型才出现，放在这一栏最后、最靠右；没有未保存修改时置灰不可点。
              刻意复用 PreviewContext 的 saveContent（与 Ctrl/Cmd+S 快捷键同一条路径），
              不引入上游那条本仓没有的 refresh 链。
              Save: only for editable types, placed last (rightmost) in this row; greyed
              out and inert when there is nothing unsaved. Deliberately reuses
              PreviewContext's saveContent — the same path as the Ctrl/Cmd+S shortcut —
              instead of pulling in the upstream refresh chain this fork does not have. */}
          {showSave && (
            <Tooltip content={saveActionable ? t('preview.save.tooltip') : t('preview.save.clean')}>
              <div
              ...
              >
                <Save theme='outline' size={toolbarIconSize} fill='currentColor' strokeWidth={3} />
