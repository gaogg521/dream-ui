...
const PreviewPanel: React.FC = () => {
   ...
   * 工具栏保存按钮。
   *
   * 刻意不写成 `() => void saveContent()`：saveContent 的三种失败（写盘返回 false、
   * 缺 file_path/workspace 返回 false、抛错）在那种写法下全部被丢弃，用户点完按钮
   * 什么都不会发生、按钮还停在「有未保存修改」的状态，会以为已经存上了——静默丢编辑。
   * 复用「保存并关闭」那条路径已有的提示方式，保持两个保存入口行为一致。
   *
   * Deliberately not `() => void saveContent()`: that discards all three of saveContent's
   * failure modes, leaving the user with an unchanged dirty button and no message — a silent
   * loss of their edit. Mirrors the handling already used by save-and-close so both save
      ...
      <div className='h-full flex flex-col bg-1'>
        ...
        {content_type !== 'url' && content_type !== 'browser' && !metadata?.missingFile && (
          <PreviewToolbar
            ...
            /*
             * 只有「可编辑类型 + 内容完整（非超限）+ 真的存得下去」才给保存入口。
             *  - metadata.oversized：后端因超出体积上限而拒绝返回完整内容，保存会把残缺内容
             *    写回真实文件；
             *  - file_path / workspace：saveContent 要求两者同时存在才会写盘，缺任一它直接
             *    返回 false —— 少了这一条就会渲染出一个点了什么也不会发生的按钮。
             *
             * Only editable types whose content is complete (not oversized) AND actually
             * writable get a save entry. `oversized` means the backend refused to return the
             * full body past the size ceiling; `file_path` and `workspace` are what
             * saveContent requires before it writes at all, and without them the control
             * would render as a button that silently does nothing.
             */
            showSave={
              isEditable &&
