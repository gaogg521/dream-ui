...
const MessageAcpToolCall: React.FC<{ message: IMessageAcpToolCall }> = ({ message }) => {
  ...
  // Assets announced by the built-in media tool arrive as text in the tool's
  // content items, not through `getAcpImagePath` (that path is Codex's own
  // image plumbing). Videos and multi-image results only exist here.
  const generatedMedia = useMemo(() => {
    const text = (diffContent || [])
      .map((item) => (item.type === 'content' && item.content?.type === 'text' ? item.content.text || '' : ''))
      .filter(Boolean)
      .join('\n');
    // Never double-render what the Codex image path already showed.
    return parseGeneratedMediaAssets(text).filter((asset) => asset.filePath !== imagePath);
  }, [diffContent, imagePath]);
    ...
    <Card className='w-full mb-2' size='small' bordered>
      ...
      <div className='flex items-start gap-3'>
        <div className='flex-1 min-w-0'>
          ...
          <GeneratedMediaView assets={generatedMedia} />
