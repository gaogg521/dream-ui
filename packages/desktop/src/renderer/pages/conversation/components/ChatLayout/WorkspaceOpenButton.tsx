...
const WorkspaceOpenButton: React.FC<WorkspaceOpenButtonProps> = ({ workspacePath, isTemporary }) => {
  ...
  // Determine current tool: explicit user preference > explorer > first available.
  // The button's tooltip and default affordance promise "open the project
  // folder" — falling back straight to `availableOptions[0]` silently defaulted to
  // VS Code whenever it happened to be installed (it's listed first in
  // `toolOptions`), so a developer machine would open a code editor on the very
  // first click, before the user ever chose that tool. Only an explicit prior
  // choice (saved to `preferredTool`) should override the file-manager default.
  // Explorer itself is unavailable for temp workspaces (no real project folder),
  // where Browser is the only option — fall back to whatever *is* available
  // rather than a hardcoded 'explorer' that would open nothing.
  const currentTool: ToolType = useMemo(() => {
    if (preferredTool && availableOptions.some((opt) => opt.key === preferredTool)) {
      return preferredTool;
    }
    if (availableOptions.some((opt) => opt.key === 'explorer')) {
      return 'explorer';
    }
