...
import { resolveBackendLabel } from '@/renderer/utils/model/backendDisplay';
...
const DreamEngineSendBox: React.FC<{
  ...
  useEffect(() => {
    ...
    const processInitialMessage = async () => {
      ...
      try {
        ...
        console.error('[DreamEngineSendBox] Failed to send initial message:', error);
  ...
  const onSendHandler = async (message: string): Promise<void | false> => {
    ...
    if (mediaComposer.mode !== 'off') {
      ...
      // ref points at an entry the media engine knows nothing about. Non-images
      // are dropped with a notice rather than forwarded — the adapters treat
      // every input as an image, so a PDF came back as an opaque provider parse
      ...
      const started = await mediaComposer.submit(message, conversationContext?.workspace, referencePaths);
      if (!started.started) {
  ...
  const handleSheetModeChange = useCallback(
    async (mode: string) => {
      ...
      try {
        ...
        console.error('[DreamEngineSendBox] Failed to switch mode via sheet:', error);
      ...
      modelSelection.getAvailableModels(provider).map((modelName) => ({
        ...
        label: (
          <span className='flex items-center gap-6px min-w-0'>
            ...
            <ModelKindTag {...modelKindLabelOf(provider, modelName)} />
          </span>
        ),
  ...
  const handleStop = async (): Promise<void> => {
    ...
    try {
      ...
      console.warn('[DreamEngineSendBox] stop request failed', error);
...
export default DreamEngineSendBox;
