...
import { useClaudeBridgeEnabled } from '@/renderer/hooks/agent/useClaudeBridgeStatus';
import { useCodexBridgeEnabled } from '@/renderer/hooks/agent/useCodexBridgeStatus';
...
const AcpSendBox: React.FC<{
  ...
  /** Where generated media is written; media mode needs it. */
  workspace?: string;
  backend: string;
  session_mode?: string;
  agent_name?: string;
  ...
  const executeCommand = useCallback(
    async ({ input, files }: Pick<ConversationCommandQueueItem, 'input' | 'files'>) => {
      ...
      // [[DREAM_FILES]] marker at the send edge (no front-end path/marker building).
      try {
...
Please check your local CLI tool authentication status`,
  ...
  const onSendHandler = async (message: string): Promise<void | false> => {
    ...
    // conversation about one. The job still belongs to this conversation and
    // renders in it, so switching back to chat is just talking again.
    if (mediaComposer.mode !== 'off') {
      if (mediaComposer.needsModel) {
        Message.warning(t('conversation.mediaModeNoModel'));
        return;
      }
      // Only files with a real path can be a reference image. A project-scoped
      // ref is relative to a project entry the media engine knows nothing
      // about, so passing it would resolve to the wrong file or none at all.
      // Non-images are dropped with a notice: every adapter treats an input as
      // an image, so a PDF surfaced as an opaque provider parse error.
      const { images: referencePaths, rejected } = splitReferenceInputs(
        allFiles
          .filter((file): file is Extract<ChatFileRef, { path: string }> => 'path' in file)
          .map((file) => file.path)
      );
      if (rejected.length) {
      ...
      const started = await mediaComposer.submit(message, workspace, referencePaths);
      if (!started.started) {
  ...
  return (
    <div className={`${sendBoxWidthClass} flex flex-col mt-auto mb-16px`}>
      ...
      <SendBox
        ...
        disabled={false}
        placeholder={
          // In media mode the input no longer sends a message, so it must stop
          // reading like a chat box — this line is what the user is looking at
          // the instant before they hit Enter.
          mediaComposer.mode !== 'off'
            ? t(
                mediaComposer.mode === 'video'
                  ? mediaComposer.supportsReference
                    ? 'conversation.mediaModeVideoHintRef'
                    : 'conversation.mediaModeVideoHint'
                  : mediaComposer.supportsReference
