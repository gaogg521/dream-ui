 ...
 * with the `id` the backend later carries on `confirmation.remove` so the
 * waiting state clears against the right pending request:
 *  - `ask`            -> `request_id`
 ...
 * concrete `confirmation.remove` invalidates this coarse guess.
 */
...
/**
 * Pure decision helper mirroring the generating reconcile: a runtime summary
 * lights the sidebar "waiting" icon only when it reports pending confirmations.
 * Clearing is never done via reconcile.
 */
 ...
 * per-conversation hydrate. Mirrors `reconcileGeneratingFromRuntime`: it only
 * ever marks (using a sentinel id, as the runtime summary reports a count, not
 * ids) so a stale idle summary cannot fight a live stream; clearing stays with
 * `confirmation.remove` and terminal frames. Covers the window-reload case where
 * the marking stream frame was missed.
 */
...
const initializeConversationListSyncStore = () => {
  ...
  ipcBridge.conversation.responseStream.on((message) => {
    ...
    // the distinct "waiting" icon. Independent of the generating guard: an `ask`
    // frame is not in the generating whitelist, and waiting takes display
    // precedence over the spinner regardless.
