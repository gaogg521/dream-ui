...
import { DreamSearchInput, DreamInlineSearchInput } from '@renderer/components/base';
import type { MarketplacePersona } from '@/common/types/agent/assistantTypes';
import {
  AssistantOptionLabel,
  assistantKey,
  mergeWithMarketplaceMatches,
...
type Props = {
  ...
  /**
   * 搜索框样式：'default' 用常驻搜索栏样式（DreamSearchInput，34px 描边）；
   * 'inline' 用下拉列表专用的柔和样式（DreamInlineSearchInput，浅灰填充无边框）。
   * 点击展开的下拉列表传 'inline'，常驻列表用默认。
   */
  searchVariant?: 'default' | 'inline';
  /** Full marketplace catalog — only consulted once the user searches, so a
   * not-yet-installed expert can still be found (and one-click installed). */
  ...
  /** Fires whenever the search query or the resulting row count changes — lets a caller
   * with its own "All assistants (N)" header (e.g. `TeamCreateModal`) reflect that a search
   * can surface more rows than the static installed-count implies (marketplace matches). */
  onResultsChange?: (info: { query: string; count: number }) => void;
};
...
const TeamAssistantPicker: React.FC<Props> = ({
  ...
  const filteredAssistants = useMemo(() => {
    ...
    const installedMatches = q
      ? assistants.filter((assistant) => assistant.name.toLowerCase().includes(q))
      : assistants;
    return mergeWithMarketplaceMatches(installedMatches, marketplacePersonas, query);
  }, [assistants, marketplacePersonas, query]);

  useEffect(() => {
    onResultsChange?.({ query, count: filteredAssistants.length });
  }, [query, filteredAssistants.length, onResultsChange]);
        ...
        {filteredAssistants.length === 0 ? (
          ...
          <div className={isModalDensity ? 'flex flex-col gap-6px' : 'flex flex-col gap-6px'}>
            {filteredAssistants.map((assistant, index) => {
              ...
              const notInstalled = assistant.installed === false;
              const addIcon = (
                <span
                  ...
                  title={
                    notInstalled
                      ? t('team.create.marketplaceInstallHint', {
                          defaultValue: 'Not added yet — clicking will install it to My Assistants and add it here',
                        })
                      : undefined
                  }
                >
              ...
              const row = blockReason ? (
                ...
                <Button
                  ...
                  className={rowClassName}
                  onClick={() => (notInstalled ? onInstallAndSelect?.(assistant.id) : onSelect(assistant))}
                  data-testid={`${testIdPrefix}-option-${assistantKey(assistant)}`}
                >
                  <div className={rowContentClassName} data-testid='team-assistant-picker-row-content'>
                    <div
