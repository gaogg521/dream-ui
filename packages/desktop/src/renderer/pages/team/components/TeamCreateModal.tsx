...
import { assistantToOption, type TeamAssistantOption } from './assistantSelectUtils';
...
// 注意：迁移到 DreamModal variant='standard' 后，关闭按钮为 button[aria-label="Close"]，
...
const TeamCreateModal: React.FC<Props> = ({ visible, onClose, onCreated }) => {
  ...
  // Reflects what `TeamAssistantPicker`'s own search box is currently showing — a search can
  // surface marketplace matches beyond the installed set, so the "All assistants (N)" header
  // must not keep quoting the static installed count while a query is active.
  const [assistantSearchInfo, setAssistantSearchInfo] = useState<{ query: string; count: number }>({
    query: '',
    count: 0,
  });
  const handleAssistantResultsChange = useCallback((info: { query: string; count: number }) => {
    setAssistantSearchInfo(info);
  }, []);
  ...
  const handleInstallAndSelectAssistant = useCallback(
    async (assistantId: string) => {
      if (pendingAssistantId) return;
      setPendingAssistantId(assistantId);
      try {
        const installed = await installMarketplacePersona(assistantId);
        void refreshManagedAgentCatalogAndAssistants();
        handleSelectAssistant(assistantToOption(installed, localeKey));
      } catch (error) {
    ...
    [pendingAssistantId, installMarketplacePersona, localeKey, t]
  );
    ...
    <>
      {allAssistants.length === 0 ? (
        ...
        <TeamAssistantPicker
          ...
          onInstallAndSelect={handleInstallAndSelectAssistant}
          pendingAssistantId={pendingAssistantId}
          onResultsChange={handleAssistantResultsChange}
        />
      )}
    </>
  );
    ...
    >
      ...
      >
        <div className='mb-12px text-15px font-600 leading-22px text-t-secondary'>
          {assistantSearchInfo.query.trim()
            ? t('team.create.searchResultsWithCount', {
                count: assistantSearchInfo.count,
                defaultValue: `${assistantSearchInfo.count} results`,
              })
  ...
  const handleInstallAndSelectFromDropdown = (assistantId: string) => {
    void handleInstallAndSelectAssistant(assistantId);
    setAssistantDropdownOpen(false);
  };
  const addMemberDropdown = (
    <TeamAssistantPickerDropdown
      ...
      marketplacePersonas={marketplacePersonas}
      onInstallAndSelect={handleInstallAndSelectFromDropdown}
      pendingAssistantId={pendingAssistantId}
    >
      <Button
        type='outline'
