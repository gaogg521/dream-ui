...
import BackendSelectionArea from './components/BackendSelectionArea';
...
const GuidPage: React.FC = () => {
  ...
  const modelSelection = useGuidModelSelection('dream');
  ...
  // Bare CLIs now live only in the backend switcher above, not mixed into
  // the persona pill list.
  const personaAssistants = useMemo(
    () => agentSelection.assistants.filter((assistant) => assistant.source !== 'generated'),
  ...
  // Media generation from the welcome page: there is no conversation yet, so
  // the composer runs unbound and `handleSend` passes the freshly created id to
  // `submit`. Starting a job with no conversation would attach it to a
  // workspace instead, which is exactly the "it ended up somewhere else"
  // failure the job-belongs-to-a-conversation rule exists to prevent.
  const { data: mediaProviders } = useProvidersQuery();
  const mediaComposer = useMediaComposer(undefined, mediaProviders);
  ...
  const send = useGuidSend({
    ...
    selectedBackendAvailable: agentSelection.selectedBackendAvailable,
  ...
  const handleInputKeyDown = useCallback(
    (event: React.KeyboardEvent) => {
      ...
      if (event.key === 'Enter' && !event.shiftKey) {
        ...
        if (!guidInput.input.trim() && guidInput.files.length === 0) return;
        send.sendMessageHandler();
      }
    },
    [guidInput.files.length, guidInput.input, send.sendMessageHandler, slashController]
  );
  ...
  // "Summon more experts" in the "+" menu's expert picker — jump straight to
  // the marketplace tab rather than the assistants page's default "Mine" tab.
  const handleBrowseMoreExperts = useCallback(() => {
    navigate('/settings/assistants', { state: { initialTab: 'marketplace' } });
  }, [navigate]);

  // The "+" menu's expert picker searches the full marketplace catalog (not
  // just already-installed assistants) — see GuidExpertPickerGrid for why.
  const { personas: marketplacePersonas, install: installMarketplacePersona } = useMarketplacePersonas();

  const handleInstallAndSelectPersona = useCallback(
    (assistantId: string) => {
      void installMarketplacePersona(assistantId)
        .then(() => refreshManagedAgentCatalogAndAssistants())
        .then(() => {
          agentSelection.setSelectedAssistantId(assistantId);
        })
        .catch((error) => {
          console.error('[Guid] Failed to install marketplace persona from expert picker:', error);
        });
    },
    [installMarketplacePersona, agentSelection]
  );

  // A bare-CLI assistant (source === 'generated') is the "no persona
  // layered on top" state — only show the selected-persona chip and expose
  // "clear" when something more specific than that is actually selected.
  const selectedPersona =
    agentSelection.selectedAssistant && agentSelection.selectedAssistant.source !== 'generated'
      ? agentSelection.selectedAssistant
      : null;

  const handleClearPersona = useCallback(() => {
    const bareCliAssistant = agentSelection.assistants.find(
      (assistant) => assistant.source === 'generated' && assistant.agent_id === agentSelection.selectedBackendAgentId
    );
    if (bareCliAssistant) {
      agentSelection.setSelectedAssistantId(bareCliAssistant.id);
    }
  }, [agentSelection]);

  const handleSelectBackend = useCallback(
    (agentId: string) => {
      agentSelection.setSelectedBackendAgentId(agentId);
      // No persona selected (bare-CLI mode): switching backends here used to
      // mean switching *which* bare CLI is selected outright, before personas
      // and backends were split into independent controls. Keep that — the
      // selected assistant's identity (name/avatar) should stay consistent
      // with the backend it's actually running on when there's no persona
      // layered on top of it.
      if (!agentSelection.selectedAssistant || agentSelection.selectedAssistant.source === 'generated') {
        const bareCliAssistant = agentSelection.assistants.find(
          (assistant) => assistant.source === 'generated' && assistant.agent_id === agentId
        );
        if (bareCliAssistant) {
          agentSelection.setSelectedAssistantId(bareCliAssistant.id);
        }
  ...
  useEffect(() => {
    ...
    const signature = JSON.stringify({
      ...
      availableModels: {
        ...
        dream: modelSelection.modelList.map((provider) => ({
          id: provider.id,
    ...
    const applyAssistantDefaults = async () => {
      ...
      if (shouldApplyDefaultModel && effectiveBackend === 'dream') {
  ...
  const PROVIDER_BASED_AGENTS = new Set(['dream']);
  ...
  const actionRowNode = (
    <GuidActionRow
      ...
      /* Lets the row drop the chat model selector while generating — see
         `showModelSelector` there for why the two must not show together. */
      mediaActive={mediaComposer.mode !== 'off'}
      referenceOnly={mediaComposer.mode !== 'off'}
      mediaControlNode={
        <MediaModeControl
          mode={mediaComposer.mode}
      ...
      personaAssistants={personaAssistants}
      marketplacePersonas={marketplacePersonas}
      selectedAssistantId={agentSelection.selectedAssistantId}
      selectedPersona={selectedPersona}
      localeKey={localeKey}
      onSelectAssistant={handleSelectAssistant}
      onInstallAndSelectPersona={handleInstallAndSelectPersona}
      onBrowseMoreExperts={handleBrowseMoreExperts}
