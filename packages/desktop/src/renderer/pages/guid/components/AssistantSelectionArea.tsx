...
const AssistantSelectionArea: React.FC<AssistantSelectionAreaProps> = ({
  ...
  const renderAssistantPill = (assistant: Assistant, testId: string, fullWidth = false) => {
    ...
    return (
      <Button
        ...
        style={isSelected ? { background: 'rgb(var(--success-6))', color: '#fff' } : { background: 'transparent' }}
        onClick={() => {
  ...
  const overflowPanel = (
    <div
      ...
      className={`absolute start-0 top-[calc(100%+8px)] z-100 w-full rounded-12px border border-2 p-8px shadow-lg ${styles.assistantOverflowPanel}`}
