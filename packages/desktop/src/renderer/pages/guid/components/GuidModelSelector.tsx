...
import { modelKindLabelByName, modelKindLabelOf } from '@/common/utils/modelCapabilities';
...
type GuidModelSelectorProps = {
  ...
  /** Currently selected agent tab's backend (e.g. 'codex', 'claude') — used
   * to lock this pre-conversation picker the same way `AcpModelSelector`
   * locks the in-conversation one when the matching bridge is enabled. */
  backend?: string;
};
...
const GuidModelSelector: React.FC<GuidModelSelectorProps> = ({
  ...
  const bridgeLockedLabelKey = backend === 'codex' ? 'agent.model.codexBridgeLocked' : 'agent.model.claudeBridgeLocked';
  const bridgeLockedTooltipKey =
    backend === 'codex' ? 'agent.model.codexBridgeLockedTooltip' : 'agent.model.claudeBridgeLockedTooltip';
  // Same staleness fix as AcpModelSelector: prefer the bridge's own
  // configured model over the ACP session's snapshot-at-spawn advertised name.
  ...
  // The ACP list comes from the CLI session, not from a provider, so the kind
  // can only be recovered by matching the model name against what the user
  // declared elsewhere — and only when that match is unambiguous.
  const acpModelsWithKind = React.useMemo(
    () =>
      (currentAcpCachedModelInfo?.available_models ?? []).map((model) => ({
        ...model,
        ...modelKindLabelByName(enabledModelList, model.id),
      })),
    [currentAcpCachedModelInfo?.available_models, enabledModelList]
  );
  ...
  // Same staleness fix as AcpModelSelector: the ACP session's advertised model
  // name is a snapshot from spawn time and can lag the bridge's actual
  // (currently saved) model, so prefer the bridge's own config for the model
  // portion when locked while still composing with the live thought level.
  const lockedCombinedAcpButtonLabel = bridgeConfiguredModel
    ? composeRuntimeSelectorLabel({ modelLabel: bridgeConfiguredModel, thoughtLevel: normalizedThoughtLevelOption })
      ...
      return (
        <Tooltip content={t(bridgeLockedTooltipKey)} position='top'>
          <Button
            ...
            data-testid='guid-model-selector-bridge-locked'
          >
            <span className='flex items-center gap-6px min-w-0'>
              <Brain theme='outline' size='14' fill={iconColors.secondary} className='shrink-0' />
              <span className='guid-model-label'>
                {lockedCombinedAcpButtonLabel} · {t(bridgeLockedLabelKey)}
              </span>
            </span>
          </Button>
        </Tooltip>
