...
import { isMutableAssistantSource } from '@/common/types/agent/assistantTypes';
...
export const filterAssistants = (
  ...
  return assistants.filter((assistant) => {
    ...
    switch (filter) {
      ...
      case 'user':
        return isMutableAssistantSource(assistant.source);
      case 'all':
      default:
        return true;
    }
...
export const groupMyAssistants = (assistants: AssistantListItem[]) => {
  return {
    ...
    // 'user' == hand-authored; 'imported' == persona-library import. Both are
    // freely mutable/deletable, so both belong in the same "created" group.
    createdAssistants: assistants.filter((a) => isMutableAssistantSource(a.source)).toSorted(byAssistantSortOrder),
  };
};
