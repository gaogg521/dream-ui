...
const MyAssistantCard: React.FC<MyAssistantCardProps> = ({
  ...
  // Fork: marketplace-installed personas carry source 'imported', which is just
  // as user-owned as 'user' — gate on the shared predicate so they stay
  // deletable (upstream only knows about 'user').
  const canDelete = isMutableAssistantSource(assistant.source);
