...
const SectionCard: React.FC<{
  ...
  <section data-testid={dataTestId} className='rounded-16px border border-2 bg-base px-20px py-18px'>
...
const SkillDetailPage: React.FC = () => {
  ...
  const skillsTab = (location.state as SkillDetailLocationState | null)?.skillsTab;

  const goBack = useCallback(() => {
    void navigate(skillsRoutes.listPath, { state: { skillsTab } });
  }, [navigate, skillsRoutes.listPath, skillsTab]);

  const editViaChat = useCallback(
    (skillNameForPrompt: string) => {
      void talkToButler({
        prompt: `I'd like to improve this Skill: ${skillNameForPrompt}\n\nPlease review its content and help me make improvements. My suggestions are:`,
      });
    },
    [talkToButler]
  );
  ...
  const sourceLabel = (s: SkillInfo): string => {
    if (s.source === 'custom') return t('settings.skillsHub.custom', { defaultValue: 'Custom' });
    if (s.source === 'team') {
      return s.is_auto_inject
        ? t('settings.skillsHub.teamAuto', { defaultValue: 'Team · Auto' })
    ...
    return t('settings.skillsHub.builtin', { defaultValue: 'Built-in' });
  };
    ...
    <SettingsPageWrapper>
      <div data-testid='skill-detail-page' className='flex flex-col gap-16px'>
        <div className='flex items-center gap-10px'>
          ...
          {/* The skill name itself is already shown once, prominently, in the
              info card below — repeating it here would just be noise. */}
          <div className='truncate text-14px font-600 text-t-primary'>
            {t('settings.skillsHub.detailTitle', { defaultValue: 'Skill Details' })}
          </div>
        </div>
        ...
        {loading ? (
          ...
          <div
            ...
            className='rounded-12px border border-dashed border-1 bg-fill-1 px-16px py-40px text-center text-13px text-t-tertiary'
          >
          ...
          <div className='mx-auto flex w-full max-w-760px flex-col gap-16px'>
              ...
              <div className='flex gap-16px'>
                ...
                <div className='min-w-0 flex flex-col gap-6px'>
                  <div className='flex items-center gap-8px'>
                    ...
                    <span className='rounded-4px border border-2 bg-fill-1 px-6px py-1px text-11px text-t-secondary'>
                      {sourceLabel(skill)}
                    </span>
                  </div>
            ...
            {/* Files */}
            <SectionCard
              title={t('settings.skillsHub.detailFilesTitle', { defaultValue: 'Files' })}
              ...
              extra={
                <Button
                  ...
                  onClick={() => editViaChat(skill.name)}
                >
                  {t('settings.skillsHub.detailEditViaChat', { defaultValue: 'Edit via chat' })}
                </Button>
              }
            >
              <SkillFileBrowser skill={{ location: skill.location }} />
            </SectionCard>
                  ...
                  {usingAssistants.map((assistant) => {
                    ...
                    return (
                      ...
                      >
                        ...
                        {isReadonly ? (
                          <span className='rounded-4px border border-2 bg-fill-1 px-6px py-1px text-11px text-t-tertiary'>
