...
const LOCAL_AGENT_SETUP_GUIDE_URL = 'https://1one.1oneclaw.com';
...
const LocalAgents: React.FC = () => {
  ...
  const { agents: allAgents, isRefreshing, refreshCatalog, error: agentsError } = useManagedAgents();
  const lastAgentsErrorRef = useRef<unknown>(null);

  useEffect(() => {
    if (!agentsError || agentsError === lastAgentsErrorRef.current) {
      return;
    }
    lastAgentsErrorRef.current = agentsError;
    Message.error(parseError(agentsError));
  }, [agentsError]);
