 ...
 * Only show ones whose backing CLI is actually installed on this machine.
 *
 * "Installed" means the agent probe resolved the binary — i.e. status is
 * `online` (installed and the ACP handshake succeeded) OR `offline`
 * (installed, but the last handshake failed, e.g. the CLI needs the user to
 * log in first — Cursor's `agent`). Both are worth surfacing so the user can
 ...
 */
export const isInstalledGeneratedCliAssistant = (assistant: Assistant): boolean => {
...
export const selectableAssistants = (assistants: Assistant[], preferredOrder?: readonly string[]): Assistant[] => {
  const legacyOrdered = assistants
    ...
    // Fork: hide auto-generated CLI assistants whose backing agent isn't
    // installed/online — they can't actually run, so they shouldn't be
    // selectable. Upstream's compareLegacyAssistantOrder supersedes the inline
    // sort this used to carry (same group/sort_order rule, plus an id tiebreak).
