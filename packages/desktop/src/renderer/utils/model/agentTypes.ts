...
export const FULL_AUTO_MODE_VALUES = ['yolo', 'bypassPermissions', 'yoloNoSandbox'] as const;

// Given the mode options an agent actually advertises, return the value of
// its fully-automatic mode if it offers one, else undefined. Lets new
// conversations default to full-auto across both backends without
// hardcoding a single backend's mode string.
export function pickFullAutoMode(options: readonly AgentModeOption[] | undefined): string | undefined {
  if (!options || options.length === 0) return undefined;
  for (const candidate of FULL_AUTO_MODE_VALUES) {
...
export async function fetchManagedAgents(): Promise<ManagedAgent[]> {
  ...
  if (!Array.isArray(agents)) {
    throw new Error('Managed agents response is not an array');
  }
