        ...
        if (result.success) {
          ...
          await updateServerStatus('connected', {
            tools: result.tools?.map((tool) => ({
              ...
              ...(tool.incompatibilities?.length ? { incompatibilities: tool.incompatibilities } : {}),
          ...
          // rejects. Reporting plain "success" here is what let a broken
          // schema reach the provider and fail every later message with an
          // opaque 400 — so surface the offending tool and key by name.
          const rejected = (result.tools ?? []).filter((tool) => tool.incompatibilities?.length);
            ...
            await globalMessageQueue.add(() => {
              try {
                if (rejected.length > 0) {
                  ...
                  const firstKey = first.incompatibilities?.[0]?.key ?? '';
                  message.warning(
                    `${server.name}: ${t('settings.mcpIncompatibleToolsWarning', {
