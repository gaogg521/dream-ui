...
const setDraft = <K extends DraftConversationType>(
  ...
  switch (type) {
    ...
    case 'dream':
      if (draft) {
        store.dream.set(conversation_id, draft as Extract<Draft, { _type: 'dream' }>);
      } else {
        store.dream.delete(conversation_id);
      }
      break;
    default:
      break;
...
const getDraft = <K extends DraftConversationType>(
  ...
  switch (type) {
    ...
    case 'dream':
      return store.dream.get(conversation_id) as Extract<Draft, { _type: K }>;
    default:
      return undefined;
  }
};
