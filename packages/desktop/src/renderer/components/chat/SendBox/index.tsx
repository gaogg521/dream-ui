...
const SendBox: React.FC<{
   ...
   *
   * For send boxes whose Enter does not send a message — media generation
   * mode — where those affordances do not exist and naming them contradicts
   * the placeholder right next to them.
   */
  hideChatHint?: boolean;
  /**
  ...
  const hasSendableContent = hasDraftToSend || hasPendingAttachments;
  ...
  const isSendActionDisabled = disabled || sendDisabled || isUploading || !hasSendableContent;
        ...
        >
          ...
          >
            ...
            <Input.TextArea
              ...
              value={input}
              placeholder={(() => {
                // The trailing hint advertises chat affordances — slash
                // commands and message history. A send box that is not sending
                // a message has neither, so it must not advertise them.
                const hint = hideChatHint
                  ? undefined
                  : ((bottomHint as string | undefined) ??
                    t('conversation.sendbox.hint', { defaultValue: 'Type / for commands, @ to reference files' }));
                if (isMobileCompact) return placeholder ?? hint;
                if (placeholder) return hint ? `${placeholder}  ${hint}` : placeholder;
                return hint;
              })()}
