      ...
      >
        ...
        >
          {releaseNotes ? (
            ...
            // No notes published for this version (or the fetch failed). Either
            // way the panel must say so and offer a way out — it used to sit on
            // "更新内容获取中" forever, or render the dangling "获取失败，可"
            // with no link when the release URL was missing.
            <div className='flex items-center gap-6px'>
              <span>
                {state.releaseNotesStatus === 'failed' ? t('update.releaseNotesFailed') : t('update.noReleaseNotes')}
              </span>
              <button
                type='button'
