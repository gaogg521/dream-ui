...
export const UPDATE_AVAILABLE_EVENT = 'one-update-available';
...
export const useUpdateNotificationController = () => {
  ...
  useEffect(() => {
    ...
    window.addEventListener('one-open-update-modal', handleWindowOpen);
    ...
    return () => {
      removeOpenListener();
      window.removeEventListener('one-open-update-modal', handleWindowOpen);
  ...
  const openReleasePage = useCallback(() => {
    // Fall back to the public landing page: a "前往查看" link that silently does
    // nothing is worse than one that lands on the downloads page.
    const target = state.releasePageUrl || RELEASES_PAGE_URL;
