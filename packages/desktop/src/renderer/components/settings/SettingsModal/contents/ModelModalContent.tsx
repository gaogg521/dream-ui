...
import { consumeModelSettingsHighlight } from '@/renderer/hooks/media/mediaSettingsHighlight';
...
const ModelModalContent: React.FC = () => {
  ...
  /**
   * Promote every inferred kind on this provider into a real declaration.
   *
   * Deliberately writes only `model_kind`, leaving any other per-model setting
   * untouched — this is the user agreeing with a reading of the name, not a
   * reset of what they configured. Each one stays individually editable
   * afterwards, which is what makes accepting them in bulk safe.
   */
  const acceptInferredKinds = (platform: IProvider) => {
    ...
    updatePlatform({ ...platform, model_settings }, () => {
      Message.success(t('settings.acceptInferredKindsDone', { count: entries.length }));
    });
  };
  ...
  // A failed media job card can send the user here to fix a bad
  // media_endpoint choice — expand and scroll to that provider once its data
  // has loaded (see mediaSettingsHighlight.ts).
  useEffect(() => {
    if (!data) return;
    const providerId = consumeModelSettingsHighlight();
    if (!providerId || !data.some((item) => item.id === providerId)) return;
    setCollapseKey((prev) => ({ ...prev, [providerId]: true }));
    requestAnimationFrame(() => {
      document.getElementById(`provider-card-${providerId}`)?.scrollIntoView({ behavior: 'smooth', block: 'center' });
    });
  }, [data]);
  ...
  return (
    ...
    >
      ...
      {/* Content Area */}
      <DreamScrollArea className='flex-1 min-h-0' disableOverflow={isPageMode}>
        {!data || data.length === 0 ? (
          ...
          <div className='space-y-16px'>
            {(data || []).map((platform: IProvider) => {
              ...
              return (
                <div key={key} id={`provider-card-${platform.id}`}>
                  <Collapse
                  ...
                  >
                      ...
                      header={
                        <div className='group flex items-center justify-between w-full min-h-32px gap-8px min-w-0'>
                          ...
                          >
                            ...
                            />
                            {/* A company-provisioned channel is not this member's
                              to edit: the sync would restore whatever they
                              changed, so offering the buttons would only
                              produce edits that silently revert. The backend
                              refuses these writes too — this just stops the
                              user from being invited to make them.
                              ⚠️ Not rendered at all rather than hidden with a
                              class: `hidden` next to `flex` does nothing, because
                              both are single-class specificity and UnoCSS emits
                              `.flex{display:flex}` after `.hidden{display:none}`.
                              Real-machine CDP caught that — the class was on the
                              element and `display` still computed to `flex`. */}
                            {platform.managed_by !== 'enterprise' && (
                              <div className='flex items-center gap-4px'>
                                {/* Only offered when this provider actually has
                                guesses to settle — a permanently visible button
                                for a list with nothing inferred is a puzzle. */}
                                {inferredModelKinds(platform).length > 0 && (
                                  <Popconfirm
                                    title={t('settings.acceptInferredKindsConfirm', {
                                      count: inferredModelKinds(platform).length,
                                    })}
                                    onOk={() => acceptInferredKinds(platform)}
                                  >
                                    <Tooltip content={t('settings.acceptInferredKinds')}>
                                      <Button
                                        size='mini'
                                        className='model-provider-action-btn !w-28px !h-28px !min-w-28px text-t-secondary hover:text-t-primary'
                                        aria-label={t('settings.acceptInferredKinds')}
                                        data-testid='accept-inferred-kinds'
                    ...
                    >
                      {(platform.models ?? []).map((model: string, index: number, arr: string[]) => {
                        ...
                        const modelKindLabel = modelKindLabelOf(platform, model);
                        ...
                        // Health check probes via a chat completion. Form A/C image and
                        // video models have no chat endpoint at all — probing them always
                        // 404s even though the dedicated generation endpoint works fine.
                        const canHealthCheck = isChatCapableModel(platform, model);
                          ...
                          <div key={model}>
                            <div className='flex items-center justify-between gap-8px px-8px py-12px transition-colors hover:bg-[var(--fill-0)]'>
                              <div className='flex min-w-0 flex-1 items-center gap-8px'>
                                ...
                                {healthStatus !== 'unknown' && canHealthCheck && (
                                  <Tooltip
                                    content={
                                      <div>
                                        <div className='flex items-center gap-4px'>
                                ...
                                {/* Declaration → catalog → a reading of the name. The last of
                                  those renders muted with a "?" so a guess never passes for
                                  a fact, while still leaving the list legible at a glance. */}
                                {/* Clickable here, and only here: this is the one
                                  screen where the kind can actually be changed,
                                  so the tag doubles as the way in. That is what
                                  lets the tag elsewhere state a reading plainly
                                  instead of hedging with a "?". */}
                                <ModelKindTag
                                  {...modelKindLabel}
                                  showUndeclared
                                ...
                                <Switch
                                  className='shrink-0 settings-switch-on'
                                  size='small'
                              ...
                              <div className='flex items-center gap-6px shrink-0'>
                                ...
                                {/* 心跳检测按钮 / Health check button — only for chat-capable
                                    models; Form A/C image and video models have no chat
                                    endpoint to probe. */}
                                {canHealthCheck && (
