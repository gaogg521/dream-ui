...
export const SubModal: React.FC<SubModalProps> = ({ visible, onCancel, title, children }) => {
  return (
    <DreamModal
      ...
      title={title}
    >
      <DreamScrollArea className='h-full px-20px pb-16px text-14px text-t-primary'>{children}</DreamScrollArea>
    </DreamModal>
  );
};
...
const SettingsModal: React.FC<SettingsModalProps> = ({ visible, onCancel, defaultTab = 'model' }) => {
  ...
  const desktopMenu = (
    <DreamScrollArea className='flex-shrink-0 b-color-border-2 scrollbar-hide' style={{ width: `${SIDEBAR_WIDTH}px` }}>
