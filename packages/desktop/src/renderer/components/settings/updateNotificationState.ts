    ...
    case 'checkAvailable':
      return {
        state: {
          ...
          pendingInstallerLastFailure: undefined,
          presentation: 'card',
          // The check itself succeeded — whether it carried notes is a separate
          // question, answered by the (possibly empty) body. Reporting "failed"
          // for a release that simply has no changelog told the user their
          // network was broken when nothing was.
    ...
    case 'manualReleaseInfoLoaded':
      return {
        state: {
          ...
          updateInfo: event.updateInfo,
          releasePageUrl: event.releasePageUrl,
          // Loaded means the release info arrived; an empty body means the
          // release has no changelog, not that fetching it failed.
          releaseNotesStatus: 'loaded',
        },
        effects: [],
      };
