...
const AIONUI_DOWNLOAD_URL = 'https://work.1oneclaw.com/';
...
export const InstallationIntegrityFooter: React.FC<{
  ...
  diagnosticsKind?: InstallationIntegrityDialogKind;
  onDismiss?: () => void;
}> = ({ diagnostics, diagnosticsKind = 'incomplete_installation', onDismiss }) => {
  ...
  const handleRecoverCorruptedDatabase = () => {
    ...
    Modal.confirm({
      title: dialogKindText(t, diagnosticsKind, 'confirmDialog.title'),
      content: dialogKindText(t, diagnosticsKind, 'confirmDialog.content'),
      okText: dialogKindText(t, diagnosticsKind, 'confirmDialog.okText'),
      cancelText: dialogKindText(t, diagnosticsKind, 'confirmDialog.cancelText'),
  ...
  return (
    <Space>
      {onDismiss ? (
        <Button data-testid='installation-integrity-dismiss' onClick={onDismiss}>
          {t('common.backendStartup.incompleteInstallation.continueWithoutComponent')}
        </Button>
      ) : null}
      {actions.reportText ? (
        <Button
...
export function showInstallationIntegrityModal(
  ...
  // Dismissibility follows what is actually broken, not what the message says.
  // Backend startup failed => there is nothing behind this dialog, so keeping it
  // modal is honest. A single runtime component failed => the rest of the app
  // still works (other backends, other models), and trapping the user in a modal
  // turns "Claude is unavailable" into "the app is unusable".
  //
  // Esc always closed this dialog (Arco's escToExit defaults to true), so the
  // fix is a visible way out, not a new one. The reconciler also closes this
  // programmatically via the returned handle when the resource self-heals.
  const dismissible = options.dismissible === true;
  let handle: ReturnType<InstallationIntegrityModalController['error']>;
  ...
  handle = modal.error({
    ...
    footer: (
      <InstallationIntegrityFooter
        ...
        onDismiss={dismissible ? () => handle.close() : undefined}
      />
    ),
