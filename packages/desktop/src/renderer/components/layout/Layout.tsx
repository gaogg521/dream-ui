...
// Desktop free-resize bounds: drag the handle to set any width in [min, max];
// dragging narrower than the snap threshold collapses the sider instead.
const DESKTOP_SIDER_MIN_WIDTH = 200;
const DESKTOP_SIDER_MAX_WIDTH = 480;
const SIDER_DRAG_SNAP_THRESHOLD = Math.round(DESKTOP_SIDER_MIN_WIDTH / 2);
const SIDER_DRAG_HYSTERESIS = 6;
const SIDER_WIDTH_STORAGE_KEY = 'aionui:desktopSiderWidth';
const MOBILE_SIDER_WIDTH_RATIO = 0.67;
const MOBILE_SIDER_MIN_WIDTH = 260;
const MOBILE_SIDER_MAX_WIDTH = 420;

// Read the persisted desktop sider width, clamped to the allowed range.
const readStoredSiderWidth = (): number => {
  ...
  try {
    ...
    if (Number.isFinite(parsed)) {
      return Math.max(DESKTOP_SIDER_MIN_WIDTH, Math.min(DESKTOP_SIDER_MAX_WIDTH, parsed));
    }
  } catch {
...
const Layout: React.FC<{
  ...
  // Desktop sider width, freely draggable and persisted across sessions.
  ...
  useGovernanceEndpointSync();
  // Report this machine into the enterprise runtime-node roster. No-op in
  // standalone mode (the hook gates on a resolved tenant).
  useRuntimeNodeHeartbeat();
  ...
  // The "Dream UI" wordmark acts as Home / Back-to-Chat, but only from settings routes.
  ...
  useEffect(() => {
    const handleMouseMove = (event: MouseEvent) => {
      ...
      // Below the snap threshold the sider collapses; above it, the drag freely
      // sets the width within [min, max]. Hysteresis avoids toggling jitter
      // right at the collapse boundary.
      ...
      if (!shouldCollapse) {
        const clamped = Math.max(DESKTOP_SIDER_MIN_WIDTH, Math.min(DESKTOP_SIDER_MAX_WIDTH, draggedWidth));
    ...
    const endDrag = () => {
      ...
      // Persist the final width so it survives reloads/restarts.
      try {
        window.localStorage.setItem(SIDER_WIDTH_STORAGE_KEY, String(desktopSiderWidthRef.current));
      } catch {
        // ignore storage access errors
      }
    };
            ...
            >
              ...
              >
                <div
                  ...
                  onClick={onClick}
                >
                  <img src={brandLogo} alt='One Work' className='w-full h-full object-contain' />
                </div>
                {isSettingsRoute ? (
                  ...
                  <div className='text-16px text-t-primary collapsed-hidden font-semibold'>One Work</div>
                )}
                {isMobile && !collapsed && (
                  <button
                    type='button'
