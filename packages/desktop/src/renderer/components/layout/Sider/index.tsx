...
import type { OneThemePreset } from '@renderer/utils/theme/applyOneThemePreset';
import { SiderToolbar, SiderSearchEntry, SiderScheduledEntry, SiderAssistantEntry, SiderMcpEntry } from './SiderNav';
import SiderFooter from './SiderFooter';
import WorkspaceIdentityEntry from '../WorkspaceIdentityEntry';
...
const Sider: React.FC<SiderProps> = ({ onSessionClick, collapsed = false }) => {
  ...
  const { theme, setTheme, setColorScheme, selectTheme } = useThemeContext();
  ...
  const handleMcpClick = () => {
    ...
    closePreview();
    setIsBatchMode(false);
    Promise.resolve(navigate('/mcp')).catch((error) => {
      console.error('Navigation failed:', error);
    });
    if (onSessionClick) {
      onSessionClick();
  ...
  const handleThemePresetApplied = useCallback(
    (preset: OneThemePreset) => {
      void setColorScheme(preset.colorScheme);
      void selectTheme(preset.theme === 'dark' ? 'dark' : 'light');
    },
    [selectTheme, setColorScheme]
  );
  ...
  return (
    <div className='size-full flex flex-col'>
      ...
      <div className='flex-1 min-h-0 overflow-hidden'>
        {isSettings ? (
          ...
          <div className='size-full flex flex-col gap-2px min-h-0'>
            ...
            />
            {/* Team Ops section — hoisted directly under the assistant entry so it
                sits near the top and no longer competes for space with Projects
                and History in the scroll area below. */}
            <TeamSiderSection
              collapsed={collapsed}
              pathname={pathname}
              siderTooltipProps={siderTooltipProps}
            ...
            />
            <div className={classNames('flex-1 min-h-0 flex flex-col overflow-hidden', siderStyles.scrollArea)}>
      ...
      {/* Footer */}
      <WorkspaceIdentityEntry collapsed={collapsed} />
      <SiderFooter
        isMobile={isMobile}
