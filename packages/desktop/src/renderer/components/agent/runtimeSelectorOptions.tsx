...
/**
 * `kind` is the user's declaration from model settings. It rides along on the
 * option so the send box shows the same coloured tag the settings list does —
 * a provider list mixes chat, image and video models under names that rarely
 * say which is which, and the picker is where that costs the user something.
 */
type RuntimeSelectorModel = {
   ...
   *
   * Inferred kinds are shown here too. An earlier version withheld them on the
   * grounds that everything in a chat picker is a chat model anyway, so a muted
   * "text?" on every row would be noise — but that reasoning only holds for the
   * rows it is right about. The same list carries `gemini-3-pro-image`, which
   * chats *and* returns pictures, and a name alone does not say so. Withholding
   * the label from every row to avoid repeating it on most of them took the
   * signal out with the noise.
   *
   * The tag appears only while choosing. Once a model is picked, the collapsed
   * trigger shows the name alone — at that point the kind has done its job and
   * would just crowd the send box.
   */
  kind?: ModelKind;
  /** True when `kind` was read off the name; renders muted with a `?`. */
  inferred?: boolean;
};
...
}> = ({ selected, description, trailing, children }) => {
  const content = (
    <div className='flex items-center gap-8px w-full min-w-0'>
      ...
      {trailing ? <span className='ml-auto shrink-0 flex items-center'>{trailing}</span> : null}
    </div>
  );
 ...
 * - the dream selector — provider-grouped via `groups`.
  ...
  const renderRow = (model: RuntimeSelectorModel) => (
    ...
    >
      <RuntimeSelectorCheckedItem
        ...
        trailing={<ModelKindTag kind={model.kind} inferred={model.inferred} />}
      >
