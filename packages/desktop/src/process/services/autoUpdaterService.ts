...
const FORCE_DEV_AUTO_UPDATE_ENV = 'DREAM_FORCE_DEV_AUTO_UPDATE';
...
class AutoUpdaterService extends EventEmitter {
  ...
  /**
   * Most recent release metadata resolved from the CDN update feed, captured from the
   * `update-available`/`update-not-available` events regardless of which entry point
   * triggered the check. Lets the manual "check for updates" IPC handler build a full
   * release description (version, notes, per-platform asset) without a second CDN fetch.
   */
  private _lastCheckedInfo: UpdateInfo | undefined;
  ...
  private ensureDevUpdateConfig(): void {
    try {
      ...
      const devConfig = [
        'provider: generic',
        `url: ${cdnFeedOptions.url}`,
        'updaterCacheDirName: com.huanle.oneone.ai',
        '',
      ].join('\n');
  ...
  reset(): void {
    ...
    this._lastCheckedInfo = undefined;
  }
  ...
  private setupEventHandlers(): void {
    ...
    register('update-not-available', (info: UpdateInfo) => {
   ...
   * while the downloaded archive contains the packaged app (com.huanle.oneone.ai). Squirrel.Mac
  ...
  async checkForUpdates(): Promise<{ success: boolean; updateInfo?: UpdateInfo; error?: string }> {
    try {
      ...
      if (this._allowPrerelease) {
        // No prerelease/dev channel is published to the CDN (see release-distribute.yml,
        // which only distributes non-prerelease GitHub Releases), so there is nothing to check.
        log.info('Skipping electron-updater check for prerelease manual mode');
        log.debug('[auto-update] CDN stable feed skipped: no prerelease channel is published to the CDN');
        return { success: true };
      }
      ...
      // Cache regardless of availability so getLastCheckedReleaseInfo()/
      // ensureLatestReleaseInfoChecked() (used by the manual "check for updates" IPC
      // handler) can build a full release description without a second CDN fetch.
      this._lastCheckedInfo = result.updateInfo;
  ...
  /**
   * Returns the most recently resolved CDN release metadata (from the last
   * `update-available`/`update-not-available` event), regardless of whether
   * that version was newer than the installed app. Used by the manual
   * "check for updates" IPC handler so it doesn't need its own CDN fetch.
   */
  getLastCheckedReleaseInfo(): UpdateInfo | undefined {
    return this._lastCheckedInfo;
  }

  /**
   * Ensures `getLastCheckedReleaseInfo()` has data by running a check if none has
   * happened yet. No-ops (network-wise) if a result is already cached — callers
   * that just ran `checkForUpdates()`/`checkForUpdatesAndNotify()` won't trigger
   * a second CDN request.
   */
  async ensureLatestReleaseInfoChecked(): Promise<UpdateInfo | undefined> {
    if (this._lastCheckedInfo) {
      return this._lastCheckedInfo;
    }
