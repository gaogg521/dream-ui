...
import { setGovernanceEndpoint } from '@process/services/governanceEndpoint';
import * as path from 'path';
import { initApplicationBridgeCore } from './applicationBridgeCore';
import type {
  IExtractRagFileResult,
  IReadTextFilesResult,
  IRuntimeNodeIdentity,
  IStartOnBootStatus,
} from '@/common/adapter/ipcBridge';
import { restartApplication } from './restartApplication';
import { extractTextFromFile, extractTextFromUrl } from '@process/utils/documentTextExtraction';
...
function listIpAddresses(): string[] {
...
async function getRuntimeNodeIdentity(): Promise<IRuntimeNodeIdentity> {
  return {
...
export function initApplicationBridge(): void {
  ...
  // Keeps every main-process governance caller — the team-knowledge MCP tool
  // and the media spend gate — pointed at whichever backend actually holds the
  // company's data (local, or the remote server in enterprise client mode).
  ipcBridge.application.setGovernanceEndpoint.provider(async (endpoint) => {
    setGovernanceEndpoint(endpoint ?? null);
  });
  ...
  ipcBridge.application.getRuntimeNodeIdentity.provider(async () => getRuntimeNodeIdentity());

  ipcBridge.application.extractRagDocumentFiles.provider(async ({ filePaths }) => {
    const results: IExtractRagFileResult[] = [];
    for (const filePath of filePaths) {
      ...
      try {
        ...
        results.push({ filePath, fileName, fileSize, ...extracted });
      } catch (e) {
        results.push({ filePath, fileName, fileSize, error: e instanceof Error ? e.message : String(e) });
      }
    }
    return results;
  });

  ipcBridge.application.extractRagDocumentUrl.provider(async ({ url }) => extractTextFromUrl(url));

  ipcBridge.application.readTextFiles.provider(async ({ filePaths }) => {
    const results: IReadTextFilesResult[] = [];
    for (const filePath of filePaths) {
      const fileName = path.basename(filePath);
      try {
        ...
        results.push({ filePath, fileName, error: e instanceof Error ? e.message : String(e) });
      }
    }
    return results;
  });
