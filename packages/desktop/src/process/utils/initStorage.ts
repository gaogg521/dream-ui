...
const initStorage = async () => {
  ...
  const mark = (label: string) => console.log(`[OneWork:init] ${label} +${Math.round(performance.now() - t0)}ms`);
  mark('start');
  ...
  // 6.5. One-shot import of a 1ONE ClaudeCode (1one) install's data + config.
  //      Runs after the handoff upgrade so the target catalog carries the v26
  //      baseline, and before the backend spawns so there is no writer race.
  try {
    const { runOneLegacyImport } = await import('@process/services/oneMigration');
    const oneImportSummary = await runOneLegacyImport(
      configFile as unknown as import('@process/services/oneMigration/importOneLegacyConfig').OneLegacyConfigFile
    );
