...
import { startWebHost, getLanIP, type WebHostHandle } from '@dream/web-host';
import { getDataPath } from './utils';
import { createMediaRouteDeps, handleMediaHttpRequest } from '@process/services/mediaHttpRoute';
import { mediaJobRouteDeps } from '@process/bridge/mediaBridge';
...
export async function startDesktopWebUI(opts: { port?: number; allowRemote?: boolean }): Promise<DesktopWebUIHandle> {
  ...
  const handle = await startWebHost({
    ...
    // Same media route as the desktop path — both hosts serve the same renderer
    // bundle, so both need it.
    extraHandlers: [
      (req, res) => handleMediaHttpRequest(req, res, createMediaRouteDeps(sysDir.workDir, mediaJobRouteDeps())),
    ],
    port: preferredPort,
    allowRemote,
