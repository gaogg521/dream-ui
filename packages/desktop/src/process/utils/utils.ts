...
export const getTempPath = () => {
  ...
  return path.join(rootPath, '1one');
};
 ...
 * Release builds use ~/.1one; dev builds use ~/.1one-dev.
 * 获取数据目录路径，macOS 上使用符号链接。
 * Release 使用 ~/.1one，Dev 模式使用 ~/.1one-dev。
 */
export const getDataPath = (): string => {
  const rootPath = getElectronPathOrFallback('userData');
  const dataPath = path.join(rootPath, '1one');
  return ensureCliSafeSymlink(dataPath, getEnvAwareName('.1one'));
};

/**
 * Get config path, using CLI-safe symlink on macOS.
 * Release builds use ~/.1one-config; dev builds use ~/.1one-config-dev.
 * 获取配置目录路径，macOS 上使用符号链接。
 * Release 使用 ~/.1one-config，Dev 模式使用 ~/.1one-config-dev。
 */
export const getConfigPath = (): string => {
  const rootPath = getElectronPathOrFallback('userData');
  const configPath = path.join(rootPath, 'config');
  return ensureCliSafeSymlink(configPath, getEnvAwareName('.1one-config'));
};
