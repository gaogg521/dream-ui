...
function isBackendStartupFailureEvent(event: { tags?: Record<string, unknown> }): boolean {
  return event.tags?.['dream.failure'] === 'backend_startup';
}

function isUserFeedbackEvent(event: { tags?: Record<string, unknown> }): boolean {
  return event.tags?.type === 'user-feedback' || event.tags?.['dream.installation_integrity.user_report'] === 'true';
}
...
export async function captureBackendStartupFailure(error: unknown): Promise<void> {
  ...
  Sentry.withScope((scope) => {
    scope.setTag('dream.failure', 'backend_startup');
    scope.setTag('dream.backend_startup.reason', failureInfo.reason);
    if (failureInfo.runtime) {
      scope.setTag('dream.backend_startup.runtime', failureInfo.runtime);
    }
    if (failureInfo.packageArch) {
      scope.setTag('dream.backend_startup.package_arch', failureInfo.packageArch);
    }
    if (failureInfo.deviceArch) {
      scope.setTag('dream.backend_startup.device_arch', failureInfo.deviceArch);
    }
    if (failureInfo.expectedDownloadArch) {
      scope.setTag('dream.backend_startup.expected_download_arch', failureInfo.expectedDownloadArch);
    }
    if (typeof failureInfo.isRosettaTranslated === 'boolean') {
      scope.setTag('dream.backend_startup.rosetta_translated', getBooleanTagValue(failureInfo.isRosettaTranslated));
    }
    if (typeof details?.stage === 'string') {
      scope.setTag('dream.backend_startup.stage', details.stage);
    }
    if (failureInfo.backendBoundaryCode) {
      scope.setTag('dream.backend_startup.boundary_code', failureInfo.backendBoundaryCode);
    }
    if (failureInfo.backendBoundaryStage) {
      scope.setTag('dream.backend_startup.boundary_stage', failureInfo.backendBoundaryStage);
    }
    if (failureInfo.localDataIssueKind) {
      scope.setTag('dream.backend_startup.local_data_issue_kind', failureInfo.localDataIssueKind);
    }
    if (failureInfo.incompleteInstallationKind) {
      scope.setTag('dream.backend_startup.incomplete_installation_kind', failureInfo.incompleteInstallationKind);
    }
    for (const [tag, value] of [
      ['dream.backend_startup.missing_bundled_dir', getBooleanTagValue(failureInfo.missingBundledDreamcoreDir)],
      ['dream.backend_startup.missing_runtime_dir', getBooleanTagValue(failureInfo.missingRuntimeDir)],
      ['dream.backend_startup.missing_binary', getBooleanTagValue(failureInfo.missingBackendBinary)],
      ['dream.backend_startup.missing_hub_dir', getBooleanTagValue(failureInfo.missingHubDir)],
      ['dream.backend_startup.missing_pet_states_dir', getBooleanTagValue(failureInfo.missingPetStatesDir)],
      ['dream.backend_startup.missing_pwa_dir', getBooleanTagValue(failureInfo.missingPwaDir)],
      ['dream.backend_startup.install_path_kind', getInstallPathKind(details?.resourcesPath)],
      ['dream.backend_startup.last_update_status', getString(autoUpdateDiagnostics?.lastEvent?.status)],
      [
        'dream.backend_startup.health_polling_delayed',
        getBooleanTagValue(
          typeof details?.healthCheckPollingDelayed === 'boolean' ? details.healthCheckPollingDelayed : undefined
        ),
      ],
      ['dream.backend_startup.health_attempts_bucket', getHealthAttemptBucket(details?.healthCheckAttempts)],
      [
        'dream.backend_startup.health_attempt_deficit_bucket',
        getHealthAttemptBucket(details?.healthCheckAttemptDeficit),
      ],
      ['dream.backend_startup.health_timeout_overrun_bucket', getDurationBucket(details?.healthCheckTimeoutOverrunMs)],
      ['dream.backend_startup.health_max_attempt_gap_bucket', getDurationBucket(details?.healthCheckMaxAttemptGapMs)],
      [
        'dream.backend_startup.seconds_since_quit_and_install',
    ...
    if (details) {
      scope.setContext('dreamcore_startup', details);
      scope.setExtra('dreamcore_startup', details);
    }
    scope.setContext('dreamcore_startup_classification', { ...failureInfo });
    scope.setExtra('dreamcore_startup_classification', failureInfo);
    scope.setContext('dreamcore_install_diagnostics', installDiagnostics);
    scope.setExtra('dreamcore_install_diagnostics', installDiagnostics);
