...
import { captureBackendStartupFailure, initSentry, setSentryDeviceId } from './sentry';
...
import { installMediaProtocolHandler, registerMediaProtocolScheme } from '@process/services/mediaProtocol';
import { initMediaBridge, mediaJobRouteDeps } from '@process/bridge/mediaBridge';
...
import { BackendLifecycleManager } from '@dream/web-host';
...
import { startWebHost } from '@dream/web-host';
...
import { createMediaRouteDeps, handleMediaHttpRequest } from './process/services/mediaHttpRoute';
import { getMediaJobManager } from './process/services/mediaJob';
...
// Privileged schemes must be declared before `app` emits `ready`, so this runs
...
const onSecondInstance = (
  ...
  console.log('[OneWork] second-instance: deep link', deepLinkUrl ? 'received' : 'absent');
  if (deepLinkUrl) {
  ...
  if (app.isReady()) {
    showOrCreateMainWindow({
      mainWindow,
      createWindow: () => {
        console.log('[OneWork] second-instance received with no active main window, recreating main window');
        createWindow();
      },
    });
  }
...
if (useDeepLinkBus) {
  if (requestDeepLinkBusLock()) {
    ...
    console.warn('[OneWork] Deep-link bus held by a running instance; forwarding protocol URL and exiting.');
    app.quit();
  }
  ...
  console.warn('[OneWork] Another instance is already running; current process will exit.');
  app.quit();
} else {
  app.on('second-instance', onSecondInstance);
}
...
function logUncaught(diagnostics: UncaughtErrorDiagnostics): void {
  try {
    console.error(`[OneWork] ${diagnostics.origin}:`, diagnostics);
  } catch {
...
function markBackendRuntimeCrashed(context: { exitCode?: number; signal?: string; restartCount: number }): void {
  ...
  (globalThis as typeof globalThis & { __backendStartupFailed?: boolean }).__backendStartupFailed = true;
  console.error('[OneWork] backend crashed past its restart budget; surfacing to the user', context);
  broadcastBackendStartupState(backendStartupFailureInfo);
}
...
function registerCronResumeBridge(backendPort: number): void {
  ...
  const onResume = () => {
    void fetch(`http://127.0.0.1:${backendPort}/api/cron/internal/system-resume`, {
      ...
      console.error('[OneWork] Failed to notify backend about system resume:', error);
    });
  };
...
const scheduleBackendMigrations = (): void => {
  ...
  void (async () => {
    try {
      ...
      console.info('[OneWork] runBackendMigrations completed');
    } catch (error) {
      console.error('[OneWork] Backend migration hook threw:', error);
    }
  })();
};
...
function markBackendReady(backendPort: number, source: string): void {
  ...
  console.log(`[OneWork] ${source} ready (port=${backendPort})`);
...
function resolveDebugBackendStartupFailure(): BackendStartupFailureInfo | null {
  ...
  if ((app.isPackaged && !isE2ETestMode) || isWebUIMode || isResetPasswordMode) {
    console.warn('[OneWork] Ignoring DREAM_DEBUG_BACKEND_STARTUP_FAILURE outside desktop dev/e2e mode.');
    return null;
  }
  ...
  console.warn(`[OneWork] Ignoring unknown DREAM_DEBUG_BACKEND_STARTUP_FAILURE value: ${reason}`);
  return null;
}
...
const createWindow = ({ showOnReady = true }: { showOnReady?: boolean } = {}): void => {
  console.log('[OneWork] Creating main window...');
  ...
  console.log(`[OneWork] Main window created (id=${mainWindow.id})`);
  ...
  if (showOnReady) {
    const showWindow = () => {
      if (!mainWindow.isDestroyed() && !mainWindow.isVisible()) {
        console.log('[OneWork] Showing main window');
        mainWindow.show();
        mainWindow.focus();
      }
    };
    mainWindow.once('ready-to-show', () => {
      console.log('[OneWork] Window ready-to-show');
      showWindow();
    });
    // Belt-and-suspenders: also show on did-finish-load in case ready-to-show already fired
    mainWindow.webContents.once('did-finish-load', () => {
      console.log('[OneWork] Renderer did-finish-load');
      showWindow();
      scheduleBackendMigrations();
    });
  ...
  if (!disableAutoUpdater) {
    ...
    console.log('[OneWork] Auto-updater disabled via env/CI guard');
  }
  ...
  if (!app.isPackaged && rendererUrl) {
    console.log(`[OneWork] Loading renderer URL: ${rendererUrl}`);
    mainWindow.loadURL(rendererUrl).catch((error) => {
      console.error('[OneWork] loadURL failed, falling back to file:', error.message || error);
      mainWindow.loadFile(fallbackFile).catch((e2) => {
        console.error('[OneWork] loadFile fallback also failed:', e2.message || e2);
      });
    });
  } else {
    console.log(`[OneWork] Loading renderer file: ${fallbackFile}`);
    mainWindow.loadFile(fallbackFile).catch((error) => {
      console.error('[OneWork] loadFile failed:', error.message || error);
    });
  }

  mainWindow.webContents.on('did-fail-load', (_event, errorCode, errorDescription, validatedURL, isMainFrame) => {
    console.error('[OneWork] did-fail-load:', { errorCode, errorDescription, validatedURL, isMainFrame });
  });
  ...
  mainWindow.webContents.on('render-process-gone', (_event, details) => {
    console.error('[OneWork] render-process-gone:', details);
    if (mainWindow.isDestroyed()) return;

    const action = rendererRecovery.onCrash(details.reason);

    if (action.kind === 'relaunch') {
      console.warn(`[OneWork] renderer cannot be recovered in-place (reason=${details.reason}); relaunching app`);
      app.relaunch();
      app.exit(0);
      return;
    }
    ...
    if (action.kind === 'give-up') {
      console.error(`[OneWork] renderer recovery exhausted (reason=${details.reason}); not retrying`);
      return;
    }
    ...
    const reload = () => {
      if (mainWindow.isDestroyed()) return;
      console.log('[OneWork] Attempting to recover from renderer crash by reloading...');

      if (!app.isPackaged && rendererUrl) {
        mainWindow.loadURL(rendererUrl).catch((error) => {
          console.error('[OneWork] Recovery loadURL failed:', error.message || error);
        });
      } else {
        mainWindow.loadFile(fallbackFile).catch((error) => {
          console.error('[OneWork] Recovery loadFile failed:', error.message || error);
        });
      }
    };
  ...
  mainWindow.webContents.on('unresponsive', () => {
    console.warn('[OneWork] Renderer became unresponsive');
  });

  mainWindow.on('closed', () => {
    console.log('[OneWork] Main window closed');
  });
...
const handleAppReady = async (): Promise<void> => {
  ...
  const mark = (label: string) => console.log(`[OneWork:ready] ${label} +${Math.round(performance.now() - t0)}ms`);
  mark('start');

  // Serves generated media to the renderer. Must be installed after ready; the
  // scheme itself is registered at module load (pre-ready), as Electron requires.
  installMediaProtocolHandler();
  // Media jobs outlive the tool call that started them, so the window learns
  // about them through this bridge rather than the conversation stream.
  initMediaBridge();
        ...
        return backendManager.start(
          ...
          },
          {
            ...
            onPeerRetriesExhausted: async () => {
              ...
              console.warn(`[OneWork] Cleared ${killed} stale dreamcore process(es) holding the data directory.`);
            },
  ...
  try {
    ...
    console.error('[OneWork] Failed to restore zoom factor:', error);
  ...
  try {
    ...
    console.error('[OneWork] Failed to restore window bounds:', error);
  ...
  if (isResetPasswordMode) {
    ...
    try {
      ...
      const handle = await startWebHost({
        ...
        // In dev, `electron-vite dev` refreshes only out/main and out/preload — the
        // renderer lives on its own dev server and out/renderer keeps whatever the
        // last full build wrote. Proxying to the dev server keeps the WebUI in step
        // with the desktop window; without it the WebUI silently served a stale
        ...
        // Generated media for browser clients: `one-media://` only resolves
        // inside the desktop window, so the WebUI needs an HTTP route. Fenced
        // to the media job store's workspaces — see mediaHttpRoute.ts.
        extraHandlers: [
          (req, res) =>
            handleMediaHttpRequest(req, res, createMediaRouteDeps(sysDirWebUI.workDir, mediaJobRouteDeps())),
        ],
        port: resolvedPort,
        allowRemote,
        dataDir: getDataPath(),
...
if (shouldRegisterBackendStartup(gotTheLock)) {
  void app
    ...
    .catch((error) => {
      ...
      console.error('[OneWork] App initialization failed:', error);
      app.quit();
    });
}
...
app.on('will-quit', () => {
  console.log('[OneWork] will-quit — all cleanup should be complete');
});

app.on('quit', (_event, exitCode) => {
  console.log(`[OneWork] quit (exitCode=${exitCode})`);
});
