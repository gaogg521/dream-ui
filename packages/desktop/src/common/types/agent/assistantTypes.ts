...
// Mirror of dream-api-types/src/assistant.rs.
// Any shape change on either side requires a same-PR update on the other.

export type AssistantSource = 'builtin' | 'generated' | 'user' | 'imported';
...
/**
 * `user` (hand-authored) and `imported` (persona-library import) assistants
 * are both freely mutable/deletable by the owner. `builtin` and `generated`
 * (bare-CLI) rows are not.
 */
export function isMutableAssistantSource(source: AssistantSource | string | undefined): boolean {
...
export interface CreateAssistantRequest {
  ...
  /** Only honored by `assistants.importPersonas`; ignored elsewhere. */
  rule_content?: string;
}
 ...
 * real owned assistant until `installed` flips true via `install`. */
export interface MarketplacePersona {
  ...
  role_name?: string;
  category?: string;
  /** Backend-relative avatar byte route, present only when the catalog entry ships an image. */
  avatar?: string;
}
