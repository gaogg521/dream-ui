...
export type MediaGenParams = {
  ...
  camera?: string;
  /**
   * Generate an audio track along with the video.
   *
   * Left undefined by default rather than defaulting to a value here: the
   * vendor has its own default, and audio is a billable extra on some of them —
   * choosing on the user's behalf would silently change what they are charged.
   */
