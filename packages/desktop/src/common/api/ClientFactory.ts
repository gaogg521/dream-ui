...
/**
 * How this app identifies itself to OpenRouter-style gateways.
 *
 * These are not internal identifiers: `X-Title` is the name shown on the
 * provider's own app listings, and `HTTP-Referer` is the site it links to. They
 * were still the upstream project's name and domain, so every request this fork
 * made introduced itself as someone else's product.
 *
 * Kept as one constant because the two client paths below had their own copies,
 * and a divergence would be invisible from inside the app — the only place the
 * difference shows is a third party's dashboard.
 */
const ATTRIBUTION_HEADERS = {
  'HTTP-Referer': 'https://work.1oneclaw.com',
  'X-Title': 'One Work',
} as const;
