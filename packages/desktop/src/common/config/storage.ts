...
export interface IConfigStorageRefer {
  ...
  // 企业运行时节点心跳用的稳定机器 ID，首次生成后持久化 / Stable machine id for enterprise runtime-node heartbeats, generated once and persisted
  ...
  /** Enterprise deployment: this machine hosts the org (server) or connects remotely (client). */
  'webui.deploymentRole'?: 'server' | 'client';
  /** Remote enterprise server address when deploymentRole is client. */
  'webui.enterpriseServerUrl'?: string;
  // Memory Management: explicit project root override for ~/.claude/projects/{sanitized}/memory
  // and project CLAUDE.md. When unset, falls back to the app work directory.
  'memory.claudeProjectRoot'?: string;
  // Memory Management: extra project roots whose memory directories are also loaded.
 ...
 * 会话来源类型 - 标识会话创建的来源
 */
export type ConversationSource = 'dream' | 'telegram' | 'lark' | 'dingtalk' | 'weixin' | 'wecom' | (string & {});
...
interface IChatConversation<T, Extra> {
  ...
  /** 会话来源，默认为 dream / Conversation source, defaults to dream */
...
export type TChatConversation =
  ...
  // Dream Core/crates/dream-common/src/enums.rs and factory.rs.
 ...
 *
 * Declared rather than inferred: new image and video models ship constantly,
 * and matching them by name means every new one needs a code change before it
 * can be used. The user already knows what they added — this is where they say
 * so, and it drives both the coloured tag in the model list and whether the
 * model is offered as an image/video generator.
 *
 * `multimodal` means "text in/out, understands images" (a vision chat model),
 * which is different from `image` — a model that *produces* images.
 */
export type ModelKind = 'text' | 'multimodal' | 'image' | 'video' | 'audio';

export const MODEL_KINDS: ModelKind[] = ['text', 'multimodal', 'image', 'video', 'audio'];
...
export type ModelSettings = {
  ...
  /**
   * Which media API this model speaks, for models the built-in catalog does not
   * recognize. Absent = let the catalog decide. Only meaningful when
   * `model_kind` is `image` or `video`.
   */
  media_endpoint?: string;
  /**
   * Price of one generated asset (image) or one second (video), in USD.
   *
   * User-entered on purpose: a gateway's price is the user's contract with
   * their vendor, not something the app can read. When set, this replaces the
...
export interface IProvider {
  ...
  is_full_url?: boolean;
  /**
   * 存储的 API Key 是否可读回。'unrecoverable' 表示密钥仍在库中但无法解密
   * （加密密钥已变更），需要用户重新录入。默认视为 'ok'。
   * Read-back status of the stored API key. 'unrecoverable' means the key is
   * present in storage but could not be decrypted (the encryption key changed)
   * and must be re-entered. Absent/undefined is treated as 'ok'.
   */
  key_status?: 'ok' | 'unrecoverable';
  /**
   * 渠道归属。缺省 / undefined = 用户自己配置的，可编辑可删除。
   * 'enterprise' = 企业下发的模型渠道：本机只读，由同步对账；它的 api_key 是
   * 一枚可撤销的渠道令牌，不是厂商密钥——真正的密钥只存在于公司服务器上。
   *
   * Ownership. Absent = the user configured it themselves. 'enterprise' = it
   * came from a company model channel: read-only here, reconciled by the sync,
   * and its api_key is a revocable channel token rather than a vendor
   * credential (which never leaves the company server).
   */
...
export interface IMcpServer {
  ...
  /** Built-in MCP server managed by Dream UI (hide edit/delete in UI) */
  builtin?: boolean;
}
...
/**
 * One reason a tool's definition will be rejected by the model API.
 *
 * Model providers constrain tool/parameter names (Anthropic: property keys
 * must match `^[a-zA-Z0-9_.-]{1,64}$`) and reject the *entire* request when
 * any tool violates that — identifying the offender only by its index in the
 * flattened tool array. The backend screens each tool so the UI can name the
 * exact tool and key instead.
 */
export interface IMcpToolIncompatibility {
  /** JSON path inside the tool definition, e.g. `inputSchema.properties`. */
  path: string;
  /** The offending key (parameter name, or the tool name itself). */
  key: string;
  /** Machine-readable rule violated: `illegal_characters` | `too_long` | `empty`. */
  reason: string;
}
...
export interface IMcpTool {
  ...
  /** Empty/absent for well-formed tools. Servers with any entry here are
   *  excluded from agent sessions to keep conversations usable. */
  incompatibilities?: IMcpToolIncompatibility[];
