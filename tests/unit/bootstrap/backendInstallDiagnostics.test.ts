...
describe('collectBackendInstallDiagnostics', () => {
  it('records packaged runtime manifest and missing backend binary metadata', () => {
    const files = new Map<string, { mtimeMs: number; size: number; content?: string }>([
      ...
      ['C:\\AionUi\\resources\\bundled-dreamcore\\win32-x64', { mtimeMs: 2000, size: 0 }],
      [
        'C:\\AionUi\\resources\\bundled-dreamcore\\win32-x64\\manifest.json',
        {
          mtimeMs: 3000,
          size: 88,
          content: JSON.stringify({
            ...
            sourceType: 'download',
            files: ['dreamcore.exe', 'managed-resources/'],
          }),
        },
      ],
    ]);
    ...
    const diagnostics = collectBackendInstallDiagnostics(
      {
        ...
        checkedBundledPath: 'C:\\AionUi\\resources\\bundled-dreamcore\\win32-x64\\dreamcore.exe',
      },
      {
        appVersion: '2.1.7',
        arch: 'x64',
    ...
    expect(diagnostics).toEqual({
      ...
      binaryExists: false,
      binaryName: 'dreamcore.exe',
      binaryPath: 'C:\\AionUi\\resources\\bundled-dreamcore\\win32-x64\\dreamcore.exe',
      bundledDirPath: 'C:\\AionUi\\resources\\bundled-dreamcore',
      legacyBundledDirPath: 'C:\\AionUi\\resources\\bundled-aioncore',
      execPath: 'C:\\AionUi\\AionUi.exe',
      isPackaged: true,
      manifestExists: true,
      manifestFiles: ['dreamcore.exe', 'managed-resources/'],
      manifestGeneratedAt: '2026-05-29T12:00:00.000Z',
      manifestPath: 'C:\\AionUi\\resources\\bundled-dreamcore\\win32-x64\\manifest.json',
      manifestSize: 88,
      ...
      runtimeDirPath: 'C:\\AionUi\\resources\\bundled-dreamcore\\win32-x64',
      runtimeKey: 'win32-x64',
    });
  });
});
