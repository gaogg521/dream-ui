...
const teams: TTeam[] = [
  ...
  },
  {
    ...
    workspace_mode: 'shared',
    leader_assistant_id: 'security-lead',
    assistants: [],
    created_at: 1,
    updated_at: 1,
  },
  {
    ...
    workspace_mode: 'shared',
    leader_assistant_id: 'custom-lead',
    assistants: [],
    created_at: 1,
    updated_at: 1,
  },
...
vi.mock('@icon-park/react', async () => {
  ...
  return {
    ...
    // Fork-only: the "view all teams" affordance on the section header, which
    // sends the user to the Session Center (the sidebar only shows a preview).
    FullScreen: icon('FullScreen'),
    ChartStock: icon('ChartStock'),
    Peoples: icon('Peoples'),
    Plus: icon('Plus'),
    Pushpin: icon('Pushpin'),
    Right: icon('Right'),
    Shield: icon('Shield'),
    StockMarket: icon('StockMarket'),
    TopicDiscussion: icon('TopicDiscussion'),
  };
});

const renderSection = (collapsed: boolean, pathname = '/guid') =>
  render(<TeamSiderSection collapsed={collapsed} pathname={pathname} siderTooltipProps={{}} />);
...
describe('TeamSiderSection running state', () => {
  ...
  it('uses a semantic icon when the team name describes a security workflow', () => {
    localStorage.setItem('team-section-expanded', 'true');
    renderSection(false);

    expect(screen.getByTestId('team-icon-security-team')).toHaveAttribute('data-mock-icon', 'Shield');
  });

  it('generates a stable initial avatar when a custom team has no known theme keyword', () => {
    localStorage.setItem('team-section-expanded', 'true');
    renderSection(false, '/team/custom-team');

    expect(screen.getByTestId('team-icon-custom-team')).toHaveTextContent('创');
  });
