...
/**
 * Entering image/video mode fills this row: a mode pill, a parameter pill
 * naming the media model, the price, and an exit button — all to the left of
 * the chat model selector.
 */
describe('GuidActionRow model selector during generation', () => {
  beforeEach(() => {
    vi.clearAllMocks();
  });
  ...
  it('shows the chat model selector in conversation mode', () => {
    renderActionRow({ modelSelectorNode: <div data-testid='chat-model-selector' /> });
    expect(screen.getByTestId('chat-model-selector')).toBeInTheDocument();
  });

  /**
   * The chat model takes no part in a generation send — the job runs on the
   * media model shown in the parameter pill. Rendering both left two
   * disagreeing answers to "which model runs my next Enter" on screen at once,
   * and the overflow drew straight over the selector.
   */
  it('hides it while a generation mode is on', () => {
    renderActionRow({
      modelSelectorNode: <div data-testid='chat-model-selector' />,
      mediaActive: true,
    });
    expect(screen.queryByTestId('chat-model-selector')).not.toBeInTheDocument();
  });

  // The permission selector is not about which model runs, so it stays.
  it('keeps the mode switch, which is not a model indicator', () => {
    renderActionRow({
      modelSelectorNode: <div data-testid='chat-model-selector' />,
      mediaActive: true,
      dynamicModes: [{ value: 'auto', label: 'Auto' }],
    });
    expect(screen.queryByTestId('chat-model-selector')).not.toBeInTheDocument();
...
describe('GuidActionRow skill/MCP submenu search', () => {
  ...
  it('still toggles a skill from a filtered list', () => {
    ...
    fireEvent.change(screen.getByTestId('guid-skill-search'), { target: { value: 'skill-3' } });
    // Each row is a `button role="option"` inside a listbox;
    // `closest('[role="menuitem"]')` matched nothing and returned null,
    // which fireEvent reports as "please provide a DOM element" rather than
    // as a missing element. Query the row's own testid so the next markup
    // change breaks the component test, not the selector.
    fireEvent.click(screen.getByTestId('guid-skill-row-skill-3'));
