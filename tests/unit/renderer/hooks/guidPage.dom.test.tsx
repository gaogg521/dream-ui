...
// This fork replaced AssistantSelectionArea with BackendSelectionArea when it
// split 'pick a backend' from 'pick a persona' (07-28); GuidPage renders the
// latter, so mock that one or the capture array stays empty.
vi.mock('@/renderer/pages/guid/components/BackendSelectionArea', () => ({
  default: (props: Record<string, unknown>) => {
    capturedAssistantSelectionAreaProps.push(props);
    return <div data-testid='backend-selection-area' />;
  },
}));
...
describe('GuidPage', () => {
  ...
  it('keeps a generic conversation heading and omits assistant-detail chrome on the home page', () => {
    ...
    expect(screen.getByTestId('backend-selection-area')).toBeInTheDocument();
...
describe('GuidInputCard prefill focus', () => {
  // These tests need the *real* GuidInputCard, and importing it cold drags in
  // Arco's Input, icon-park, FilePreview, UploadProgressBar, the workspace
  // footnote and i18n. Doing that inside the first `it` was measured at 9687ms
  // against the 10000ms default testTimeout — the test spent its whole budget
  // on a module import before asserting anything, so any extra CPU contention
  // from the full 444-file run tipped it into a timeout that surfaced as the
  // focus assertions failing. The import always resolves (it cannot hang, it is
  // just slow), so load it once up front under its own budget and leave the
  // tests measuring behaviour only.
  let GuidInputCard: typeof import('@/renderer/pages/guid/components/GuidInputCard').default;

  beforeAll(async () => {
    const actual = await vi.importActual<typeof import('@/renderer/pages/guid/components/GuidInputCard')>(
      '@/renderer/pages/guid/components/GuidInputCard'
    );
    GuidInputCard = actual.default;
  }, 120000);

  // Focus state is global to the document, so a test that fails part-way through
  // must not leave a focused element behind for the next one to trip over.
  const outsideTargets: HTMLButtonElement[] = [];
  ...
  afterEach(() => {
    while (outsideTargets.length > 0) {
      outsideTargets.pop()?.remove();
    }
  });
  ...
  it('focuses a draft-preserving prefill on desktop', async () => {
    ...
    const outsideTarget = createOutsideTarget();
    outsideTarget.focus();
    rerender(
      <LayoutContext.Provider value={{ isMobile: false, siderCollapsed: false, setSiderCollapsed: vi.fn() }}>
  ...
  it('does not force focus for a mobile prefill', () => {
    render(
  ...
  it('does not focus without an explicit request key', () => {
    const outsideTarget = createOutsideTarget();
    outsideTarget.focus();
