...
// Mirror DreamModal's real prop contract: header/footer may be config objects.
...
describe('TeamCreateModal', () => {
  ...
  it('keeps blocked assistants visible and prevents selecting them', () => {
    ...
    expect(screen.getByTestId('team-create-agent-option-bare-dream-engine')).toBeInTheDocument();
  ...
  it('renders the reference two-column creation layout with details on the selected-member side', () => {
    ...
    expect(within(assistantPane).getByTestId('team-create-agent-option-bare-dream-engine')).toHaveClass('!h-44px');
    expect(within(assistantPane).getByTestId('team-create-agent-option-bare-dream-engine')).toHaveClass(
      'hover:!bg-fill-2'
    );
  ...
  it('passes assistant identity through when creating a team with an assistant leader', async () => {
    ...
    fireEvent.click(screen.getByTestId('team-create-agent-option-bare-dream-engine'));
    ...
    expect(resolveDefaultTeamAgentModelMock).toHaveBeenCalledWith({
      assistant_id: 'bare-dream-engine',
      assistant_backend: 'dream-engine',
    });
...
describe('TeamCreateModal · mobile (narrow screen)', () => {
  ...
  it('creates a team from the mobile flow with the first member as leader', async () => {
    ...
    expect(payload.agents[0]).toMatchObject({ role: 'leader', assistant_id: 'bare-dream-engine' });
  });
});
