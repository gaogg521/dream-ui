...
describe('simplifyWarmupError', () => {
  it('strips the rebuild noise and generic wrapper prefixes', () => {
    const raw =
      "Invalid request: failed to warm up rebuilt agent 019f459c-1f5c-7652-b996-33782c32418e: Invalid request: Bad request: Provider 'dream-engine' not found";
    expect(simplifyWarmupError(raw)).toBe("Provider 'dream-engine' not found");
  });
