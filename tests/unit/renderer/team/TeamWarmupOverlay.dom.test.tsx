...
describe('TeamWarmupOverlay failure states', () => {
  it('renders a single-member failure with its simplified error', () => {
    render(
      <TeamWarmupOverlay
        ...
        runtimeStatus={runtime([
          ['l', 'ready'],
          ['m', 'failed', "Invalid request: Bad request: Provider 'dream-engine' not found"],
        ])}
        colorOf={colorOf}
        onRetry={() => {}}
      />
    ...
    expect(screen.getByTestId('team-warmup-error')).toHaveTextContent("Provider 'dream-engine' not found");
  ...
  it('summarises multiple failures as a scrollable per-member list', () => {
    render(
      <TeamWarmupOverlay
        ...
        runtimeStatus={runtime([
          ['l', 'failed', 'ACP error'],
          ['a', 'failed', "Bad request: Provider 'dream-engine' not found"],
          ['c', 'ready'],
        ])}
        colorOf={colorOf}
        onRetry={() => {}}
    ...
    expect(box).toHaveTextContent("Provider 'dream-engine' not found");
