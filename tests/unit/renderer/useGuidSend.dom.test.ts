...
describe('useGuidSend', () => {
  ...
  it('blocks a known unavailable backend before creating a failed conversation', async () => {
    const deps = createDeps();
    deps.selectedBackendAvailable = false;
  ...
  it('allows an attachment-only conversation to be created', async () => {
    const deps = createDeps();
    deps.input = '';
    deps.files = [{ kind: 'upload', path: 'C:/tmp/report.pdf' }];
