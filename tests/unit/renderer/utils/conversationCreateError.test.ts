...
const t = (key: string, options?: { defaultValue?: string; workspacePath?: string }) => {
  const translations: Record<string, string> = {
    ...
    'conversation.agentError.codes.PROVIDER_NOT_FOUND.body':
      'This conversation was created with a model provider that has since been deleted or replaced. Please choose an available model before sending.',
  };
...
describe('conversationCreateError', () => {
  ...
  it('surfaces a friendly message when the conversation references a deleted provider', () => {
    const error = httpError('PROVIDER_NOT_FOUND', "Provider 'ff9e8905' not found", { provider_id: 'ff9e8905' });
