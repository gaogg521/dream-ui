...
const {
  ...
  messageErrorMock,
  useAcpModelInfoMock,
  useCodexBridgeEnabledMock,
  useClaudeBridgeEnabledMock,
  useCodexBridgeModelMock,
  useClaudeBridgeModelMock,
} = vi.hoisted(() => ({
...
describe('AcpModelSelector runtime options', () => {
  ...
  it('locks the model as read-only when the Codex bridge is enabled for a codex conversation', () => {
    useCodexBridgeEnabledMock.mockReturnValue(true);

    render(<AcpModelSelector conversation_id='conversation-1' backend='codex' />);

    const pill = screen.getByTestId('acp-model-selector-bridge-locked');
    expect(pill).toHaveTextContent('GPT-5.2 · High · Bridged');
  ...
  it('locks the model as read-only when the Claude bridge is enabled for a claude conversation', () => {
    useClaudeBridgeEnabledMock.mockReturnValue(true);

    render(<AcpModelSelector conversation_id='conversation-1' backend='claude' />);

    const pill = screen.getByTestId('acp-model-selector-bridge-locked');
    expect(pill).toHaveTextContent('GPT-5.2 · High · Claude Bridged');
    expect(pill.closest('[data-tooltip-content]')).toHaveAttribute('data-tooltip-content', 'Claude Bridged tooltip');
    expect(screen.queryByTestId('acp-model-selector')).not.toBeInTheDocument();
  });

  it('shows the bridge-configured model instead of the ACP session-advertised model when locked', () => {
    useCodexBridgeEnabledMock.mockReturnValue(true);
    ...
    const pill = screen.getByTestId('acp-model-selector-bridge-locked');
    expect(pill).toHaveTextContent('glm-5-2-aliyun · High · Bridged');
    expect(pill).not.toHaveTextContent('GPT-5.2');
  });

  it('does not lock the model for non-codex backends even when the codex bridge is enabled', () => {
    useCodexBridgeEnabledMock.mockReturnValue(true);

    render(<AcpModelSelector conversation_id='conversation-1' backend='gemini' />);

    expect(screen.getByTestId('acp-model-selector')).toBeInTheDocument();
    expect(screen.queryByTestId('acp-model-selector-bridge-locked')).not.toBeInTheDocument();
  });
