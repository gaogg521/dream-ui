...
describe('channel assistant binding helpers', () => {
  const assistants = [
    assistant({ id: 'bare-dream-engine', name: 'Aion CLI', source: 'generated', preset_agent_type: 'dream-engine' }),
  ...
  it('prefers the generated dream-engine assistant as the default selection', () => {
    expect(getDefaultChannelAssistant(assistants)?.id).toBe('bare-dream-engine');
  });
  ...
  it('falls back to the default assistant only when no binding was saved', () => {
    expect(resolveChannelAssistantId(undefined, assistants)).toBe('bare-dream-engine');
  });
