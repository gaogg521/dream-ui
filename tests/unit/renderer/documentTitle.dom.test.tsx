...
describe('titleForPath', () => {
  ...
  it('uses the login title on the login route only', () => {
    ...
    expect(titleForPath('/guid', t)).toBe('One Work');
    expect(titleForPath('/conversation/abc', t)).toBe('One Work');
    expect(titleForPath('/settings/agent', t)).toBe('One Work');
  });
});

describe('DocumentTitle', () => {
  it('resets the title to One Work after leaving the login page', () => {
    ...
    document.title = 'One Work - stale login title';
    render(
      <MemoryRouter initialEntries={['/guid']}>
        <DocumentTitle />
      </MemoryRouter>
    );
    expect(document.title).toBe('One Work');
  });
