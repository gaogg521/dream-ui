...
// TaskDetailPage renders CreateTaskDialog, which now mounts through DreamModal
...
describe('TaskDetailPage', () => {
  ...
  it('does not present a stale next run time for a paused task', async () => {
    getJobInvokeMock.mockResolvedValue(job({ enabled: false, state: { next_run_at_ms: 1 } }));
    ...
    await waitFor(() => expect(getJobInvokeMock).toHaveBeenCalledWith({ job_id: 'job-1' }));
    expect(screen.queryByText(/cron\.nextRun/)).not.toBeInTheDocument();
  });
