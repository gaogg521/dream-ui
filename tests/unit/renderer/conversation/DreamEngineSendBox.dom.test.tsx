...
vi.mock('react-i18next', () => ({
  // `i18n` is not optional: the send box reads `i18n.language` to pick the
  // backend's localised product name. Omitting it threw on render and took all
  // five cases in this file down at once — including the guard against warming
  // up a team session on every keystroke.
  useTranslation: () => ({ t: translateMock, i18n: { language: 'en' } }),
}));
vi.mock('@/renderer/pages/conversation/platforms/dreamEngine/useDreamEngineMessage', () => ({
  useDreamEngineMessage: () => ({
