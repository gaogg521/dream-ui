...
describe('buildSendFailureError', () => {
  it('classifies 409 already-processing as DREAM_CONVERSATION_BUSY (wait, not retry)', () => {
  ...
  it('classifies 409 already-running as DREAM_CONVERSATION_BUSY', () => {
  ...
  it('classifies runtime shutdown conflict as non-retryable busy without raw internal diagnostics', () => {
    ...
    expect(result.code).toBe('DREAM_CONVERSATION_BUSY');
  ...
  it('falls back to DREAM_INTERNAL_ERROR for non-conflict 409 (different message)', () => {
    ...
    expect(result.code).toBe('DREAM_INTERNAL_ERROR');
    expect(result.retryable).toBe(true);
  });

  it('falls back to DREAM_INTERNAL_ERROR for non-HTTP errors', () => {
    const result = buildSendFailureError(new Error('boom'), 'boom');

    expect(result.code).toBe('DREAM_INTERNAL_ERROR');
    expect(result.ownership).toBe('dream');
    expect(result.retryable).toBe(true);
  });
