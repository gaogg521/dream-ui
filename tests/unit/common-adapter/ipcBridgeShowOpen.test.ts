...
vi.mock('@/common/adapter/httpBridge', () => {
  ...
  return {
    ...
    httpPatch: provider(),
    httpDelete: provider(),
    // Fork-only: calls that must always hit the local backend even when a
    // remote project-group server is connected (see getLocalBaseUrl).
    httpGetLocal: provider(),
    httpPostLocal: provider(),
    httpPutLocal: provider(),
    httpDeleteLocal: provider(),
    httpRequest: vi.fn(),
    getBaseUrl: vi.fn(() => ''),
