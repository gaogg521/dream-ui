...
describe('httpBridge', () => {
  ...
  describe('getBaseUrl', () => {
    ...
    it('preferLocalBackend uses local port even when enterprise remote is active', async () => {
      vi.stubGlobal('window', { __backendPort: 25001 });
      ...
      await httpRequest('POST', '/api/one/employee/agents', { name: 'x' }, { preferLocalBackend: true });
  ...
  describe('error handling', () => {
    ...
    it('aborts and throws REQUEST_TIMEOUT when the backend never responds', async () => {
      // Mock fetch that never resolves on its own but rejects when its
      // AbortSignal fires — mirrors how the real fetch reacts to abort().
      ...
      try {
        await httpRequest('GET', '/api/hang', undefined, { timeoutMs: 10 });
        expect.fail('Should have thrown');
      } catch (e) {
        expect(e).toBeInstanceOf(BackendHttpError);
        const err = e as BackendHttpError;
        ...
        expect(err.backendMessage).toContain('timed out');
      }
      expect(fetchSpy.mock.calls[0][1]?.signal).toBeInstanceOf(AbortSignal);
    });

    it('does not abort a fast response when a timeout is set', async () => {
      const fetchSpy = vi.fn().mockResolvedValue(
        new Response(JSON.stringify({ data: { ok: true } }), {
          status: 200,
          headers: { 'Content-Type': 'application/json' },
      ...
      const result = await httpRequest<{ ok: boolean }>('GET', '/api/fast', undefined, { timeoutMs: 5000 });
  ...
  describe('httpRequest', () => {
    it('performs fetch and unwraps data envelope', async () => {
      ...
      expect(fetchSpy).toHaveBeenCalledWith(expect.stringContaining('/api/test'), {
        ...
        headers: {},
        body: undefined,
        // A default request timeout now arms an AbortController per request.
