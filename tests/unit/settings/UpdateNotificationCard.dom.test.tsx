...
describe('UpdateNotificationCard', () => {
  ...
  it('restores a cached completed auto-update on mount', async () => {
    mocks.autoUpdateRestoreDownloadedMock.mockResolvedValue({
      ...
      data: {
        ready: true,
        version: '2.1.14',
        filePath: '/cache/pending/OneWork-2.1.14-mac.zip',
      },
    });
  ...
  it('says a release simply has no notes instead of spinning on "loading" forever', async () => {
    // The channel manifest carries no release notes (electron-builder omits the
    // field unless a notes resource exists at build time) and neither does the
    // sidecar — the panel must state that and still offer the release page.
    mocks.updateCheckMock.mockResolvedValue({
      success: true,
      data: {
        currentVersion: '2.1.13',
    ...
    fireEvent.click(await screen.findByText('update.releaseLog'));
    expect(await screen.findByText('update.viewRelease')).toBeInTheDocument();
