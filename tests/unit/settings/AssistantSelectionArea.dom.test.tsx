...
describe('AssistantSelectionArea', () => {
  it('uses the semantic success fill for the selected assistant', () => {
    render(
      <ConfigProvider>
    ...
    expect(screen.getByTestId('preset-pill-cowork')).toHaveStyle({
      background: 'rgb(var(--success-6))',
      color: '#fff',
    });
    expect(screen.getByTestId('preset-pill-cowork').className).toContain('agentSelectorActive');
    expect(screen.getByTestId('preset-pill-writer')).toHaveStyle({ background: 'transparent' });
  });
