...
import CodexBridgeModalContent from '@/renderer/pages/settings/CodexBridgeSettings/CodexBridgeModalContent';

describe('CodexBridgeModalContent', () => {
  beforeEach(() => {
    vi.clearAllMocks();
    messages.length = 0;
  ...
  it('loads and displays the saved provider/model when already configured', async () => {
    getConfigMock.mockResolvedValue({ enabled: true, provider_id: 'prov-1', model: 'model-a', configured: true });

    render(<CodexBridgeModalContent />);

    await waitFor(() => {
      expect(screen.getByTestId('codex-bridge-enable-switch')).toBeInTheDocument();
    });
    expect(screen.getByTestId('codex-bridge-enable-switch')).toHaveAttribute('aria-checked', 'true');
  ...
  it('shows a load-failure message and keeps the form usable when the config fetch rejects', async () => {
    ...
    await waitFor(() => {
      expect(messages).toContain('settings.codexBridge.loadFailed');
    });
    // Falls back to disabled/unconfigured rather than getting stuck loading.
    expect(screen.getByTestId('codex-bridge-enable-switch')).not.toBeDisabled();
    expect(screen.getByTestId('codex-bridge-enable-switch')).toHaveAttribute('aria-checked', 'false');
  });

  it('blocks saving when enabling without a selected provider and model', async () => {
    getConfigMock.mockResolvedValue({ enabled: false, provider_id: null, model: null, configured: false });
    const user = userEvent.setup();

    render(<CodexBridgeModalContent />);
    await waitFor(() => expect(screen.getByTestId('codex-bridge-enable-switch')).not.toBeDisabled());

    fireEvent.click(screen.getByTestId('codex-bridge-enable-switch'));
    await waitFor(() => {
      expect(screen.getByTestId('codex-bridge-enable-switch')).toHaveAttribute('aria-checked', 'true');
    });
    await user.click(screen.getByTestId('codex-bridge-save-button'));

    await waitFor(() => {
      expect(messages).toContain('settings.codexBridge.providerModelRequired');
    });
  ...
  it('saves the enabled config with the selected provider and model', async () => {
    getConfigMock.mockResolvedValue({ enabled: true, provider_id: 'prov-1', model: 'model-a', configured: true });
    setConfigMock.mockResolvedValue({ enabled: true, provider_id: 'prov-1', model: 'model-a', configured: true });
    const user = userEvent.setup();
    ...
    expect(messages).toContain('settings.codexBridge.saveSuccess');
  });

  it('resets the model selection when switching to a provider that lacks the current model', async () => {
    getConfigMock.mockResolvedValue({ enabled: true, provider_id: 'prov-1', model: 'model-a', configured: true });
    setConfigMock.mockResolvedValue({ enabled: true, provider_id: 'prov-2', model: 'model-c', configured: true });
    const user = userEvent.setup();

    render(<CodexBridgeModalContent />);
    await waitFor(() => expect(screen.getByTestId('codex-bridge-provider-select')).not.toBeDisabled());

    fireEvent.change(screen.getByTestId('codex-bridge-provider-select'), { target: { value: 'prov-2' } });
    await user.click(screen.getByTestId('codex-bridge-save-button'));
