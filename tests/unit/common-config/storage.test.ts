...
describe('storage runtime exports', () => {
  ...
  describe('EnvStorage', () => {
    ...
    it('set/get roundtrip with one.dir object', async () => {
      const dirs = {
        workDir: '/a',
        cacheDir: '/b',
        dataDir: '/c',
      ...
      await EnvStorage.set('one.dir', dirs);
      const result = await EnvStorage.get('one.dir');
  ...
  describe('storage isolation', () => {
    it('ConfigStorage and EnvStorage do not share state', async () => {
      ...
      const envDir = await EnvStorage.get('one.dir');
