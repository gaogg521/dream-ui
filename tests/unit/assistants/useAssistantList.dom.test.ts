...
describe('useAssistantList', () => {
  ...
  it('loads assistants on mount and selects first by default', async () => {
    ...
    const { result } = renderHook(() => useAssistantList(), { wrapper: swrWrapper });
  ...
  it('exposes loading=true until the first list fetch settles', async () => {
    let resolveList: (value: Assistant[]) => void = () => {};
    (ipcBridge.assistants.list.invoke as any).mockReturnValue(
    ...
    await act(async () => {
      resolveList([{ id: '1', name: 'A', sort_order: 1, source: 'user', enabled: true }]);
    });
  ...
  it('keeps empty list when load fails', async () => {
  ...
  it('reorders only enabled assistants through the shared preference', async () => {
    ...
    const initialList: Assistant[] = [
      ...
      { id: 'cli', name: 'CLI', sort_order: 4, source: 'generated', enabled: true, agent_status: 'online' },
    ];
    (ipcBridge.assistants.list.invoke as any).mockResolvedValue(initialList);
  ...
  it('restores the enabled order when preference persistence fails', async () => {
    ...
    const initialList: Assistant[] = [
      { id: 'official', name: 'Official', sort_order: 1, source: 'builtin', enabled: true },
      { id: 'cli', name: 'CLI', sort_order: 2, source: 'generated', enabled: true, agent_status: 'online' },
    ];
    (ipcBridge.assistants.list.invoke as any).mockResolvedValue(initialList);
