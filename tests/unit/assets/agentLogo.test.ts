...
describe('agentLogo', () => {
  ...
  describe('resolveAgentLogo (backend lookup)', () => {
    ...
    it('does not expose local absolute paths as logo sources', () => {
      expect(resolveAgentLogo(LOGOS, { icon: '/Users/demo/.one/agent-avatars/custom.png' })).toBeNull();
    });
  });
  ...
  describe('resolveAgentAvatar', () => {
    ...
    it('falls back to backend catalog logos when explicit local paths leak through', () => {
      expect(
        resolveAgentAvatar(LOGOS, { icon: '/Users/demo/.one/agent-avatars/custom.png', backend: 'claude' })
      ).toEqual({
        kind: 'image',
