...
// Cold import of this viewer's dependency graph was measured at 14568ms with
// only the preview suites running, i.e. already half of the previous 30000ms
// budget before the other ~430 files start competing for CPU. Every later test
// in this file that re-imports the module costs ~1ms, so this one test carries
// the whole graph. It cannot hang (a module import resolves or throws), so the
// budget is simply sized off the measured cost with headroom.
 ...
 * officecli has to be installed on the machine running Dream UI, so the error
