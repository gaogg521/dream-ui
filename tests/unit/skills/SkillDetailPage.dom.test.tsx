...
import SkillDetailPage from '@/renderer/pages/settings/skillsHub/SkillDetailPage';
import { getAssistantsUsingSkill } from '@/renderer/pages/settings/skillsHub/SkillUsedByStack';
...
describe('SkillDetailPage', () => {
  ...
  it('removes the duplicate skill name from the breadcrumb and auto-selects SKILL.md', async () => {
    ...
    // The markdown pane renders into a shadow root for style isolation (see
    // ShadowView), so its text is not reachable through the light-DOM
    // textContent assertions the rest of this suite uses — the auto-select
    // itself is the thing under test, and readSkillFile being called with
    // SKILL.md (not some other file, and not left unselected) proves it.
    await waitFor(() =>
