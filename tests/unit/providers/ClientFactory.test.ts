...
describe('ClientFactory', () => {
  ...
  describe('createRotatingClient', () => {
    ...
    /**
     * These two headers are how OpenRouter-style gateways label the caller on
     * their own public app listings, so they are a **display name**, not an
     * identifier anything is keyed on (unlike `appId` or the userData folder,
     * which are pinned to historical values because changing them loses user
     * data). Reporting this fork under the upstream project's name would
     * attribute its traffic to someone else's entry.
     */
    it('identifies this app, not upstream, to gateways that list callers', async () => {
      await ClientFactory.createRotatingClient(mockProvider);
      const calls = (OpenAIRotatingClient as any).mock.calls;
      const config = calls[0][1];
