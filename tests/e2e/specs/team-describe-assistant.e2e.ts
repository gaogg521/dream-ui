...
/** Backend /api/teams/:id GET response shape — aligns with dreamcore schema. */
  ...
  test('describes a preset and then spawns it as a teammate via the real TCP bridge', async ({ page }) => {
    ...
    try {
      ...
      try {
        ...
        expect(describeText).toContain('Backend: dream-engine');
        ...
        expect(spawned!.assistant_backend ?? spawned!.backend).toBe('dream-engine'); // preset backend wins
      } finally {
        client.close();
      }
    } finally {
