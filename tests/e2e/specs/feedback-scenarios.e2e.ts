...
/** Close the feedback modal (DreamModal sets closable=false so Escape is a no-op). */
async function closeFeedbackModal(page: Page) {
  // The feedback modal is an DreamModal (standard variant); its header close
...
/** Close any open DreamModal (e.g. the Agent editor) so the next test starts clean. */
...
async function openCustomAgentEditor(page: Page, command: string) {
  // Defensive: close any DreamModal left over from a prior test so the
