...
- 上传后显示文件预览卡片（`DreamEngineSendBox.tsx:413-421`）
...
- `src/renderer/pages/conversation/platforms/dreamEngine/useDreamEngineModelSelection.ts:34-40`:
  ```typescript
...
- 对话页: `DreamEngineSendBox.tsx:391-401` — 同样使用 `AgentModeSelector`
...
- 操作: 点击对话页 `DreamEngineModelSelector` 按钮 → 选择不同模型
...
3. **源码追溯补充**:
   ...
   - `src/renderer/pages/conversation/platforms/dreamEngine/useDreamEngineModelSelection.ts:34-40` — 过滤 Google Auth
