...
**Critical bottleneck**: `workers: 1` enforced + shared `one.db` → parallel execution impossible without refactoring.
...
1. Extend `ONE_E2E_TEST` to include worker ID:
   ```typescript
   ONE_E2E_TEST_WORKER_ID: process.env.PLAYWRIGHT_WORKER_INDEX || '0';
   ```
2. Modify `getDevAppName()` to return worker-specific name:
   ```typescript
   const workerId = process.env.ONE_E2E_TEST_WORKER_ID || '0';
   return `OneWork-E2E-Worker-${workerId}`;
   ```
3. Each worker gets isolated:
   - `~/Library/Application Support/OneWork-E2E-Worker-0/config/one.db`
   - `~/Library/Application Support/OneWork-E2E-Worker-1/config/one.db`
...
**Complexity**: High (requires main process to read `ONE_DATABASE_PATH`, conflicts with userData convention)
