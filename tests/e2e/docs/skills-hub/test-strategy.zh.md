...
- ✅ SQLite 数据库独立(`one.db`)
- ✅ 扩展路径指向示例目录(`ONE_EXTENSIONS_PATH=examples/`)
...
// Returns:
// {
//   userSkillsDir: '~/.one/skills/',     // My Skills 存储目录
...
- E2E 框架已设置 `ONE_EXTENSIONS_PATH=examples/`(`fixtures.ts:112`)
...
```typescript
// Setup: 创建临时目录
const tempDir = fs.mkdtempSync(path.join(os.tmpdir(), 'one-e2e-custom-'));
...
| `auto-skill-card-${normalizeTestId(skill.name)}`      | 自动注入技能卡片            | line ~520 `<div className="skill-card">`        | `auto-skill-card-one-skills`          |
