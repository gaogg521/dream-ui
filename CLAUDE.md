...
| 文档                                                                                                                                                         | 说明                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                       |
| ------------------------------------------------------------------------------------------------------------------------------------------------------------ | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------ |
| [session-2026-09-15-web-search-and-hosted-broker.zh-CN.md](./docs/guides/session-2026-09-15-web-search-and-hosted-broker.zh-CN.md)                           | **最新**：联网搜索从零接入（此前三层都没有：引擎 9 个工具不碰网络、`ModelType::WebSearch` 只是按模型名打的 UI 标签、`one-browser` 是自动化不是搜索）——统一内置 MCP `one-web-search` + 9 家服务商 + 自定义端点 + 设置页入口。修掉的真实 bug：内置 MCP 要进 dream-core 白名单才进会话（`enabled` 只是必要条件）、`updateServer` 静默忽略 `enabled`（只有 `toggleServer` 能改，且是翻转不是设置）、豆包指向方舟/阿里云指向 IQS（**两次都是被"401 不是 404 就说明端点对"骗的** —— 探测只能证明有服务在，不能证明是你要的那个）、火山 PascalCase 字段匹配不上、智谱发裸 key、判别式联合不收窄（tsconfig 其实没开 strict，AGENTS.md 那句话是错的）。「把 Tavily key 内置进 App」最终否决 —— 公开仓库 + asar 可解包 = 客户端藏不住密钥，改走 broker mode C（`POST /v1/search`，先预留再调用、失败退还、全局当日闸）。**线上还没开 mode C，见 §6** |
| [session-2026-09-11-i18n-skills-ui-agnes-media.zh-CN.md](./docs/guides/session-2026-09-11-i18n-skills-ui-agnes-media.zh-CN.md)                               | 专家市场 252 人 + 技能 141 条三语本地化（语料无 locale 字段，走前端覆盖层；技能 `description` 是模型可见的，刻意不翻）+ 卡片排版（`h-full` 在 auto 行轨上是循环依赖、max-content 不认 `line-clamp`；`content-visibility: auto` 固定加 19px 且 `contain-intrinsic-size` 无解 —— 技能卡 227→157px）+ Agnes 媒体接入（video 2.5 禁掉 2.0 全部尺寸字段；image 与 OpenAI body 有四处不兼容，`400 n must be 1` 只是第一个暴露的）+ 安装器不再让用户关一个 Windows 没点名的进程 + CI 不再把 macOS 签名身份塞给 Windows 构建。**贯穿全天只有一个 bug 类型：同一条规则写在两处、或对没测过的东西写乐观断言；分类表写了 141 条其中 114 条从没被读到过，见 §2**                                                                                                                                                                                       |
| [session-2026-09-09-package-size-and-install-speed.zh-CN.md](./docs/guides/session-2026-09-09-package-size-and-install-speed.zh-CN.md)                       | 安装包瘦身（win x64 装机 2,279.9 → 1,387.2 MB，安装包 462.4 → 340.4 MB）—— 删掉 dream-core 从不 spawn 的 `cli/` 原生二进制（−696 MB，含一份 SHA256 相同的 codex.exe 重复）+ 渲染层专属依赖不进 asar（−119 MB，Vite 已编译进 out/renderer）+ better-sqlite3 编译产物 + locale 对齐 13 种 UI 语言（改用内置 `electronLanguages`，Mac 首次受益）；`useZip` 实测后否决（11.4 s 换 +170 MB 不划算）、改开差分更新（arm64 例外保 zip 载荷）。顺带修一处既有 bug：`require('@process/...')` 别名 Rollup 不重写 → 媒体工作目录永远回退 cwd。**自查抓到两个本轮自伤的 bug，见 §9**                                                                                                                                                                                                                                                                  |
| [session-2026-09-04-enterprise-entry-consolidation.zh-CN.md](./docs/guides/session-2026-09-04-enterprise-entry-consolidation.zh-CN.md)                       | 企业入口信息架构收敛（部署卡从「远程连接」搬进「企业身份」，地址+连接开关必须整体搬）+ 「本机作为服务器」置灰（企业版拆分后本地建组必 501，但保留可见让存量 server 状态能切回）+ 侧边栏「企业管理后台」入口改按「是否已连接企业」判定（原来依赖 isServer，客户端永远看不到）+ SSO 按钮置灰时说明原因（原来 disabled 让解释文案永远触发不了）+ 模型平台预设改为从后端 `/api/model-platforms` 原地 merge 同步（刻意不改任何现有调用点）。**三仓完整交接见 dream-en/docs/handoff-2026-09-04-enterprise-capability-distribution.zh-CN.md**                                                                                                                                                                                                                                                                                                     |
...
| [session-2026-09-01-aion-residue-cleanup-and-onework-rename.zh-CN.md](./docs/guides/session-2026-09-01-aion-residue-cleanup-and-onework-rename.zh-CN.md)     | aion 残留全量清理（P4 映射表漏了小写 `aioncore` 是残留主因）+ `executableName: onework`（进程/安装目录无空格，NSIS patcher 需兼容已打旧补丁的 node_modules）+ moltbook 移除 + 外部契约逐项判定（hub 目录来自上游 iOfficeAI/AionHub 打包时下载，本地改了会被覆盖 → 保留并记录自建步骤）；顺手修 cron `resolve_model` 旧值比较、Dockerfile/compose 旧环境变量名、自动更新默认仓库指向上游三个真实 bug。运行时契约全走「新值写入 + 旧值兼容读」。                                                                                                                                                                                                                                                                                                                                                                                             |
