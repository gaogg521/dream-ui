...
### 11.17 充值 / 用量查询的产品级 UI 重做（2026-09-24 晚，接 §11.16）

§11.16 修完崩溃之后用户提了一句关键的产品意见：这两个入口"看起来像开发者临时
拼的，不像菜单功能"。顺着改了三件事：
...
- 多把 key 轮询时只送第一把；没有 key 就原样打开让页面自己问（分享链接也是这个
  形态）。
- ⚠️ **坑**：浏览器第二次打开同一个页面时只换 fragment、不重新加载文档，所以
  只在 load 时读一次的话，第二次点击看起来完全没反应。加了 `hashchange`
  监听才真正可用——这个是实测出来的，不是想出来的。

**2. 用量页面重做**：剩余额度做主数字 + 消耗进度条，已用 / 总额度次级；调用记录
改成真表格（相对日期、token 数、单次花费、截断的 request id）；补了暗色模式、
移动端布局；报错文案从"未找到匹配的有效 Key"改成"没有找到这把 Key。请确认粘贴
完整，并且它还没有被停用或替换。"

**3. 充值弹窗重做**：原来第一行是"选择金额"这个标签，现在第一行是**这笔交易要
改变的那个数字**——当前余额，选了金额之后就地显示"充值后 ¥36.00"，按钮文案也跟着
变成"支付 ¥20.00"（最后点的那一下明确说明在付多少）。快捷金额从小圆角 chip 改成
4 格网格，支付步骤给二维码留足空间、金额放在上方。

**dream-ui 侧同批修的一个大面积问题**（提交 `b7a2a9a`）：Arco 的调色板变量存的是
**逗号分隔**通道（`--primary-6` 是 `22,93,255`），而 UnoCSS 把
`text-[rgb(var(--primary-6))]` 编译成 `rgb(var(--primary-6) / var(--un-text-opacity))`
这种**空格语法**——逗号让它非法，浏览器直接丢掉整条声明。**全仓 41 处、17 个文件
全是死的**（成功勾、危险色文字、警告条、会话 minimap 的选中态，全部渲染成继承的
默认色）。改成 `rgba(var(--x),1)` 并加了守卫测试。判据是在运行中的 app 里实测：
`rgb(var(--primary-6) / 1)` 算出来是继承色，`rgba(var(--primary-6), 1)` 才是真色。

### 11.18 用量页重做成完整仪表盘 + 回归揪出两个真问题 + 左栏按产品判断改双模式（2026-09-25）

用户拿 fushuo 那类 NewAPI 控制台当参考，要求把"一张孤卡"的首页做丰富。摸底结论
决定了做法：`usage_by_key` 每次实时从宝云拉日志（分页上限 20×100=2000 条，按
token_id 过滤），且 `UsageLogView` 的 `use_time_ms`、`is_stream` 页面从来没用过
——所以**零后端改动**，前端自己聚合，`usage.html` 单文件重写，依旧 `include_str!`
零依赖，图表全部手写 SVG/CSS。左栏（品牌+Key）+ 右栏仪表盘（状态横幅、四统计卡、
四图表标签页、明细表格）；表格把 `use_time_ms`（用时）和 `is_stream`（流式角标）
用上，加类型/模型筛选、分页、CSV 导出（带 BOM）。

- ⚠️ `use_time` 的单位宝云没标（代码里 `_ms` 只是推断）。渲染时数据驱动判读：
  任一值 >400 整批按毫秒，否则按秒——LLM 调用不可能几百毫秒完成。
- 两个"花费"刻意分开叫：统计卡「累计花费」= 厂商权威 `used_usd`；环形图中心
  「窗口内扣费」= 本页记录合计。最新一笔没出账时两者对不齐，分开叫不显得矛盾。
- 「最近查询」显式 opt-in：勾选才写 localStorage，取消勾选即清，默认维持 §11.15
  "不留痕"的承诺。
...
2. 刷新时无条件回到 intro，每次点刷新整页闪回首页。修复：仪表盘保持挂载，失败时
   错误卡叠在上面（`958918b`）。

**左栏双模式（接 §11.17 的产品判断）**：fragment 一键带入是主路径，粘贴只是兜底
——但左栏仍把"粘贴 API Key"摆在最显眼处，对从 app 进来的用户正是那个"多余"。
现在 fragment 带入时左栏切成"已自动带入 Key"安静态（对勾+尾号+fragment 不经服务
