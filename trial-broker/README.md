...
| `DAILY_BUDGET_USD_CAP`           | no       | `50.0`                                            | Circuit breaker on _new liability handed out per day_ — at the defaults, at most 50 new trial users per day.                                                                                               |
...
```json
{
  ...
  "models": ["openrouter/free", "openrouter/auto-beta", "~deepseek/deepseek-v4-flash-latest"]
}
```
...
slot is refunded only when _no_ provider managed to search at all; an empty
...
{
  "provider": "tavily",
  "results": [{ "title": "…", "url": "https://…", "snippet": "…", "published_at": "2026-01-02" }],
...
A slot is reserved _before_ the upstream call (so concurrent requests cannot
...
`liability_added_today_usd` is liability _created_ today, not spend incurred:
